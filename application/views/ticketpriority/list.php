...
<ul class="page-breadcrumb breadcrumb">
 ...
 <?php if($this->input->get('ins')==1){?>  
              ...
              <?php if($this->input->get('del')==1){?> 
               <div class="alert alert-success alert-dismissable" id="del">
                <button aria-hidden="true" data-dismiss="alert" class="close" type="button"></button>
...
<form name="form1" method="post" id="form1" action="<?php  echo $this->config->site_url();?>/ticketpriority/delete?rows=<?php echo $rows;?>" >

<div class="right-btns">
<a href="<?php echo base_url();?>index.php/ticketpriority/add" class="attach-subs tools"><img src="<?php echo base_url('assets/images/plus.png');?>">Add New</a>
...
<th><div class="checker"><span><input type="checkbox" class="group-checkable" id="selectall"></span></div></th>
 <th>
                                    <a href="<?php echo $this->config->site_url();?>/ticketpriority?sort_by=<?php if($sort_by=='asc'){echo 'desc';}else{echo 'asc';}?>&searchterm=<?php echo $searchterm;?>&rows=<?php echo $rows;?>">Ticket Priority</a></th>
                                    <th>Status</th>
...
<?php 
	...
	<div class="checker">
     ...
     <td>
									...
									<?php if($result['status']==1){?>
	   	                             <span class="label label-success"> <a href="<?php echo $this->config->site_url();?>/ticketpriority/changestat/<?php echo $result['ticket_priority_id']?>?stat=0">Active</a> </span>
                                    <?php } ?>
                                    
    								<?php if($result['status']==0){?>
                                    <span class="label label-danger"> <a href="<?php echo $this->config->site_url();?>/ticketpriority/changestat/<?php echo $result['ticket_priority_id']?>?stat=1">Inactive</a> </span>
                                   <?php } ?>
    ...
    <td>
     ...
     <a href="<?php  echo $this->config->site_url();?>/ticketpriority/edit/<?php echo $result['ticket_priority_id']?>" class="views" title="Edit"><img src="<?php echo base_url('assets/images/edits.png');?>"></a>
			<a href="<?php  echo $this->config->site_url();?>/ticketpriority/delete/<?php echo $result['ticket_priority_id']?>" class="views" title="Delete" onclick="return confirm('Are you sure you want to delete?')"><img src="<?php echo base_url('assets/images/deletes.png');?>"></a>
    </td>
    </tr>
    <?php }
  }else{?>
...
$(document).ready(function()
{
	...
	$("#search").click(function(){
		...
		window.location.href = '<?php echo $this->config->site_url();?>/ticketpriority?searchterm='+searchterm;
	});
	$("#sel_limit1").change(function(){
		var limits = $(this).find(":selected").val();
		window.location.href = '<?php echo $this->config->site_url();?>/ticketpriority?limit='+limits;
	});
