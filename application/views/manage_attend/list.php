...
<form name="form1" method="post" id="form1" action="<?php  echo $this->config->site_url();?>/manage_attend/multidelete?rows=<?php echo $rows;?>" >

<div class="right-btns">
<a href="<?php echo base_url();?>index.php/manage_attend/add" class="attach-subs tools"><img src="<?php echo base_url('assets/images/plus.png');?>">Add New</a>
...
<th><div class="checker"><span><input type="checkbox" class="group-checkable" id="selectall"></span></div></th>
                         <th>
                         <a href="<?php echo $this->config->site_url();?>/manage_attend?sort_by=<?php if($sort_by=='asc'){echo 'desc';}else{echo 'asc';}?>&searchterm=<?php echo $searchterm;?>&rows=<?php echo $rows;?>">Name</a>
						 </th>
						 <th>Date</th>
						  <th>Time in (24 hr format)</th>
						  <th>Time out (24 hr format)</th>
						  <th>Approve</th>
						  <th>Approved By</th>
						  <th>Actions</th>
...
?>
		<tr>
			<td align="center"><input type="checkbox" name="checkbox[]"  class="checkboxes" value="<?php echo $result['atten_id']?>" ></td>
					<td><?php echo $result['first_name']?></td>
					<td><?php echo $result['date']?></td>
					<td><?php echo $result['time_in']?></td>
					<td><?php echo $result['time_out']?></td>
					<td><?php if($result['approved_by']==0){ ?><a href="<?php echo base_url();?>index.php/manage_attend/statchange/<?php echo $result['atten_id']?>"> Approve </a><?php } 
                    else echo "Approved"; ?></td>
					<td><?php foreach($approved_by as $app)
                    {
						if($app->candidate_id == $result['approved_by'])
						{
						echo $app->first_name;echo $app->last_name; 
						}
					}
                    //~ echo $result['fname']
                    ?></td>
            ...
            <td>
             <a href="<?php echo base_url();?>index.php/manage_attend/edit/<?php echo $result['atten_id']?>" class="views" title="Edit"><img src="<?php echo base_url('assets/images/edits.png');?>"></a>
			<a href="<?php echo base_url();?>index.php/manage_attend/delete/<?php echo $result['atten_id']?>" class="views" title="Delete" onclick="return confirm('Are you sure you want to delete?')"><img src="<?php echo base_url('assets/images/deletes.png');?>"></a>
		   </td>
		</tr>
...
$(document).ready(function()
{
	...
	$("#search").click(function(){
		...
		window.location.href = '<?php echo $this->config->site_url();?>/manage_attend?searchterm='+searchterm;
	});
	$("#sel_limit1").change(function(){
		var limits = $(this).find(":selected").val();
		window.location.href = '<?php echo $this->config->site_url();?>/manage_attend?limit='+limits;
	});
