...
<a href="<?php echo base_url();?>index.php/tasks/add" class="attach-subs tools"><img src="<?php echo base_url('assets/images/plus.png');?>">Add New</a>
<a href="#" id="deleteall" class="attach-subs tools"><img src="<?php echo base_url('assets/images/deletes_new.png');?>">Delete</a>
</div>


<form id="searchForm" method="get" action="<?php  echo $this->config->site_url();?>/tasks/">
<table class="tool-table">
<tbody>
<tr>
<td><input class="form-control" type="text" name="searchterm" value="<?php echo $searchterm;?>" id="search_term" placeholder="Search Task Title" style="width: 185px;">&nbsp;<?php  echo form_dropdown('candidate_id',  $admin_users_lists, $candidate_id,'class="form-control" id="candidate_id"');?> </td>
...
<td><input type="radio" name="date_range" id="date_range" value="2" <?php if($date_range==2)echo 'checked="checked"';?> /> Due on -- <?php echo date('d M Y');?></td>
...
<td colspan="6"><table width="100%" border="1">
  <tr>
    ...
    <td><input type="radio" name="status" id="status" value="3" <?php if($status==3)echo 'checked="checked"';?> />Hold</td>
    <td><input type="radio" name="status" id="status" value="" <?php if($status=='')echo 'checked="checked"';?> />All</td>
    <td>&nbsp;</td>
    ...
    <td><input type="radio" name="priority" id="priority" value="" <?php if($priority=='')echo 'checked="checked"';?> />All</td>
  </tr>
</table></td>
...
<form name="form1" method="post" id="form1" action="<?php  echo $this->config->site_url();?>/tasks/multidelete?rows=<?php echo $rows;?>" >
<div class="sep-bar">
<div class="page">
<?php echo $pagination; ?>
</div>
...
<tr role="row" class="heading">
    ...
    <th> <a href="<?php echo $this->config->site_url()?>/tasks?sort_by=<?php if($sort_by=='asc'){echo 'desc';}else{echo 'asc';};?>&searchterm=<?php echo $searchterm;?>&rows=<?php echo $rows;?>">Task Title</a></th>
    <th>Priority</th>
    <th>User</th>
    <th>Status</th>
    <th>Start </th>
...
foreach($records as $result){ ?>
                   ...
                   <td align="center"><input type="checkbox" name="checkbox[]" class="checkboxes" value="<?php echo $result['task_id']?>" ></td>
                            ...
                            <a class="label label-sm label-success" href="<?php echo $this->config->site_url();?>/tasks/changestat/<?php echo $result['task_id']?>?stat=0&searchterm=<?php echo $searchterm;?>&date_range=<?php echo $date_range;?>">Open</a>
                            <?php } ?>
                            
                            <?php if($result['status']==0){?>
                            <a class="label label-sm label-danger" href="<?php echo $this->config->site_url();?>/tasks/changestat/<?php echo $result['task_id']?>?stat=1&searchterm=<?php echo $searchterm;?>&date_range=<?php echo $date_range;?>">Closed</a>
                            <?php } ?>                            </td>
                               <td><a href="<?php  echo $this->config->site_url();?>/tasks/followup/<?php echo $result["task_id"]; ?>">Follow-up</a></td>
                    
                               <td>
                    <a href="<?php echo base_url();?>index.php/tasks/edit/<?php echo $result['task_id']?>" class="views" title="Edit"><img src="<?php echo base_url('assets/images/edits.png');?>"></a>
                    
                    <a href="<?php echo base_url();?>index.php/tasks/delete/<?php echo $result['task_id']?>" class="views" title="Delete" onclick="return confirm('Are you sure you want to delete?')"><img src="<?php echo base_url('assets/images/deletes.png');?>"></a>
                      <a class="tooltips" data-original-title="Follow up" href="<?php  echo $this->config->site_url();?>/tasks/followup/<?php echo $result["task_id"]; ?>"><i class="icon-share-alt"></i></a>
                           
                            <a class="tooltips" data-original-title="Attachments" href="<?php  echo $this->config->site_url();?>/tasks/files/<?php echo $result["task_id"]; ?>"><i class="icon-folder-open"></i></a>
...
$(document).ready(function()
{
	...
	$("#search").click(function(){
		...
		window.location.href = '<?php echo $this->config->site_url();?>/tasks?searchterm='+searchterm;
	});
	$("#sel_limit1").change(function(){
		var limits = $(this).find(":selected").val();
		window.location.href = '<?php echo $this->config->site_url();?>/tasks?limit='+limits;
	});
