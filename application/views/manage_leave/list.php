...
for ($i=0; $i<($maxday+$startday); $i++) 
    ...
    else 
    {
		echo "<td align='center' style='padding:25px;' valign='middle' height='20px'>". ($i - $startday + 1);
			...
			$qry="SELECT a.*, b.first_name, b.last_name FROM `pms_admin_leave` a, pms_candidate b WHERE '$dte' >= date_from and '$dte' <= date_to and a.candidate_id=b.candidate_id and a.leave_status=2";
			...
			if(isset($rest) && !empty($rest))
			{
				foreach($rest as $res)
				{
					echo "<br>".$res->first_name." ".$res->last_name;
				}
			}
		echo "</td>";
