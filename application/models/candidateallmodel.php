...
class Candidateallmodel extends CI_Model {
	...
	function record_count($search_email,$search_name,$search_mobile,$reg_status,$branch_id,$skills,$level_id,$course_id,$spcl_id,$exp_years) 
	{
		$sql="select count(a.candidate_id)as total_rec  from pms_candidate a LEFT JOIN pms_candidate_to_skills_primary c on a.candidate_id=c.candidate_id LEFT JOIN pms_candidate_education d on a.candidate_id=d.candidate_id LEFT JOIN pms_candidate_contract g on a.candidate_id=g.candidate_id";
		$cond='';

		if($exp_years!='')
		{
			if($cond!='')
				$cond.=" and g.total_exp >=". $exp_years ;
			else
				$cond ="  g.total_exp >=". $exp_years ;
...
//specialisation
	...
	function get_list($start,$limit,$search_email,$search_name,$search_mobile,$sort_by,$reg_status,$branch_id,$skills,$level_id,$course_id,$spcl_id,$exp_years)
	{

		$sql="SELECT a.candidate_id, a.first_name, a.last_name, a.username,a.reg_date,a.lead_opportunity, a.mobile,c.skill_id FROM ".$this->table_name." a LEFT JOIN pms_candidate_to_skills_primary c on a.candidate_id=c.candidate_id LEFT JOIN pms_candidate_education d on a.candidate_id=d.candidate_id LEFT JOIN pms_candidate_contract g on a.candidate_id=g.candidate_id";
		$cond='';
...
//specialisation
   ...
   function get_candidate_id($string_hash){
	   
	   $query = $this->db->query("select a.candidate_id,b.job_app_id,c.short_id from pms_candidate a inner join pms_job_apps b on a.candidate_id=b.candidate_id inner join pms_job_apps_shortlisted c on c.app_id=b.job_app_id where md5(concat(b.candidate_id,b.job_app_id,c.short_id))='".$string_hash."'");
...
function all_func_list()
	...
	function add_feedback_url($app_id,$candidate_id,$short_id,$data)
	{
		$this->db->where('md5(candidate_id)', $candidate_id);
		$this->db->where('md5(app_id)', $app_id);
		$this->db->where('md5(short_id)', $short_id);
		$this->db->update('pms_job_apps_shortlisted', $data);
		return 0;
	}

	function add_feedback_form($app_id,$candidate_id,$short_id,$data)
	{
		$this->db->where('candidate_id', $candidate_id);
		$this->db->where('app_id', $app_id);
		$this->db->where('short_id', $short_id);
		$this->db->update('pms_job_apps_shortlisted', $data);
		return 0;
	}
   ...
   {
   		$query = $this->db->query('select a.* from pms_candidate a left join pms_candidate_address b on a.candidate_id=b.candidate_id where a.candidate_id='.$candidate_id);
	return $query->row_array();
   }
  
   function education_deatils($candidate_id)
   {
   		$query = $this->db->query('select a.*,b.course_name,c.level_name,d.spcl_name,e.course_type,f.country_name from pms_candidate_education a left join pms_courses b on a.course_id=b.course_id left join pms_education_level c on a.level_id=c.level_id left join pms_specialisation d on d.spcl_id=a.spcl_id left join pms_course_type e on e.course_type_id=a.course_type_id left join pms_country f on a.edu_country=f.country_id where a.candidate_id='.$candidate_id.' order by a.eucation_id');
		return $query->result_array();
   }   
...
function select_aplication_visa($candidate_id)
    ...
    {
        $query=$this->db->query("select a.*,b.*,c.*,d.* from pms_candidate_job_profile a left join pms_job_category b on a.job_cat_id=b.job_cat_id left join pms_job_functional_area c on a.func_id=c.func_id left join pms_designation d on a.desig_id=d.desig_id where a.candidate_id=".$candidate_id." order by a.from_date desc");
		return $query->result_array();
	}	

	 function all_messages($candidate_id)
    {
		$query = $this->db->query("SELECT a.*,b.first_name,c.firstname as recruiter,c.admin_id FROM pms_candidate_messages a inner join pms_candidate b on a.candidate_id=b.candidate_id left join pms_admin_users c on a.admin_id=c.admin_id where a.candidate_id=".$candidate_id." order by a.message_id desc");
		...
		{
			$message_list[]=array('message_id' => $main['message_id'],'message_title' => $main['message_title'],'message_text' => $main['message_text'],'message_date' => $main['message_date'],'message_time' => $main['message_time'],'message_status' => $main['message_status'],'candidate_id' => $main['candidate_id'],'first_name' => $main['first_name'], 'recruiter' => $main['recruiter'], 'admin_id' => $main['admin_id']);
		}
		return $message_list;		
	}	
...
$this->db->query("update pms_candidate_files set file_name='no_photo.png',file_type='no_photo.png' where file_name='".$row['photo']."' and candidate_id=".$id);
	...
	function location_list_by_state($state_id='')
    {
		...
		if($state_id !='')
			$query=$this->db->query("select * from pms_city where state_id=".$state_id);
		else
...
//EDIT PASSPORT NUMBER AND TYPE	
	...
	function get_profession($candidateId)
	{
		$query=$this->db->query("select a.*,b.exp_years,b.exp_months,c.job_cat_name,d.func_area from pms_candidate_job_profile a inner join pms_candidate b on a.candidate_id=b.candidate_id left join pms_job_category c on a.job_cat_id=c.job_cat_id left join pms_job_functional_area d on a.func_id=d.func_id where a.candidate_id=$candidateId order by a.job_profile_id desc");
		return $query->result_array();
	}

	function get_consultant_feedback($candidate_id)
	{
		$query=$this->db->query("select a.* from pms_candidate_job_search a where a.candidate_id=".$candidate_id);
		return $query->row_array();
	}
	...
	function assign_admin_user($candidateId,$adminId){
		$query = $this->db->query("select id from pms_admin_candidates where candidate_id=".$adminId." and candidate_id=".$candidateId);		
		if ($query->num_rows() == 0){
		
			$this->db->where('candidate_id',$adminId);
			$this->db->where('candidate_id',$candidateId);
			$this->db->delete('pms_admin_candidates');
	...
	function getAdminEmail($adminId){
		$query = $this->db->query('select email,first_name from pms_candidate where candidate_id='.$adminId);
		return $query->row_array();
	}
	function get_admin_users_lists(){
	  $query = $this->db->query('select distinct candidate_id, first_name, last_name from pms_candidate order by candidate_id asc');
	  $dropdowns = $query->result();
...
// select jobs based on industry(job category) and functional area
		$query = $this->db->query("select a.job_title,a.job_expiry_date,a.job_id,a.job_post_date from pms_jobs a order by a.job_post_date desc");
...
//getting jobs applied by candidate
	...
	function get_job_details($candidate_id,$job_app_id)
	{
		if($candidate_id=='')return;
		
		$query = $this->db->query('select DISTINCT a.*,b.* from pms_jobs a inner join pms_job_apps b on a.job_id=b.job_id where b.candidate_id='.$candidate_id.' and b.job_app_id='.$job_app_id);

		$job_row = $query->row_array();	
		return $job_row;
	}
...
//STOP	
    ...
    {
        $query=$this->db->query("select a.*,b.username from pms_candidate_followup a left join pms_candidate b on a.assigned_to=b.candidate_id where a.candidate_id=".$candidate_id);
		return $query->result_array();
	}	
}
