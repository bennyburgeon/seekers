...
//insert job planning details
	function insert_job_search($candidateId){
		$data = array(
				'candidate_id' => $candidateId,
...
//FETCHING Colleges
   ...
   function get_cert()
   	{
	  $query=$this->db->query("select * from pms_candidate_certification order by cert_name asc");
	  return $query->result_array();
	}
...
//FETCHING ALL DOMAINS
	function get_domains()
  	 {
	  $query=$this->db->query("select * from pms_candidate_domain order by domain_name asc");
	  return $query->result_array();
	}
	...
	{
		$sql="select * from pms_jobs order by job_title asc";
