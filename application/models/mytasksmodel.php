...
class Mytasksmodel extends CI_Model{
	...
	function record_count($searchterm,$date_range,$priority,$status) 
	{
		...
		$cond='';
		$cond .=" a.candidate_id=" . $_SESSION['candidate_session'];
	...
	{
		$data = array();
		$query=$this->db->query("select milestone_id,milestone_title from ".$this->project_milestone." where project_id='7'");
		$dropDownList = array();
	...
	{
		$data = array();
		$query=$this->db->query("select candidate_id,username from pms_candidate");
		$dropDownList = array();
		...
		{
			$dropDownList[$dropdown->candidate_id] = $dropdown->username;
		}
	...
	{
		$query = $this->db->query("SELECT a.user_id,b.first_name from ".$this->project_team." a JOIN ".$this->admin_users." b on a.user_id=b.candidate_id where a.project_id='7'");
		$dropDownList = array();
