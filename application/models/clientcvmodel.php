<?php 
class Clientcvmodel extends CI_Model {
	var $table_name='';
...
//specialisation
   ...
   function get_profile_rms($string_hash){
	   
	   $query = $this->db->query("select a.candidate_id from pms_candidate a inner join pms_job_apps b on a.candidate_id=b.candidate_id where md5(a.candidate_id)='".$string_hash."'");
...
//  suggestion_logic start from here
    ...
    {
        $query=$this->db->query("select a.*,b.level_name,c.*,d.spcl_name,a.univ_name,f.course_type,g.country_name from pms_candidate_education a left join pms_education_level b on a.level_id=b.level_id left join pms_courses c on a.
course_id=c.course_id left join pms_specialisation d on a.spcl_id=d.spcl_id left join pms_university e on a.univ_id=e.univ_id left join pms_course_type f on a.course_type_id=f.course_type_id left join pms_country g on a.edu_country=g.country_id where a.candidate_id=".$candidate_id);
		return $query->result_array();
	}
...
//EDIT PASSPORT NUMBER AND TYPE	
	...
	{
		$query=$this->db->query("select a.*,b.exp_years,b.exp_months,c.job_cat_name,d.func_area,e.city_name from pms_candidate_job_profile a inner join pms_candidate b on a.candidate_id=b.candidate_id left join pms_job_category c on a.job_cat_id=c.job_cat_id left join pms_job_functional_area d on a.func_id=d.func_id left join pms_city e on a.city_id=e.city_id where a.candidate_id=$candidateId order by a.present_job desc, a.from_date desc");
		return $query->result_array();
	}
	...
	function get_single_record($candidateId){
		...
		$query=$this->db->query("select a.*,c.visa_type, b.country_name,(select country_name from pms_country vc where vc.country_id=a.driving_license_country)as  driving_license_issuance,(select city_name from pms_city cl where cl.city_id=a.city_id)as  current_location,c.visa_type from ".$this->table_name." a left join pms_country b on a.nationality=b.country_id left join pms_job_visa_type c on a.visa_type_id=c.visa_type_id where a.candidate_id=".$candidateId);
		return $query->row_array();
	}
...
//GET ALL CANDIDATES LANGUAGES
   ...
   function candidate_languages($candidate_id)
   {
     $query=$this->db->query("SELECT a.lang_id,a.lang_name FROM `pms_languages` a inner join pms_cand_lang b on a.lang_id=b.lang_id where b.candidate_id=".$candidate_id." order by a.lang_name");
		...
		foreach($dropdowns as $dropdown)
		{
			 $survey_result[$dropdown->lang_id] =  $dropdown->lang_name;
		}
...
//getting jobs applied by candidate
	...
	function get_job_details_rms($candidate_id,$job_app_id)
	{
		if($candidate_id=='')return;
