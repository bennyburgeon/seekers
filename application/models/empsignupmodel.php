<?php 
class Empsignupmodel extends CI_Model {
...
function country_list()
	{
		...
		$query = $this->db->query("select distinct a.country_id, a.country_name from pms_country a where country_id='162' order by country_name asc");
		$dropdowns = $query->result();
  ...
  function state_list_by_city($country_id='')
	...
	function insert_employer()
    {
		...
		'reg_date'			 => date('Y-m-d'),
			...
			$usr = $this->db->insert('pms_company_users', $data);	
			$ins_id=$this->db->insert_id($usr);
		//echo $ins_id; exit();
		$this->load->library('upload');
		if(isset($_FILES['aadhar_file'])){
				if (is_uploaded_file($_FILES['aadhar_file']['tmp_name'])) 
				{       				
					$config['upload_path'] = '../rms/uploads/files/';
					...
					if ($this->upload->do_upload('aadhar_file'))
					{
						...
						$data =  $this->upload->data();	
						$this->upload_file_name=$data['file_name'];					
						$this->db->query("update pms_company_users set aadhar_file='".$this->upload_file_name."' where user_id=".$ins_id);
					}
				}
			}
			
			if(isset($_FILES['pan_file'])){
				if (is_uploaded_file($_FILES['pan_file']['tmp_name'])) 
				{       				
					$config['upload_path'] = '../rms/uploads/files/';
					$config['allowed_types'] = 'jpg|jpeg|pdf|png';
					$config['max_size']	= '0';
					...
					if ($this->upload->do_upload('pan_file'))
					{
						...
						$data =  $this->upload->data();	
						$this->upload_file_name=$data['file_name'];					
						$this->db->query("update pms_company_users set pan_file='".$this->upload_file_name."' where user_id=".$ins_id);
					}
				}
			}
			
			/* if(isset($_FILES['gst_file'])){
				if (is_uploaded_file($_FILES['gst_file']['tmp_name'])) 
				{       				
					$config['upload_path'] = '../rms/uploads/files/';
					$config['allowed_types'] = 'jpg|jpeg|pdf|png';
					$config['max_size']	= '0';
					...
					if ($this->upload->do_upload('gst_file'))
					{
						...
						$data =  $this->upload->data();	
						$this->upload_file_name=$data['file_name'];					
						$this->db->query("update pms_company_users set gst_file='".$this->upload_file_name."' where user_id=".$ins_id);
					}
				}
			} */
    ...
    function state_list()
    {
			...
			$query=$this->db->query("select a.*,b.* from pms_state a inner join pms_state_description b ON a.state_id=b.state_id inner join pms_city c ON                                  a.state_id = c.state_id where a.country_id=162 order by b.state_name");
		/*else
