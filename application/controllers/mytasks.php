...
class Mytasks extends CI_controller{
	...
	function index($offset = 0)
	{
		...
		$config['base_url'] = $this->config->item('base_url')."index.php/mytasks/?date_range=".$this->data["date_range"]."&sort_by=$sort_by&limit=$limit&searchterm=".$this->data["searchterm"]."&$query_str&priority=".$this->data["priority"]."&status=".$this->data["status"];
		...
		$this->data["projects_list"]=$this->mytasksmodel->project_ddl();
	...
	function update()
	{
		...
		//$this->form_validation->set_rules('check_ups', 'Title', 'callback_check_dups');
			...
			else
			{
				...
				$this->data["project_team"]=$this->mytasksmodel->get_project_team_ddl($this->input->post("project_id"));
				$this->data["milestone_list"]=$this->mytasksmodel->get_milestone_ddl($this->input->post("project_id"));
	...
	{ 
		...
		if($this->input->post("project_id")){
			...
			echo json_encode($this->mytasksmodel->project_team_ddl($this->input->post("project_id")));
		}
		exit;
	}
	...
	{ 
		...
		if($this->input->post("project_id")){
			 header('Content-Type: application/x-json; charset=utf-8');
			echo json_encode($this->mytasksmodel->milestone_ddl($this->input->post("project_id")));
		}
		exit;
	}
