<?php 
class Company_feeds extends CI_Controller {
	...
	function __construct()
	{
		...
		$this->load->model('company_feedsmodel');
	...
	{	$this->load->library('pagination');
		...
		$this->data['total_rows']= $this->company_feedsmodel->record_count($searchterm,$ind_id,$flp_status,$company_priority,$company_rating,$status,$flp_next_date,$date_added,$user_id,$unassigned,$city_id,$building_location);
		...
		$this->data["records"] = $this->company_feedsmodel->get_list($start,$limit,$searchterm,$ind_id,$flp_status,$sort_by,$company_priority,$company_rating,$status,$flp_next_date,$date_added,$user_id,$unassigned,$city_id,$building_location);

		$this->data["industry_list"] = $this->company_feedsmodel->industry_list_search();
		...
		*/	
		...
		$this->data["admin_list"]=$this->company_feedsmodel->admin_list();
		$this->data["bde_list"]=$this->company_feedsmodel->bde_list();

		$this->data["building_loc_list"] = $this->company_feedsmodel->building_loc_list($state_id);
		
		$this->data["city_list"] = $this->citymodel->city_list($state_id);
		$this->data["state_list"] = $this->statemodel->state_list($country_id);	
		$this->data["country_list"] = $this->company_feedsmodel->country_list();
		...
		$this->load->view('company_feeds/companylist',$this->data);
		...
		{	
			...
			{         
				...
				if ($this->upload->do_upload('userfile'))
				{
					...
					{
						$data = $company[$i];
						$this->company_feedsmodel->insert_csv_records($data);
					}
					redirect('company_feeds');
				}
			}
		}
		$this->load->view('include/header',$this->data);
		$this->load->view('company_feeds/upload-csv',$this->data);
		$this->load->view('include/footer',$this->data);		
    }
	...
	function add()
	{
		...
		$this->data["type_list"] = $this->company_feedsmodel->type_list();
		$this->data["industry_list"] = $this->company_feedsmodel->industry_list();
		...
		if($this->input->post('company_name'))
		{
			...
			{
				$id=$this->company_feedsmodel->insert_record();
				redirect('company_feeds/?ins=1');
			}
		...
		$this->load->view('include/header',$this->data);
		$this->load->view('company_feeds/addcompany',$this->data);				
		$this->load->view('include/footer',$this->data);
...
// edxit and update pages here 
	...
	function edit($id=null)
	{
		...
		if(!empty($id))
		{
			...
			$this->data['formdata']=$this->company_feedsmodel->get_single_company($id);			
		    ...
		    $this->editor($path, $width);
			$this->load->view('include/header',$this->data);
			$this->load->view('company_feeds/editcompany',$this->data);				
			$this->load->view('include/footer',$this->data);
		}
	}
	...
	function update()
	{
		...
		if($this->input->post('company_name'))
		{
			...
			$this->form_validation->set_rules('company_name', 'Company Name', 'required');
				...
				if ($this->form_validation->run() == TRUE)
				{
					$id=$this->company_feedsmodel->update_record($this->input->post('company_id'));
					redirect('company_feeds/?upd=1');
				}else{
	...
	{
		redirect('company_feeds/?delete=stop');
			if(!empty($id))
			{
				
				if($this->is_related($id)){

					redirect('company_feeds/?del=2');
				}else{
	...
	function add_calls()
	{
		...
		{
			$job_id=$this->company_feedsmodel->add_calls();	
			
			if($this->input->post('create_task')!='')
			{
				$task_id=$this->company_feedsmodel->add_task();
			}
	...
	{
		$this->data["records"] = $this->company_feedsmodel->get_calls_history($this->input->post('company_id'));
		$output_str='
	...
	function add_jobs()
	{
		...
		if($this->input->post('company_id')!='')
		{
			$job_id=$this->company_feedsmodel->add_jobs();	
			$response = array(
			    'data' => '',
				'status'=>'success',
			);
	...
	function assign_requirement()
	{
		...
		$output_str='<table width="100%" border="1" cellpadding="3" cellspacing="3">
			  ...
			  $this->data["records"] = $this->company_feedsmodel->get_assignment_recruiter($this->input->post('company_id'));
	...
	function save_assignment()
	{
		...
		if($this->input->post('company_id')!='')
		{
				$company_name=$this->company_feedsmodel->get_company_name($this->input->post('company_id'));
				...
				{	
					...
					{
						 $return_val = $this->company_feedsmodel->add_recruiter_to_company($this->input->post('company_id'),$val);
						 if($comments[$val]!='')
						 {
							 $data = array(
							...
							$return_val = $this->company_feedsmodel->add_task_assignment($data);
						 }
					}
				}
	...
	public function getstate()
	{
		...
		{
			$data=array();
			$data["state_list"] = $this->company_feedsmodel->state_list_by_city($_POST['country_id']);
	...
	{ 
		...
		{
			$data=array();
			$data["city_list"] = $this->company_feedsmodel->city_list_by_state($_POST['state_id']);
	...
	function multiple_assignment()
	{
		...
		if(is_array($id_arr))
		{
			foreach ($id_arr as $key => $val)
			{
				...
				$this->company_feedsmodel->add_recruiter_to_company($val,$this->input->post('user_id'));
			}
		}
		redirect('company_feeds/?assigned=1');
	}		
}
?>
