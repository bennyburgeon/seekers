...
class Admin_leave extends CI_Controller {
	...
	function index()
	{
		...
		$rows='';
		$this->load->model('admin_leave_model');
		...
		$this->data['total_rows']= $this->admin_leave_model->record_count($searchterm);
		...
		$config['base_url'] = $this->config->item('base_url')."index.php/admin_leave/?sort_by=$sort_by&limit=$limit&searchterm=$searchterm$query_str";
		...
		// paging ends here
		$this->data["records"] = $this->admin_leave_model->get_list($start,$limit,$searchterm,$sort_by);
		...
		$config['base_url'] = base_url().'index.php/admin_leave/?';	
		
		$this->data['approved_by']=$this->admin_leave_model->get_all_admins();
		...
		$this->load->view('admin_leave/list',$this->data);				
		$this->load->view('include/footer');
	}	

	function changestat($id=null)
	{
		if($id=='')redirect('admin_leave');
		if($this->input->get('stat')=='')redirect('admin_leave');
		$this->db->query("update pms_branch set status=".$this->input->get('stat')." where branch_id=".$id);
		redirect('admin_leave?stat=1');
	}
	...
	{	
		...
		{ 
			...
			//~ {
				...
				$id=$this->admin_leave_model->insert_record();
				redirect('admin_leave/?ins=1');
			//}
				$this->data['formdata']=array(
				...
				$this->data['admins']=$this->admin_leave_model->get_all_admins();
				//~ print_r($this->data['admins']);die;
				$this->load->view('include/header');
				$this->load->view('admin_leave/add',$this->data);	
				$this->load->view('include/footer');
	}
	...
	function edit($id=null)
	{
		...
		if(!empty($id))
		{
			...
			$data['admins']=$this->admin_leave_model->get_all_admins();
	...
	function update()
	{
		$data['page_head']= 'Edit Leave';
			...
			if($this->input->post('name'))
			{
				...
				//~ {
					$this->load->model('admin_leave_model');
					$id=$this->admin_leave_model->update_record();
					redirect('admin_leave/?update=1');
				//~ }
	...
	function delete($id=null)
	{
		...
		if(!empty($id))
		{
			$id=$this->admin_leave_model->delete($id);
			redirect('admin_leave/?del='.$id);
		}
		elseif(is_array($this->input->post('delete_rec')))
		{ 
			 foreach ($this->input->post('delete_rec') as $key => $val)
				{
					$id=$this->admin_leave_model->delete($val);
				}
			redirect('admin_leave/?del='.$id);
		}
		else
	...
	function multidelete(){
		...
		if(count($id_arr)>0){
			$this->load->model('admin_leave_model');
			$this->admin_leave_model->delete_multiple_record($id_arr);
			redirect('admin_leave/?rows='.$rows.'&del=1');
		}
		else{
			redirect('admin_leave');
		}
