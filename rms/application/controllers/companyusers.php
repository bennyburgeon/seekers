...
class Companyusers extends CI_controller
 {
	function __construct()
	{
	   ...
	   if(!isset($_SESSION['admin_session']) || $_SESSION['admin_session']=='')redirect('logout');	
		...
		$this->data['module_name'] = $this->router->fetch_class();
	...
	function add()
	{
		...
		$this->data["company_list"] = $this->companyusersmodel->fill_company();
	...
	function edit($id='')
	{
		...
		$this->data["company_list"] = $this->companyusersmodel->company_list();
		$path = '../js/ckfinder';
		$width = '700px';
		$this->editor($path, $width);		
	...
	function delete($id='')
	{
		//if($_SESSION['admin_session']==$id) redirect('companyusers');
