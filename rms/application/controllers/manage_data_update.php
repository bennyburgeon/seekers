<?php 
class Manage_data_update extends CI_Controller {

	function __construct()
	{
		parent::__construct();
		//~ $this->load->model('coursemodel');
		$this->load->model('manage_data_update_model');
	...
	function index()
	{
		$update_cand_id=$this->input->get('id');
		//~ print_r($update_cand_id);
		
		$res=$this->db->query("select * from pms_candidate where candidate_id=$update_cand_id");
		$existing_data=$res->row();
		...
		$this->load->model('manage_data_update_model');
		// paging starts here
		$this->data['total_rows']= $this->manage_data_update_model->record_count($searchterm);
		$get = $this->input->get ( NULL, TRUE );
		$page = (int) $this->input->get ( 'rows', TRUE );
		$this->data['cur_page']=$this->router->class;
		$this->load->model('manage_data_update_model'); 
		$this->data['page_head']= 'Manage Data Edit';		
		$config['base_url'] = base_url().'index.php/manage_data_update/?';	
		...
		if(isset($existing_data) && !empty($existing_data))
		{
			...
			$this->data['formdata']=array(
									...
									$this->data['formdata']['id']=$update_cand_id;
		...
		$this->data["branch_list"] = $this->manage_data_update_model->branch_list();
		$this->data["level_list"] = $this->manage_data_update_model->fill_levels();
		...
		$this->data["country_list"] 	= $this->manage_data_update_model->country_list_by_state_city_location();
		$this->data["edu_level_list"] = $this->manage_data_update_model->edu_level_list();
		$this->data["edu_years_list"] = $this->manage_data_update_model->edu_years_list();
		
		$this->data["edu_course_list"] = array('' => 'Select Course');
		$this->data["edu_spec_list"] = $this->manage_data_update_model->edu_spec_list();
		$this->data["edu_univ_list"] = $this->manage_data_update_model->edu_univ_list();
		$this->data["edu_course_type_list"] = $this->manage_data_update_model->edu_course_type_list();
		...
		$this->data["visatype_list"] = $this->manage_data_update_model->visatype_list();
		$this->data["country_list"] = $this->manage_data_update_model->country_list();
		...
		$queslist=$ques->result();
				...
				foreach($queslist as $queslist)
				{
					...
					$mydata[$id] = $this->manage_data_update_model->getanswersbyid($id);
				}
		if(isset($mydata) && !empty($mydata)){$this->data['answers1']=$mydata;}
	
		
		$this->data['skillset']=$this->manage_data_update_model->get_skills();
		$this->data['parentskill']=$this->manage_data_update_model->get_parent_skills();
		
		$this->data['cerifications']=$this->manage_data_update_model->get_cert();
		...
		//employment
		$this->data["industry_list"] = $this->manage_data_update_model->industry_list();
		$this->data["functional_list"] = $this->manage_data_update_model->functional_list();
		$this->data["currecy_list"] = $this->manage_data_update_model->currency_list();
		$this->data["years_list"] = $this->manage_data_update_model->years_list();
		$this->data["months_list"] = $this->manage_data_update_model->months_list();
		...
		$this->data["religion_list"]    = $this->manage_data_update_model->religion_list();
		...
		$this->load->view('manage_data_update/list',$this->data);				
	...
	public function getcourses()
	{
		...
		{
			$data=array();
			$data["course_list"] = $this->manage_data_update_model->get_course_list($_POST['level_study'],$_POST['int_val']);
	...
	function addEducationDetail($candidateId)
	{
		...
		$id  = $this->manage_data_update_model->insert_education_detail($candidateId);
        if ($id > 0) { //success
	...
	public function getstate()
	{
		...
		{
			$data=array();
			$data["state_list"] = $this->manage_data_update_model->state_list_by_city($_POST['country_id']);
		...
		public function getcity()
	{
		...
		{
			$data=array();
			$data["city_list"] = $this->manage_data_update_model->city_list_by_state($_POST['state_id']);
	...
	public function getlocation()
	{
		...
		{
			$data=array();
			$data["location_list"] = $this->manage_data_update_model->location_list($_POST['city_id']);
	...
	function child_skill()
	{
		...
		//~ $id=1;
		$this->data['skillset']=$this->manage_data_update_model->get_child_skills($id);
	...
	public function addCandidate()
	{
		...
		// save to contacts
			...
			$id = $this->manage_data_update_model->insert_candidate_record($data);			
			//~ $id=22;
			if ($id != '')
			{
	...
	function addPassportDetail($candidateId){
		...
		$uid = $this->manage_data_update_model->update_passport_detail($candidateId);
	...
	{
	 $id  = $this->manage_data_update_model->insert_skill_details($candidateId);
	...
	{
	 $id  = $this->manage_data_update_model->insert_cert_details($candidateId);
	...
	{
		$id  = $this->manage_data_update_model->insert_job_detail($candidateId);
		$uid = $this->manage_data_update_model->update_job_detail($candidateId);
        if ($id > 0) { //success
            $status = array("STATUS" => "1");
            echo json_encode($status);
        } else { //failure
	...
	{
		$id  = $this->manage_data_update_model->insert_contact_detail($candidateId);
		$uid = $this->manage_data_update_model->update_contact_detail($candidateId);
        if ($id > 0) { //success
			// send emails
            $status = array("STATUS" => "1");
            echo json_encode($status);
		...
		redirect('manage_data_update',"refresh");	
	}
