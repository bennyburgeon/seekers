<?php 
class Cv_parser extends CI_Controller {
	function candidates()
	{
		parent::__construct();
	...
	{	
		...
		$this->data['rows']='';
		$this->load->model('cv_parsermodel');
		...
		$this->data['skill_list']=$this->cv_parsermodel->get_skill_list();
		$this->data['edu_level_list']=$this->cv_parsermodel->get_edu_level_list();
		$this->data['industry_list']=$this->cv_parsermodel->get_industry_list();
		$this->data['designation_list']=$this->cv_parsermodel->get_designation_list();

		$this->data['total_rows']= $this->cv_parsermodel->record_count($this->data['any_keywords'],$this->data['all_keywords'],$this->data['skills'],$this->data['designation'],$this->data['edu_level'],$this->data['industry']);
		...
		$this->data['redirect_after_upload']    =$this->config->site_url().'/cv_parser/rps_process/';
		...
		$config['base_url'] = $this->config->item('base_url')."index.php/cv_parser/?sort_by=".$this->data['sort_by']."&limit=".$this->data['limit']."&any_keywords=".$this->data['any_keywords']."&all_keywords=".$this->data['all_keywords']."&skills=".$this->data['skills']."&designation=".$this->data['designation']."&edu_level=".$this->data['edu_level']."&industry=".$this->data['industry'];
		...
		$this->data["records"] = $this->cv_parsermodel->get_list($this->data['start'],$this->data['limit'], $this->data['sort_by'],$this->data['any_keywords'],$this->data['all_keywords'],$this->data['skills'],$this->data['designation'],$this->data['edu_level'],$this->data['industry']);

		$this->load->model('cv_parsermodel'); 
		
		$this->data["admin_list"]=$this->cv_parsermodel->admin_list();
		...
		$this->load->view('cv_parser/cv_parserlist',$this->data);				
	...
	{	
		...
		$this->data["edu_course_list"] = $this->cv_parsermodel->edu_course_list();
		$this->data["level_list"] = $this->coursemodel->fill_levels();
		$this->data["branch_list"] = $this->cv_parsermodel->branch_list();
		$this->data['admin_users_lists']= $this->cv_parsermodel->get_admin_users_lists();
		
		$this->data['page_head']= 'Add Candidate';
		$this->load->view('include/header',$this->data);
		$this->load->view('cv_parser/addcandidates',$this->data);	
		$this->load->view('include/footer',$this->data);
	}
...
//AFTER ADD CANDIDATES :-EDITOR FORM
	function profile_entry($id=null)
	{
		...
		$this->data["formdata"] = $this->cv_parsermodel->get_single_record($id);
		$this->data["formdata"]['profile_list'] = $this->cv_parsermodel->profile_list($id);
		...
		$this->load->view('include/header',$this->data);
		$this->load->view('cv_parser/profile_entry',$this->data);	
		$this->load->view('include/footer',$this->data);
	}
	...
	function addCandidate(){
		...
		{ 
			$id = $this->cv_parsermodel->insert_candidate_record();
			if ($id != '') { 
				...
				if(isset($_FILES['cv_file'])){
					...
					{       				
						...
						if ($this->upload->do_upload('cv_file'))
						{
							...
							$this->cv_parsermodel->insert_files($dataArr);
									$cv_file=1;
						}
					}
				}
	...
	function add_follow_up()
	{
		...
		{
			$job_id=$this->cv_parsermodel->add_follow_up();	
			
			if($this->input->post('create_task')!='')
			{
				$task_id=$this->cv_parsermodel->add_task();
			}
...
//Certification and Technical Skilss
		...
		$candidate_certifications = $this->cv_parsermodel->candidate_certifications($id);
...
//skills
		...
		$this->data['skill_list']=$this->cv_parsermodel->get_parent_skills();
		
		$candidate_skills = $this->cv_parsermodel->candidate_skills($id);
		...
		$this->data['child_skills']	=	$this->cv_parsermodel->get_child_skills($res1[0]['skill_id']);
		}
		
//all child skills		
		$this->data['all_child_skills']	=	$this->cv_parsermodel->child_skills();
//domain knowledge
		
		$this->data['domain']=$this->cv_parsermodel->get_domain();
		$candidate_domain = $this->cv_parsermodel->get_domain_details($id);
...
//Planning Job Change Details
		$this->data["formdata8"] = $this->cv_parsermodel->job_search($id);
...
//Language Deatils
		$this->data['lang_list']=$this->cv_parsermodel->get_language_set();
		$candidate_certifications =$this->cv_parsermodel->candidate_languages($id);
...
//Contact Details
		...
		$this->data["formdata3"] = $this->cv_parsermodel->get_address_single_record($id);
...
//Passport Details
		...
		$this->data["country_list"] = $this->cv_parsermodel->country_list();
		$this->data["formdata4"] = $this->cv_parsermodel->get_passport_single_record($id);

//Education Details
		$this->data["country_list"] 	= $this->countrymodel->country_list_by_state_city_location();
		$this->data["edu_level_list"] = $this->cv_parsermodel->edu_level_list();
		$this->data["edu_years_list"] = $this->cv_parsermodel->edu_years_list();
		//$this->data["edu_course_list"] = $this->cv_parsermodel->edu_course_list();
		$this->data["edu_course_list"]  = array('' => 'Select Course');
		
		$this->data["edu_spec_list"] = $this->cv_parsermodel->edu_spec_list();
		$this->data["edu_univ_list"] = $this->cv_parsermodel->edu_univ_list();
		$this->data["edu_coll_list"] = $this->cv_parsermodel->edu_coll_list();
		$this->data["edu_course_type_list"] = $this->cv_parsermodel->edu_course_type_list();
		$this->data["formdata5"] = $this->cv_parsermodel->get_education_single_record($id);
...
//Job Details
		$this->data["industries_list"] = $this->cv_parsermodel->industries_list();		
		$this->data["industry_list"] = $this->cv_parsermodel->industry_list();
		$this->data["functional_list"] = $this->cv_parsermodel->functional_list();
		$this->data["currecy_list"] = $this->cv_parsermodel->currency_list();
		$this->data["years_list"] = $this->cv_parsermodel->years_list();
		$this->data["months_list"] = $this->cv_parsermodel->months_list();
		$this->data["formdata6"] = $this->cv_parsermodel->get_job_single_record($id);
...
//FILE DEATILS
		$this->data['survey_result']=$this->cv_parsermodel->get_survey_result($id);
		$this->data["formdata7"] = $this->cv_parsermodel->get_file_single_record($id);
		...
		$this->load->view('include/header',$this->data);
		$this->load->view('cv_parser/editcandidates',$this->data);	
		$this->load->view('include/footer',$this->data);
	}
...
//SHOW CV AND PHOTOT AJAX
	function show_cv_photo()
	{
		...
		$formdata = $this->cv_parsermodel->get_single_record($id);
...
//JOB DETAILS
	function jobDetails($id)
	{
		...
		$industry_list = $this->cv_parsermodel->industry_list();
		$functional_list = $this->cv_parsermodel->functional_list();

		
		$results = $this->cv_parsermodel->get_job_details($id);
		$form_data	=	array();
		...
		$job_details_view	=	$this->load->view('cv_parser/job_details',$this->data,true);
...
// import csv
    ...
    {    
        ...
        {
			$this->load->view('include/header',$this->data);    
			$this->load->view('cv_parser/import_csv',$this->data);                
			$this->load->view('include/footer',$this->data);        
        }
        else
        {
	...
	function candidate_view($candidate_id)
	{
		...
		$this->data['detail_list'] = $this->cv_parsermodel->detail_list($candidate_id);
		...
		if($this->input->post('candidate_id')!='')
			{
			   ...
			   redirect('cv_parser/candidate_view/'.$this->input->post('candidate_id'));
		}
						
		$this->data['list']=$this->cv_parsermodel->follow_record($candidate_id);
		$this->data['note_list']=$this->cv_parsermodel->notes_record($candidate_id);		
		$this->data['coe_list']=$this->cv_parsermodel->coe_list($candidate_id);
		$this->data['visa_approval_list']=$this->cv_parsermodel->visa_approval_list($candidate_id);

		$this->data['interview_list']=$this->cv_parsermodel->interview_record($candidate_id);
		$this->data['aplication_list']=$this->cv_parsermodel->aplication_record($candidate_id);
		
		$this->data['interview_status_list']=$this->cv_parsermodel->interview_status_list();		
		$this->data['app_list']=$this->cv_parsermodel->aplication_list($candidate_id);
		$this->data['app_list_coe']=$this->cv_parsermodel->select_aplication_coe($candidate_id);
		$this->data['admin_user_list']=$this->cv_parsermodel->admin_user_list();
		$this->data['interview_type_list']=$this->cv_parsermodel->interview_type_list();
		$this->data['university_list']=$this->cv_parsermodel->university_list();
		$this->data['campus_list']=array('' => 'Select Campus');
		$this->data['intake_list']=$this->cv_parsermodel->intake_list();
		$this->data['course_list']=array('' => 'Select Course');;
		$this->data['level_list']=$this->coursemodel->fill_levels();
		$this->data['status_list']=$this->cv_parsermodel->status_list();

		$path = '../js/ckfinder';
		$width = '100%';
		$this->editor($path, $width);
		$this->load->view("include/header",$this->data);
		$this->load->view("cv_parser/candidate_view",$this->data);
		$this->load->view("include/footer",$this->data);
	}
	...
	function summary($candidate_id)
	{
		...
		$this->load->model('cv_parsermodel');
		$this->load->model('countrymodel');
		$this->data["formdata"] = $this->cv_parsermodel->get_single_record($candidate_id);
		$this->data['detail_list'] = $this->cv_parsermodel->detail_list($candidate_id);
		$this->data['candidate_languages'] = $this->cv_parsermodel->candidate_languages($candidate_id);
		$this->data['education_details'] = $this->cv_parsermodel->education_deatils($candidate_id);
		$this->data['job_history'] = $this->cv_parsermodel->job_list($candidate_id);
		$this->data['all_counselor'] = $this->cv_parsermodel->all_counselor($candidate_id);
		$this->data['candidate_counselor'] = $this->cv_parsermodel->candidate_counselor($candidate_id);
		$this->data['candidate_skill'] = $this->cv_parsermodel->candidate_skills($candidate_id);
		
//candidate doamin knowledge
		$this->data['candidate_domain'] = $this->cv_parsermodel->candidate_domains($candidate_id);
		
		$this->data['candidate_certifications'] = $this->cv_parsermodel->candidate_certifications($candidate_id);
	
		$this->data['candidate_programs_summary'] = $this->cv_parsermodel->candidate_programs_summary($candidate_id);

	
		$this->data['candidate_coe_summary'] = $this->cv_parsermodel->candidate_coe_summary($candidate_id);
		$this->data['candidate_visa_summary'] = $this->cv_parsermodel->candidate_visa_summary($candidate_id);

		$this->data['candidate_questionnaire_summary'] = $this->cv_parsermodel->get_survey_result($candidate_id);
		$this->data['candidate_files_summary'] = $this->cv_parsermodel->get_files($candidate_id);
		$this->data['candidate_complaints_summary'] = $this->cv_parsermodel->ticket_list($candidate_id);

//Job Search details(candidate_job_serach)
		$this->data['job_search'] = $this->cv_parsermodel->job_search($candidate_id);
		//print_r($this->data['job_search']);exit;
//Edit skill Modal

		$this->data['skill_list']=$this->cv_parsermodel->get_parent_skills();
		
		$candidate_skills = $this->cv_parsermodel->candidate_skills($candidate_id);
...
//Edit Language Modal
		//Language Deatils
		$this->data['lang_list']=$this->cv_parsermodel->get_language_set();
		$candidate_certifications =$this->cv_parsermodel->candidate_languages($candidate_id);
...
//applied jobs of candidate		
		$this->data['applied_jobs']=$this->cv_parsermodel->get_job_list($candidate_id);

//shortlisted jobs
		$this->data['shortlisted']=$this->cv_parsermodel->get_shortlisted($candidate_id);

//interview scheduled jobs
		$this->data['interview_list']=$this->cv_parsermodel->get_interview_list($candidate_id);

//selected jobs
		$this->data['jobs_selected']=$this->cv_parsermodel->jobs_selected($candidate_id);

//offer letters issued
		$this->data['offer_letters_issued']=$this->cv_parsermodel->offer_letters_issued($candidate_id);
		
//offer accepted
		$this->data['offer_accepted']=$this->cv_parsermodel->offer_accepted($candidate_id);

//invoice genereted
		$this->data['invoice_generated']=$this->cv_parsermodel->invoice_generated($candidate_id);

//present contract details
		$this->data['contract_details']=$this->cv_parsermodel->get_contract_detail($candidate_id);
		
//Profile completion status
		$this->data['profile_completion']=$this->cv_parsermodel->get_profile_status($candidate_id);

//Profile Assessment status
		$this->data['profile_assessment']=$this->cv_parsermodel->get_profile_assessment($candidate_id);
		
//All language details
		$this->data['lang_details']=$this->cv_parsermodel->get_lang_details($candidate_id);
			...
			if($this->input->post('candidate_id')!=''){
				...
				redirect('cv_parser/summary/'.$candidate_id);
			}
		$this->load->view("include/header",$this->data);
		$this->load->view("cv_parser/candidate_summary",$this->data);
		$this->load->view("include/footer",$this->data);
	}
	...
	function candidate_programs($candidate_id)
	{
		...
		// update suggestion logic from here
			$this->cv_parsermodel->update_suggestion_module($this->input->post('candidate_id'), $this->input->post('campus_id'), $this->input->post('course_id'), $this->input->post('total_semester'), $this->input->post('fee_per_semester'), $this->input->post('annual_tution_fee'), $this->input->post('total_tution_fee'),$this->input->post('candidate_course_id'));
		// end here	
					
			redirect('cv_parser/candidate_programs/'.$this->input->post('candidate_id').'/?upd=1');
		}
...
if($this->input->get('education')!='')
		...
		if($this->input->get('course_id')!='')
		{
			...
			$this->data['single_application']['app_details']=$this->cv_parsermodel->create_program_name($this->data['single_application']['course_id']);
		}					
		...
		$this->data['aplication_list']=$this->cv_parsermodel->aplication_record($candidate_id); // application lists - 	
		$this->data['detail_list'] = $this->cv_parsermodel->detail_list($candidate_id); // candidate details
		$this->data['university_list']=$this->cv_parsermodel->university_list(); // university list		
		$this->data['intake_list']=$this->cv_parsermodel->intake_list(); // intake list 		
		$this->data['level_list']=$this->coursemodel->fill_levels(); // education levels
		$this->data['status_list']=$this->cv_parsermodel->status_list(); // application process status list
		$this->data['campus_list']=array('' => 'Select Campus');
		$this->data['course_list']=array('' => 'Select Course');

		if($this->data['single_application']['univ_id']>0)
			$this->data['campus_list']=$this->cv_parsermodel->campus_list($this->data['single_application']['univ_id']);
		if($this->data['single_application']['level_study']>0)
			$this->data['course_list']=$this->cv_parsermodel->course_list_level($this->data['single_application']['level_study']);
			
		$this->data['candidate_qualification_list']=$this->cv_parsermodel->candidate_qualification_list($candidate_id); 
		// candidate's qualification for higher studies
		// edit mode		
		if($this->input->get('app_id')!='')
		{
			$this->data['single_application']=$this->cv_parsermodel->select_aplication_record($this->input->get('app_id'));
			if(count($this->data['single_application'])>0)
			{
				if($this->data['single_application']['univ_id']>0)$this->data['campus_list']=$this->cv_parsermodel->campus_list($this->data['single_application']['univ_id']);
				if($this->data['single_application']['level_study']>0)$this->data['course_list']=$this->cv_parsermodel->course_list_level($this->data['single_application']['level_study']);
		...
		// update program		
		$this->load->view("include/header",$this->data);
		$this->load->view("cv_parser/candidate_programs",$this->data);
		$this->load->view("include/footer",$this->data);
	}
	...
	function aplication()
	{
		...
		$this->data['aplication_list']=$this->cv_parsermodel->select_aplication_record($id);		
		...
		$dataArr = $this->load->view('cv_parser/candidate_aplication_list', $this->data,TRUE);	
		echo $dataArr;
		exit();		
	}
	...
	function candidate_coe($candidate_id)
	{
		...
		$this->editor($path, $width);
		$this->load->view("include/header",$this->data);
		$this->load->view("cv_parser/candidate_coe",$this->data);
		$this->load->view("include/footer",$this->data);
	}
...
// Manage Email & SMS
	function email_sms($candidate_id)
	{
		...
		$this->load->model('cv_parsermodel');
		$this->data['detail_list'] = $this->cv_parsermodel->detail_list($candidate_id);
		$this->data['email_sms_list']=$this->cv_parsermodel->email_sms_list($candidate_id);
		...
		if($this->input->post('send_type')!='')
		{
			...
			redirect('cv_parser/email_sms/'.$candidate_id);
		}
		$this->data['error']="Fill subject and email content to send to the candidate.";
		$this->load->view("include/header",$this->data);
		$this->load->view("cv_parser/candidate_email_sms",$this->data);
		$this->load->view("include/footer",$this->data);
	}
// Schedule an interview
	function interview(){
		...
		$id=$this->db->insert_id();
		$this->load->model('cv_parsermodel');
		$this->data['interview_list']=$this->cv_parsermodel->select_interview_record($id);
		$dataArr = $this->load->view('cv_parser/candidateinterview_list', $this->data,TRUE);
	...
	function tickets($candidate_id)
	{
	   ...
	   $this->data['error']='';
		...
		$this->data['detail_list'] = $this->cv_parsermodel->detail_list($candidate_id);
			...
			$this->data['ticket_list']=$this->cv_parsermodel->ticket_list($candidate_id);
			...
			if($this->input->post('send_type')!='')
			{
				...
				$id=$this->db->insert_id();
				redirect('cv_parser/tickets/'.$candidate_id);
			}
			...
			$this->load->view("include/header",$this->data);
			$this->load->view("cv_parser/candidate_complaints",$this->data);
			$this->load->view("include/footer",$this->data);
	}
	...
	function invoice($candidate_id)
	{
		...
		foreach($this->input->post('admin_id') as $key => $val)
		{
			...
			$this->load->view("include/header",$this->data);
			$this->load->view("cv_parser/candidate_invoice",$this->data);
			$this->load->view("include/footer",$this->data);
		}	
	}
	...
	function cvfile($candidate_id)
	{
	   ...
	   $this->data['error']='';
		...
		$this->data['cv_fileist']=$this->cv_parsermodel->cvfile_list($candidate_id);
		...
		if($this->input->post('cvfile')!=''){
			...
			redirect('cv_parser/cvfile/'.$candidate_id);
			$path = '../js/ckfinder';
			$width = '100%';
			$this->editor($path, $width);
			$this->load->view("include/header",$this->data);
			$this->load->view("cv_parser/candidate_cvfile",$this->data);
			$this->load->view("include/footer",$this->data);
		}else{
			$path = '../js/ckfinder';
			$width = '100%';
...
function certifications($candidate_id)
	{
		...
		if($this->input->post('candidate_id'))
		{
			...
			}else
			{
			   ...
			   redirect('cv_parser/certifications/'.$this->input->post('candidate_id').'?upd=1');
		}

		$this->data['certifications_list']=$this->cv_parsermodel->get_certifications_set();
		$this->data['certifications_list_current']=$this->cv_parsermodel->get_certifications_set_candidate($candidate_id);
		$this->data['detail_list'] = $this->cv_parsermodel->detail_list($candidate_id);
		$this->data['cv_fileist']=$this->cv_parsermodel->job_list($candidate_id);
		...
		$this->load->view("include/header",$this->data);
		$this->load->view("cv_parser/candidate_certifications",$this->data);
		$this->load->view("include/footer",$this->data);
	}
	...
	function followup()
	{
		...
		if(isset($_POST['candidate_id']))
		{
			...
			$this->data['single_list']=$this->cv_parsermodel->select_record($id);
		
			$dataArr = $this->load->view('cv_parser/candidatefollowup_list', $this->data,TRUE);
			echo $dataArr;
			exit();
	...
	{
		$this->load->model('cv_parsermodel');
		$this->data['skillset']=$this->cv_parsermodel->get_all_skills();
	...
	$id=$this->db->insert_id();
	$this->load->model('cv_parsermodel');
	$this->data['note_list']=$this->cv_parsermodel->select_notes_record($id);
	$dataArr = $this->load->view('cv_parser/candidatenotes_list', $this->data,TRUE);
	echo $dataArr;
	...
	function drop(){
		...
		$this->load->model('cv_parsermodel');
		 $this->cv_parsermodel->drop_record($candidate_follow_id);
		$dataArr = $this->load->view('cv_parser/candidate_view');
		echo $dataArr;
	}
	...
	function cvfile_drop(){
		...
		$this->load->model('cv_parsermodel');
		$this->cv_parsermodel->cvfile_drop_record($cvfile_id);		          
	}
		
	function drop_edu_item(){
		$eucation_id=$this->input->post('eucation_id');
		$this->load->model('cv_parsermodel');
		$this->cv_parsermodel->drop_edu_item($eucation_id);
	}	
	...
	function drop_notes(){
		...
		$this->load->model('cv_parsermodel');
		$this->cv_parsermodel->note_drop_record($candidate_note_id);
		$dataArr = $this->load->view('cv_parser/candidate_view');
		echo $dataArr;
	 ...
	 function drop_interviews(){
		...
		$this->load->model('cv_parsermodel');
		$this->cv_parsermodel->interview_drop_record($interview_id);
		$dataArr = $this->load->view('cv_parser/candidate_view');
		echo $dataArr;
	 ...
	 function drop_aplication(){
		 $app_id=$_POST['app_id'];
		$this->load->model('cv_parsermodel');
		$this->cv_parsermodel->aplication_drop_record($app_id);
		$dataArr = $this->load->view('cv_parser/candidate_view');
		echo $dataArr;		          
	}
...
/*			echo '<pre>';
		...
		$this->data['file_list']=$this->cv_parsermodel->file_list($candidate_id);
		$this->load->view("include/header",$this->data);
		$this->load->view("cv_parser/csv_data_import",$this->data);
		$this->load->view("include/footer",$this->data);
	}
	...
	function savefile()
	{
	   ...
	   if($this->input->post('title')!='')
	   {
		 if(isset($_FILES['photo']))
		 {
					...
					{
						$this->load->model('cv_parsermodel');
						$id=$this->cv_parsermodel->insert_file($candidate_id);
						$this->data['upload_list']=$this->cv_parsermodel->get_one_record($id);
						$replay=$this->load->view("cv_parser/upload_file",$this->data,TRUE);
						echo $replay;
					}
					else
					{
						redirect("cv_parser/candidate_file");
					}
				}
			else{
				 echo "Choose file";
	...
	function img_update(){
	 ...
	 if(isset($_FILES['photo'])){
						  $this->load->model('cv_parsermodel');
						   $this->cv_parsermodel->update_file($candidate_id);
	
							 $this->data['single_file']=$this->cv_parsermodel->get_one_file($candidate_id);
		...
		function deletefile1()
	{
		...
		if(!empty($id))
		{
			          ...
			          $this->load->model('cv_parsermodel');
					   $this->cv_parsermodel->delete_file($id);
					    $this->data['delete_file']=$this->cv_parsermodel->delete_one_file($id);
						
                           echo $this->data['delete_file']['photo'];  //$replay=$this->load->view("cv_parser/delete_file",$this->data,TRUE);
					         //echo $replay;
	...
	function delete_cv($id)
	{
		if(!empty($id))
		{
			...
			redirect("cv_parser/summary/".$id."?del_cv=1");
		}else
		{
			redirect("cv_parser/summary/".$id);
		}
	}
	...
	function candidate_delete($id)
	{
		...
		if(!empty($id))
		{
			$this->cv_parsermodel->candidate_delete($id);
			redirect('cv_parser/?del=1');
		}
	}
...
//onchange getcourse
	...
	public function assignAdmin()
	{
		...
		for($i=0;$i<count($candidatesArr);$i++){
			$id = $this->cv_parsermodel->assign_admin_user($candidatesArr[$i],$adminId);
		}
		/***************************** send mail to admin user ***************************/
		$data["adminEmail"] = $this->cv_parsermodel->getAdminEmail($adminId);
		$subject = 'Assignment';
...
// email ends here	
	...
	function delete_photo($id)
	{
		if(!empty($id))
		{
			...
			redirect("cv_parser/summary/".$id."?del_photo=1");
		}else
		{
			redirect("cv_parser/summary/".$id);
		}
...
//APPLY JOB
	function apply_job($candidateId,$jobId)
	{
		...
		{
		$this->load->model('cv_parsermodel');
        $this->cv_parsermodel->apply_job($candidateId,$jobId);
		}
		redirect("cv_parser/summary/".$candidateId);
	}
	
	function editSkillCertificateDetail($candidateId)
	{ 
		$this->load->model('cv_parsermodel');
	 	$id  = $this->cv_parsermodel->insert_skill_details($candidateId);
		
		$id  = $this->cv_parsermodel->insert_cert_details($candidateId);
		
		$id  = $this->cv_parsermodel->insert_domain_details($candidateId);
...
// step 1
	function update_profile_personal($candidateId){
		$this->load->model('cv_parsermodel');
        $this->cv_parsermodel->update_profile_personal($candidateId);
        $status = array("STATUS" => "1", "SUCCESS_ID" => $candidateId);
        echo json_encode($status);
	...
	function update_profile_address($candidateId){
		$this->load->model('cv_parsermodel');
        $this->cv_parsermodel->update_profile_address($candidateId);
        $status = array("STATUS" => "1", "SUCCESS_ID" => $candidateId);
        echo json_encode($status);
	...
	function update_profile_education($candidateId){
		$this->load->model('cv_parsermodel');
        $this->cv_parsermodel->update_profile_education($candidateId);
        $status = array("STATUS" => "1", "SUCCESS_ID" => $candidateId);
        echo json_encode($status);
	...
	function update_profile_profession($candidateId){
		$this->load->model('cv_parsermodel');
        $this->cv_parsermodel->update_profile_profession($candidateId);
        $status = array("STATUS" => "1", "SUCCESS_ID" => $candidateId);
        echo json_encode($status);
	...
	function update_profile_language($candidateId){
		$this->load->model('cv_parsermodel');
        $this->cv_parsermodel->update_profile_language($candidateId);
        $status = array("STATUS" => "1", "SUCCESS_ID" => $candidateId);
        echo json_encode($status);
	...
	function update_profile_tech_skills($candidateId){
		$this->load->model('cv_parsermodel');
        $this->cv_parsermodel->update_profile_tech_skills($candidateId);
        $status = array("STATUS" => "1", "SUCCESS_ID" => $candidateId);
        echo json_encode($status);
	}
	...
	function update_profile_certification($candidateId){
		$this->load->model('cv_parsermodel');
        $this->cv_parsermodel->update_profile_certification($candidateId);
        $status = array("STATUS" => "1", "SUCCESS_ID" => $candidateId);
        echo json_encode($status);
	...
	function update_profile_projects($candidateId){
		$this->load->model('cv_parsermodel');
        $this->cv_parsermodel->update_profile_projects($candidateId);
        $status = array("STATUS" => "1", "SUCCESS_ID" => $candidateId);
        echo json_encode($status);
	}
	...
	function update_profile_sports($candidateId){
		$this->load->model('cv_parsermodel');
        $this->cv_parsermodel->update_profile_sports($candidateId);
        $status = array("STATUS" => "1", "SUCCESS_ID" => $candidateId);
        echo json_encode($status);
	...
	function update_profile_social($candidateId){
		$this->load->model('cv_parsermodel');
        $this->cv_parsermodel->update_profile_social($candidateId);
        $status = array("STATUS" => "1", "SUCCESS_ID" => $candidateId);
        echo json_encode($status);
...
//EDIT PRIMARY AND SECONDARY SKILLS
	...
	{
		$this->load->model('cv_parsermodel');
	 	$id  = $this->cv_parsermodel->insert_skills($candidateId);
	...
	function add_contract_details($candidateId){
		$this->load->model('cv_parsermodel');
		$uid = $this->cv_parsermodel->update_contract_detail($candidateId);
		if($uid){
			$status = array("STATUS" => "1");
			echo json_encode($status);
		}
	...
	function change_contract_details($candidateId){
		...
		$uid = $this->cv_parsermodel->update_contract_detail($candidateId);
		if($uid){
			redirect('cv_parser/summary/'.$candidateId.'?upd=1');
		}
	}
	...
	{
		$this->load->model('cv_parsermodel');
	 	$id  = $this->cv_parsermodel->update_profile_status($candidateId);
        if ($id > 0) { //success
            $status = array("STATUS" => "1");
            echo json_encode($status);
        } else { //failure
	...
	{
		$this->load->model('cv_parsermodel');
	 	$id  = $this->cv_parsermodel->update_profile_assessment($candidateId);
        if ($id > 0) { //success
            $status = array("STATUS" => "1");
            echo json_encode($status);
        } else { //failure
	...
	{ 
		...
		$profile  = $this->cv_parsermodel->get_profile_status($candidateId);
		if(count(array_filter($profile))== 12)
		{
			$this->cv_parsermodel->process_completed($candidateId);
			redirect('cv_parser/?upd=1');
		}
		else
		{
			redirect('cv_parser/profile_approval/'.$candidateId.'?upd=2');}
	}
