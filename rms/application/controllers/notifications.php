...
class Notifications extends CI_Controller {
	...
	{	
		...
		$this->data["admin_group_list"] = $this->notificationsmodel->admin_group_list();
		...
		if($this->input->post('not_title'))
		{
			...
			{
				$not_id=$this->notificationsmodel->insert_record();
				
				if(is_array($this->input->post('admin_id')))
				{
					foreach($this->input->post('admin_id') as $key => $val)
					{
						$not_id=$this->notificationsmodel->insert_notification_users($not_id,$val);
				...
				if($this->input->post('user_grp_id')>0)
				{
					...
					$this->data["admin_users_group"] = $this->notificationsmodel->admin_users_group($this->input->post('user_grp_id'));

					foreach($this->data["admin_users_group"] as $key => $val)
					{
						$id=$this->notificationsmodel->insert_notification_users($key,$not_id);
					}					
		...
		$this->data['page_head']= 'Add Announcements';
		$this->data['module_action'] = 'Add Announcements';
