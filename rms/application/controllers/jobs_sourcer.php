<?php 
class Jobs_sourcer extends CI_Controller {
	...
	{	
		...
		$this->load->model('jobs_sourcermodel');
		...
		$this->data['total_rows']= $this->jobs_sourcermodel->record_count($searchterm,$job_status,$company_id,$job_priority);
		...
		$config['base_url'] = $this->config->item('base_url')."index.php/jobs_sourcer/?sort_by=$sort_by&searchterm=$searchterm$query_str&job_status=$job_status&company_id=$company_id&job_priority=$job_priority";
		...
		$this->data["records"] = $this->jobs_sourcermodel->get_list($start,$limit,$searchterm,$sort_by,$job_status,$company_id,$job_priority);
		...
		$this->data["company"] = $this->jobs_sourcermodel->company_has_jobs($job_status);
		$this->data["admin_list"]=$this->jobs_sourcermodel->admin_list();
		
		$this->data["priority_list"] = array('' => 'Priority', '1' => 'High','2' => 'Medium','3' => 'Low');

		$this->load->view('include/header');
		$this->load->view('jobs_sourcer/listjob',$this->data);				
	...
	{	
		...
		$this->data["company"]              = $this->jobs_sourcermodel->fill_company();
		$this->data["jobindustry"]          = $this->jobs_sourcermodel->fill_jobindustry();
		
		$this->data["functional"]           = $this->jobs_sourcermodel->fill_functional();
		$this->data["education"]            = $this->jobs_sourcermodel->fill_education();
		$this->data["salary"]               = $this->jobs_sourcermodel->fill_salary();
		$this->data["experience_list"]      = $this->jobs_sourcermodel->fill_experience();
		$this->data["worklevel"]            = $this->jobs_sourcermodel->fill_worklevel();
		$this->data["nationality"]          = $this->jobs_sourcermodel->country_list();

		$this->data["country_list"]         = $this->jobs_sourcermodel->country_list();
		$this->data["state_list"]           = array('' => 'Select State');		
		$this->data["city_list"]            = array('' => 'Select City');	
				
		//$this->data["job_location"]         = $this->jobs_sourcermodel->job_location();
		
		$this->data["jobtype"]              = $this->jobs_sourcermodel->jobtype_list();
		
		$this->data['skillset']             =$this->jobs_sourcermodel->get_parent_skills();
		$this->data['cerifications']        =$this->jobs_sourcermodel->get_cert();
		$this->data['domain']               =$this->jobs_sourcermodel->get_domain();
		...
		if($this->input->post('job_title')!='' && $this->input->post('company_id') != '0' )
		{
			...
			if ($this->form_validation->run() == TRUE)
			{
				...
				$id=$this->jobs_sourcermodel->insert_record();
				$job_cert=$this->jobs_sourcermodel->insert_cert_details($id);
				$job_skill=$this->jobs_sourcermodel->insert_skill_details($id,$this->input->post('parent'));
				$job_domain=$this->jobs_sourcermodel->insert_domain_details($id);
				redirect('jobs_sourcer/?ins=1&id='.$id);
			}
			...
			$this->load->view('include/header');
			$this->load->view('jobs_sourcer/addjob',$this->data);	
			$this->load->view('include/footer');
	}
	...
	public function getstate()
	{
		...
		{
			$data=array();
			$data["state_list"] = $this->jobs_sourcermodel->state_list_by_city($_POST['country_id']);
	...
	{ 
		...
		{
			$data=array();
			$data["city_list"] = $this->jobs_sourcermodel->city_list_by_state($_POST['state_id']);
	...
	function edit($id=null)
	{
		...
		$this->data["jobtype"] = $this->jobs_sourcermodel->jobtype_list();
		$this->data['skill_list']=$this->jobs_sourcermodel->get_parent_skills();
		$this->data['cerifications']=$this->jobs_sourcermodel->get_cert();
		$this->data['domain']=$this->jobs_sourcermodel->get_domain();
		...
		{
		 $this->data['childskill']=$this->jobs_sourcermodel->get_child_skills($data['formdata']['job_skills']);
		}
						
		
		if($this->data['formdata']['job_cat_id'] !=0)
		{
			$this->data["functional"] = $this->jobs_sourcermodel->function_list_by_category($data['formdata']['job_cat_id']);
		}
		if($this->data['formdata']['job_skills'] !=0)
		{
		 $this->data['childskill']=$this->jobs_sourcermodel->get_child_skills($data['formdata']['job_skills']);
		...
		*/
		...
		$job_certifications = $this->jobs_sourcermodel->get_certification_details($id);		
		$cerifications=array();
		...
		$job_skill = $this->jobs_sourcermodel->get_skill_details($id);		
		$skills = array();
		...
		$job_domain = $this->jobs_sourcermodel->get_domain_details($id);		
		$domain=array();
		...
		$path = '../js/ckfinder';
			...
			$this->load->view('jobs_sourcer/editjob',$this->data);	
			$this->load->view('include/footer',$this->data);
	}	

	function copy_job($id=null)
	{
		if($id=='')redirect('jobs_sourcer/');
		$this->data['page_head']= 'Copy Jobs';
		$this->data['upload_root']=$this->config->item('base_url');
		
		$this->load->model('jobs_sourcermodel');
		$id=$this->jobs_sourcermodel->copy_job($id);		
		redirect('jobs_sourcer/edit/'.$id);
	}	
		...
		if(!empty($id))
		{
			if($this->input->post('job_title') && $this->input->post('comapny_id') == 0)
			{
				...
				//$this->form_validation->set_rules('job_title_dup', 'job name', 'callback_check_dups');
					...
					{
						$this->load->model('jobs_sourcermodel');
						$id=$this->jobs_sourcermodel->update_record($id);
						$job_cert=$this->jobs_sourcermodel->insert_cert_details($this->input->post('job_id'));
						$job_skill=$this->jobs_sourcermodel->insert_skill_details($this->input->post('job_id'),$this->input->post('parent'));
						$job_domain=$this->jobs_sourcermodel->insert_domain_details($this->input->post('job_id'),$this->input->post('domain_id'));
						redirect('jobs_sourcer/?update=1');
					}else
					{
						$data['formdata']=array(
						'job_priority'        => $this->input->post('job_priority') ,
	...
	function assign_requirement()
	{
		$this->load->model('jobs_sourcermodel');
		 ...
		 $output_str='<table width="100%" border="0" cellpadding="3" cellspacing="3">
			  ...
			  $this->data["records"] = $this->jobs_sourcermodel->get_assignment_recruiter($this->input->post('job_id'));
			...
			/* $output_str.='<tr><td colspan="3">&nbsp;</td></tr><tr><td colspan="3">Vendors List</td></tr><tr><td colspan="3">&nbsp;</td></tr>';
			  ...
			  $this->data["records"] = $this->jobs_sourcermodel->get_assignment_vendor($this->input->post('job_id'));
	...
	function save_assignment()
	{
		...
		$job_details= $this->jobs_sourcermodel->get_job($this->input->post('job_id'));
		...
		if($this->input->post('job_id')!='')
		{
				if($this->input->post('admin_id')!='')
				{
					...
					{
						 $val = $this->jobs_sourcermodel->add_recruiter_to_job($this->input->post('job_id'),$val);
					}
				}
				...
				if($this->input->post('vendor_id')!='')
				{
					...
					{
						if($this->jobs_sourcermodel->check_vendor_asigned($this->input->post('job_id'),$val)==0)
						{
							 $this->jobs_sourcermodel->add_vendor_to_job($this->input->post('job_id'),$val);
							 $vendor_details= $this->jobs_sourcermodel->get_vendor($val);							 
	...
	function email_vendor($email_to,$email_to_name,$job_title)
	{
			...
			$email_array=array(
				...
				'date'                   =>  date('Y-m-d'),
				'email_template'         =>  'jobs_sourcer/email_template_vendor',
			);
	...
	function manage($id=null)
	{
		...
		if(!empty($id))
		{
			...
			$this->data['formdata']= $this->jobs_sourcermodel->get_job_complete($id);
			
			$this->data['company_info']=$this->jobs_sourcermodel->get_company_info($this->data['formdata']['company_id']);
			...
			$this->data['applied_candidates']=$this->jobs_sourcermodel->get_candidate_list($id);
			$this->data['job_change_list']=$this->jobs_sourcermodel->job_change_list($id);
			
			$this->data['rejected_candidates']=$this->jobs_sourcermodel->rejected_candidates($id);
		
			$this->data['shortlisted']=$this->jobs_sourcermodel->get_shortlisted($id);

			$this->data['interview_list']=$this->jobs_sourcermodel->get_interview_list($id);
			$this->data['interview_history']=$this->jobs_sourcermodel->get_interview_history($id);
			
			$this->data['interview_rejection_list']=$this->jobs_sourcermodel->get_interview_rejection_list($id);
	
			$this->data['candidates_selected']=$this->jobs_sourcermodel->candidates_selected($id);
			$this->data['offer_letters_issued']=$this->jobs_sourcermodel->offer_letters_issued($id);
			
			$this->data['offer_accepted']=$this->jobs_sourcermodel->offer_accepted($id);
			
			$this->data['invoice_generated']=$this->jobs_sourcermodel->invoice_generated($id);

			$this->data['invoice_list2']=$this->jobs_sourcermodel->invoice_generated($id);
		...
		$this->data["contracts_ending"]=$this->jobs_sourcermodel->contracts_ending($id,$this->data["start_date"],$this->data["end_date"]);
		
			$this->load->view('include/header');
			//$this->load->view('include/job_sidebar',$this->data);
			$this->load->view('jobs_sourcer/summary',$this->data);	
			$this->load->view('include/footer');
		}else
		{
			redirect('jobs_sourcer');
		...
		if($this->input->post('job_app_id')!='' && $this->input->post('candidate_id')!='')
		{
			...
			else
			{
				$data=array(
					...
					$this->jobs_sourcermodel->reject_interview($data);					
					$response = array(
						'data' => '',
						'status'=>'success',
					);
	...
	function add_to_shortlist()
	{
		...
		if($this->input->post('job_app_id')!='' && $this->input->post('candidate_id')!='')
		{
			...
			$this->jobs_sourcermodel->add_to_shortlist($data,$this->input->post('candidate_id'),$this->input->post('job_app_id'));
			$response=array(
						'status'            => 'success',
						); 	
    		header('Content-type: application/json');    					
	...
	function add_consultant_feedback($candidate_id)
	{
		...
		if($this->input->post('candidate_id')!='' && $this->input->post('job_id'))
		{
			...
			$this->jobs_sourcermodel->add_consultant_feedback($data,$this->data['candidate_id']);
			redirect('jobs_sourcer/manage/'.$this->data['job_id']);
		}
		exit();
	}
	...
	function open_consultant_feedback($candidate_id)
	{
		...
		$this->data['candidate_id']=$this->input->post('candidate_id');
		$this->data['job_id']=$this->input->post('job_id');
		$this->data['feedback']=$this->jobs_sourcermodel->get_consultant_feedback($this->data['candidate_id']);
		$output_html=$this->load->view('jobs_sourcer/consultant_feedback',$this->data,true);	
		echo $output_html;
	}
	...
	function candidate_profile($candidate_id)
	{
		...
		$this->load->view("jobs_sourcer/candidate_summary",$this->data);
	}
	...
	function send_shortlisted()
	{
		...
		{
			$job_details=$this->jobs_sourcermodel->get_job($this->input->post('job_id'));
			$user_details=$this->jobs_sourcermodel->get_user_info($_SESSION['admin_session']);
			...
			$job_role_name='';
			$job_role_name=$job_details['job_title'];
			$nickname='';
			...
			if(isset($user_details['nickname']) && $user_details['nickname']!='')
			{
			...
			$list_shortlisted=$this->jobs_sourcermodel->get_shortlisted_client($this->input->post('job_id'),$short_id);
			...
			foreach($list_shortlisted as $key => $val)
			{
				...
				$view_profile_url=$base_email_url.$this->config->item('profile_controller').'?view='.$cv_hash;
					...
					$data_array[]=array
						...
						'cv_url'            =>  '<strong><a style="color:#3600D1" href="'.$view_profile_url.'" target="_blank">View</a></strong>',
						); 
				}
				...
				$email_array=array(
					...
					'powered_by_web'         =>  $this->config->item('powered_by_web'),
					'date'                   =>  date('Y-m-d'),
					'email_template'         =>  'jobs_sourcer/email_template_shortlisted',
				);
			...
			//print_r($email_array);
			//exit();		
			$list_shortlisted=$this->jobs_sourcermodel->update_shortlisted_status($this->input->post('job_id'),$short_id);
			// email ending here 
			$response=array(
						'status'            => 'success',
						); 	
	...
	function client_cv()
	{
		...
		if($this->input->post('candidate_id')!='' && $this->input->post('job_app_id')!='')
		{
			...
			*/
			...
			$html_profile='<iframe width="100%;" scrolling="yes" height="800px;" src="'.$this->config->item('client_cv_preview').'profile_rms?candidate_id='.md5($candidate_id).'&job_app_id='.$job_app_id.'"></iframe>';
			echo $html_profile;
			exit();
		}else
		{
	...
	function active_seekers($id=null)
	{
		...
		if(!empty($id))
		{
			...
			$this->data['active_seekers']=$this->jobs_sourcermodel->active_seekers($id);
			...
			$this->load->view('include/header');
			$this->load->view('include/job_sidebar',$this->data);
			$this->load->view('jobs_sourcer/active_seekers',$this->data);	
			$this->load->view('include/footer');
		}else
		{
			redirect('jobs_sourcer');
...
// upcoming contracts
	function upcoming_contracts($id=null)
	{
		...
		{
			$this->jobs_sourcermodel->addcandidate($this->input->post('candidate_id'),$this->input->post('job_id'));
			redirect('jobs_sourcer/addcandidate/'.$this->input->post('job_id'));
		}
			...
			foreach ($this->input->post('candidate_id') as $key => $val)
 			{
				...
				$email_id=$this->jobs_sourcermodel->send_jd($data);
				// take email data back from database.
				$email_jd=$this->jobs_sourcermodel->get_email_jd($email_id);
				...
				$email_array=array(
					...
					'signature'              =>  '',
					'date'                   =>  date('Y-m-d'),
					'email_template'         =>  'jobs_sourcer/email_template',
				);
			// EMAIL TO ADMIN
				$this->send_email($email_array);
				redirect('jobs_sourcer/addcandidate/'.$this->input->post('job_id'));
		...
		$this->data["total_rows"]=$this->jobs_sourcermodel->get_filter_count($id);

		//paging starts here 
		$config['base_url'] = $this->config->item('base_url')."index.php/jobs_sourcer/upcoming_contracts/".$id."/?";
		$config['page_query_string'] = TRUE;
		$config['total_rows'] = $this->data["total_rows"];
		$config['query_string_segment'] = 'rows';
		$config['per_page'] =50;
		...
		$this->data["candidates"]=$this->jobs_sourcermodel->get_filter_records($id,$this->data['start'],$this->data['limit']);
		...
		$this->load->view('include/header');
		//$this->load->view('include/job_sidebar',$this->data);
		$this->load->view('jobs_sourcer/upcoming_contracts',$this->data);	
		$this->load->view('include/footer');
	...
	function suggestions($id=null)
	{
		...
		if(!empty($id))
		{
			...
			$this->data['suggestions_list']=$this->jobs_sourcermodel->get_candidate_suggestions($id);

		
		$this->load->view('include/header');
		$this->load->view('include/job_sidebar',$this->data);
		$this->load->view('jobs_sourcer/suggestions',$this->data);	
		$this->load->view('include/footer');
...
//INSTRUCTION AND GUIDE LINES
	...
	function instructions($id=null)
	{
		...
		if(!empty($id))
		{
			...
			$this->load->view('jobs_sourcer/instructions',$data);	
			$this->load->view('include/footer');
		}else
		{
			redirect('jobs_sourcer');
	...
	// adding candidates jobs_sourcer, sending invites etc.	
	function search_candidate($id=null)
	{
        ...
        $this->data['job_folders']        =$this->jobs_sourcermodel->get_job_folders();
		$this->data['job_cat_list']       =$this->jobs_sourcermodel->industry_list();
		$this->data['func_area_list']     =$this->jobs_sourcermodel->functional_list();
		$this->data['desig_list']         =$this->jobs_sourcermodel->desig_list();

		if(is_array($this->input->post('candidate_id')) && $this->input->post('job_id')!='' && $this->input->post('add_to_job')=='1')
		{
			$this->jobs_sourcermodel->addcandidate($this->input->post('candidate_id'),$this->input->post('job_id'));
			redirect('jobs_sourcer/search_candidate/'.$this->input->post('job_id'));
		}
        ...
        $this->data["total_rows"]=$this->jobs_sourcermodel->get_filter_count($id,$this->data['search_email'],$this->data['search_name'],$this->data['cur_job_status'],$this->data['job_folder_id'],$this->data['job_cat_id'],$this->data['func_id'],$this->data['desig_id'],$this->data['level_id'],$this->data['course_id'],$this->data['exp_years'],$this->data['lead_source']);

		//paging starts here 
		//$config['base_url'] = $this->config->item('base_url')."index.php/jobs_sourcer/search_candidate/".$id."/?";
        

        
        $config['base_url'] = $this->config->item('base_url')."index.php/jobs_sourcer/search_candidate/".$id."/?sort_by=".$this->data['sort_by']."&limit=".$this->data['limit']."&search_name=".$this->data['search_name']."&search_email=".$this->data['search_email']."&search_mobile=".$this->data['search_mobile']."&"."cur_job_status=".$this->data['cur_job_status']."&branch_id=".$this->data['branch_id']."&skills=".$this->data["skills"]."&level_id=".$this->data["level_id"]."&course_id=".$this->data["course_id"]."&spcl_id=".$this->data["spcl_id"]."&exp_years=".$this->data["exp_years"]."&lead_source=".$this->data['lead_source']."&job_folder_id=".$this->data['job_folder_id']."&job_cat_id=".$this->data['job_cat_id']."&func_id=".$this->data['func_id']."&desig_id=".$this->data['desig_id']."&created=".$this->data['created']."&updated=".$this->data['updated']."&exp_years=".$this->data['exp_years'];
        ...
        $this->data["candidates"]=$this->jobs_sourcermodel->get_filter_records($id,$this->data['start'],$this->data['limit'],$this->data['search_email'],$this->data['search_name'],$this->data['cur_job_status'],$this->data['job_folder_id'],$this->data['job_cat_id'],$this->data['func_id'],$this->data['desig_id'],$this->data['level_id'],$this->data['course_id'],$this->data['exp_years'],$this->data['lead_source']);
		...
		{
			$this->data["functional"] = $this->jobs_sourcermodel->function_list_by_category($this->data["postdata"]["job_cat_id"]);
		}
		else{
			$this->data["functional"]  = array('' => 'Select Functional Role');
		}
		...
		$this->load->view('include/header');
		$this->load->view('jobs_sourcer/candidates',$this->data);	
		$this->load->view('include/footer');	
	}
	...
	function send_mass_mail($id=null)
	{
		...
		if($this->input->post('emails_list')!='')
		{
			...
			if(is_array($emails_list))
			{
				foreach($emails_list as $key => $val)
				{
					if($val!='')
					{
						...
						$email_url='jobs_sourcer/job_details?job_id='.$email_md_hash;
						...
						$email_id=$this->jobs_sourcermodel->send_jd_email($data);
						// take email data back from database.
						$email_jd=$this->jobs_sourcermodel->get_email_jd($email_id);
						// send email
			...
			redirect('jobs_sourcer/send_mass_mail/'.$this->input->post('job_id'));
		}
		...
		// total count for paging
		$this->data["total_rows"]=$this->jobs_sourcermodel->get_filter_count_email($id);

		//paging starts here 
		$config['base_url'] = $this->config->item('base_url')."index.php/jobs_sourcer/send_mass_mail/".$id."/?";
		$config['page_query_string'] = TRUE;
		$config['total_rows'] = $this->data["total_rows"];
		$config['query_string_segment'] = 'rows';
		$config['per_page'] =50;
		...
		$this->data["candidates"]=$this->jobs_sourcermodel->get_filter_records_email($id,$this->data['start'],$this->data['limit']);
	
		$this->data["postdata"]["job_id"]=$id;
		
		$this->load->view('include/header');
		//$this->load->view('include/job_sidebar',$this->data);
		$this->load->view('jobs_sourcer/mass_email',$this->data);	
		$this->load->view('include/footer');
	}
	...
	function send_email($email_array=array())
	{
		...
		$headers.= "From: ".$email_array['nickname']." <".$email_array['email_from'].">\r\n";	
...
//		$headers.= "From: ".$email_array['from_name']." <".$email_array['email_from'].">\r\n";		
	...
	// add from jobs_sourcer to candidates, to job apps, skills, etc. 
	function add_candidate($job_id)
	{
		...
		{ 
			...
			*/
			...
			{
				$this->jobs_sourcermodel->addcandidate(array('candidate_id' => $id),$this->input->post('job_id'));
			}
			...
			redirect('jobs_sourcer/manage/'.$job_id.'?add=1');			
		}

		$this->data['job_change_list']=$this->jobs_sourcermodel->import_from_other_jobs($job_id);
		
		$this->data['page_head']= 'Manage Job';
		$this->data['formdata']['job_id']= $job_id;		

		$this->load->view('include/header');
		$this->load->view('jobs_sourcer/add_candidate',$this->data);	
		//$this->load->view('include/footer');
	}
	...
	function shortlist($id=null)
	{
		...
		{		
			...
			$this->jobs_sourcermodel->shortlist($this->input->post('candidate_id'),$this->input->post('job_id'),$this->input->post('app_id'));
			//echo $this->db->last_query();exit;
			redirect('jobs_sourcer/shortlist/'.$this->input->post('job_id'));
		}
		...
		if($id!='')
		{
			...
			$data['applied_candidates']=$this->jobs_sourcermodel->applied_candidates($id);

			$data['shortlisted_candidates']=$this->jobs_sourcermodel->get_shortlisted($id);
			
			$this->load->view('include/header');
			$this->load->view('include/job_sidebar',$data);
			$this->load->view('jobs_sourcer/shortlist',$data);	
			$this->load->view('include/footer');

		}else
		{
			redirect('jobs_sourcer/shortlist');
		}
	}
	...
	function manage_rejection($id=null)
	{
		...
		{		
			$this->jobs_sourcermodel->manage_rejection($this->input->post('job_app_id'),$this->input->post('job_id'));
						$response = array(
			    'data' => '',
				'status'=>'success',
			);
	...
	function add_candidate_to_job()
	{
		...
		if($this->input->get('candidate_id')!='' && $this->input->get('job_id')!='')
		{
			...
			$this->jobs_sourcermodel->addcandidate(array('candidate_id' => $this->input->get('candidate_id')),$this->input->get('job_id'));
	...
	function import_from_other_jobs()
	{
		...
		if($this->input->post('cur_job_id')!='')
		{
			if($this->input->post('candidate_source')==1)
			{
				...
				{				
					$applicants_list=$this->jobs_sourcermodel->get_from_applicant_list($val);
					...
					foreach($applicants_list as $key => $job_app)
					{
						...
						$this->jobs_sourcermodel->add_candidates_from_other_jobs($job_app);
					}
				}
			}if($this->input->post('candidate_source')==2)
			{
				foreach($this->input->post('job_id') as $key => $val)
				{				
					$applicants_list=$this->jobs_sourcermodel->get_from_short_listed_list($val);
			...
			}if($this->input->post('candidate_source')==3)
			{
				foreach($this->input->post('job_id') as $key => $val)
				{				
					$applicants_list=$this->jobs_sourcermodel->get_from_rejected_list($val);
			...
			}if($this->input->post('candidate_source')==4)
			{
				foreach($this->input->post('job_id') as $key => $val)
				{				
					$applicants_list=$this->jobs_sourcermodel->get_from_interview_list($val);
					foreach($applicants_list as $key => $job_app)
					{
						unset($job_app['job_app_id']);					
						$job_app['job_id']=$job_id;
			...
			redirect('jobs_sourcer/add_candidate/'.$job_id.'?add=1');		
		}else
		{
			$response=array(
					'success'            => 'failed',
	...
	function save_job_change()
	{
		...
		if($this->input->post('candidate_id')!='' && $this->input->post('job_app_id')!='')
		{
			if(is_array($this->input->post('job_id')))
			{
				foreach($this->input->post('job_id') as $key => $val)
				{
					$this->jobs_sourcermodel->addcandidate(array('candidate_id' => $this->input->post('candidate_id')),$val);
				}
			}
			
			if($this->input->post('remove_from')==1)
			{
				$this->jobs_sourcermodel->remove_from_apps($this->input->post('candidate_id'),$this->input->post('job_app_id'),$this->input->post('cur_job_id'));
			}
			$response=array(
						'status'            => 'success',
						); 	
	...
	function add_feedback()
	{
		...
		{
			$job_id=$this->jobs_sourcermodel->add_feedback();	
			$response = array(
			    'data' => '',
				'status'=>'success',
			);
	...
	function send_interview_list()
	{
		...
		{			
			...
			$list_shortlisted=$this->jobs_sourcermodel->get_shortlisted_client($this->input->get('job_id'));
			...
			$email_array=array(
				...
				'date'                   =>  date('Y-m-d'),
				'email_template'         =>  'jobs_sourcer/email_template_interviewlist',
			);
	...
	function addinterview()
	{
		...
		if($this->input->post('job_app_id')!='' && $this->input->post('candidate_id')!='' && $this->input->post('title')!='' )
		{
			...
			$job_id=$this->jobs_sourcermodel->add_interview($data,$this->input->post('candidate_id'),$this->input->post('job_app_id'));	
			...
			$this->jobs_sourcermodel->add_interview_history($data,$this->input->post('candidate_id'),$this->input->post('job_app_id'));			
			...
			$interview_list =$this->jobs_sourcermodel->get_interview_list($id);            
			*/
			$response = array(
			    'data' => '',
				'status'=>'success',
	...
	function add_calls()
	{
		...
		if($this->input->post('job_app_id')!='' && $this->input->post('candidate_id')!='')
		{
			$job_id=$this->jobs_sourcermodel->add_calls();	
			$response = array(
			    'data' => '',
				'status'=>'success',
			);
	...
	function add_ctc()
	{
		...
		if($this->input->post('job_app_id')!='' && $this->input->post('candidate_id')!='')
		{
			$job_id=$this->jobs_sourcermodel->add_ctc();	
			$response = array(
			    'data' => '',
				'status'=>'success',
			);
	...
	function add_notes()
	{
		...
		{
			$job_id=$this->jobs_sourcermodel->add_notes();	
			$response = array(
			    'data' => '',
				'status'=>'success',
			);
	...
	function add_message()
	{
		...
		{
			$job_id=$this->jobs_sourcermodel->add_message();	
			$response = array(
			    'data' => '',
				'status'=>'success',
			);
	...
	function select_candidate($id=null)
	{
		...
		{
			$this->jobs_sourcermodel->select_candidate();
			$this->db->where('job_id', $id);
			$query=$this->db->get('pms_jobs');
			$formdata =$query->row_array();
			//$this->jobs_sourcermodel->common_delete2($this->input->get('app_id'),$this->input->get('candidate_id'),'pms_job_apps_interviews');
			$candidates_selected =$this->jobs_sourcermodel->candidates_selected($id);
                        ...
                        <th width="26%">Action</th>
						  ...
						  foreach($candidates_selected as $selected){
							  ...
							  $html.='<tr>
										  ...
										  <td><a href="#" data-reveal-id="interview" data-animation="fade" class="btn btn-primary btn-xs">Change</a> | <a href="javascript:;"  data-url="'.base_url().'index.php/jobs_sourcer/delete_selectedcandidate/?job_app_id='.$selected['app_id'].'&candidate_id='.$selected['candidate_id'].'&job_id='.$formdata['job_id'].'"  id="delete_selected_candidate" class="btn btn-danger btn-xs">X </a>|<a href="'.base_url().'index.php/candidates_all/summary/'.$selected['candidate_id'].'" class="btn btn-info btn-xs" target="_blank"> Profile </a> | <a href="javascript:;" onclick="issue_offer('.$formdata['job_id'].','.$selected['app_id'].','.$selected['candidate_id'].');" id="issue_offer" class="btn btn-info btn-xs"> Issue Offer </a></td>';	
	...
	function get_select_candidate()
	{
		...
		$interview_schedule = $this->jobs_sourcermodel->interview_schedule($id);
	...
	function get_candidate_contract()
	{
		...
		$contracts_ending=$this->jobs_sourcermodel->contracts_ending($id,$start_date,$end_date);
					 ...
					 foreach($contracts_ending as $contract)
					 {
						...
						$html2.='<tr>
                              ...
                              <td width="13%">'. date('d-m-Y',strtotime($contract['end_date'])).'</td>
									  <td width="13%"> <a href="javascript:;" data-url="'.base_url().'index.php/jobs_sourcer/add_to_job/?candidate_id='.$contract['candidate_id'].'&job_id='.$formdata['job_id'].'"  id="add_to_job"  > Add to Job </a></td>
									  </tr>';
	...
	function get_offer_issued()
	{
		...
		$offer_letters_issued =$this->jobs_sourcermodel->offer_letters_issued($id);
	...
	{ 
		...
		if($this->input->post('app_id')!='' && $this->input->post('candidate_id')!='')
		{
			$this->jobs_sourcermodel->reject_offer();
	...
	{ 
		...
		if($this->input->post('app_id')!='' && $this->input->post('candidate_id')!='')
		{
			$this->jobs_sourcermodel->issue_offer();
	...
	function accept_offer($id=null)
	{
		...
		if($this->input->post('app_id')!='' && $this->input->post('candidate_id')!='')
		{
			$this->jobs_sourcermodel->accept_offer();			
			redirect('jobs_sourcer/manage/'.$id);
		}
		...
		$this->load->view('include/header',$this->data);
		$this->load->view('jobs_sourcer/offerletter',$this->data);	
		$this->load->view('include/footer',$this->data);		
	}
...
//GET ACCEPT OFFER LIST
	function get_offer_accepted()
	{
		...
		$offer_accepted = $this->jobs_sourcermodel->offer_accepted($id);
		...
		if(!empty($offer_accepted)){
			...
			$html2='<td colspan="2" align="center" valign="top">
				...
				$html2.='<tr>
						  ...
						  <td><p><a href="javascript:;" data-url="'.base_url().'index.php/jobs_sourcer/delete_acceptcandidate/?job_id='.$formdata['job_id'].'&app_id='.$accepted['app_id'].'&candidate_id='.$accepted['candidate_id'].'&placement_id='.$accepted['placement_id'].'" id="delete_accept_candidate" class="btn btn-danger btn-xs">X </a> <a href="'.base_url().'index.php/candidates_all/summary/'.$accepted['candidate_id'].'" target="_blank" class="btn btn-info btn-xs"> Profile </a>&nbsp;<a href="javascript:;" onclick="create_invoice('.$formdata['job_id'].','.$accepted['app_id'].','.$accepted['candidate_id'].','. $accepted['placement_id'].');" class="btn btn-primary btn-xs">Invoice</a></p></td>';
...
//get certy attestaion details
	...
	function get_cert_attest()
	{
		...
		$get_cert_attest = $this->jobs_sourcermodel->get_cert_attest($id);
		...
		if(!empty($get_cert_attest)){
			...
			$html2 =	'<td colspan="2" align="center" valign="top">
				...
				$html2.='<tr>
						  ...
						  <td><a href="#" data-reveal-id="interview" data-animation="fade">Change</a> | <a href="javascript:;"  data-url="'.base_url().'index.php/jobs_sourcer/delete_attest/?job_id='.$formdata['job_id'].'&app_id='.$cert_attest['app_id'].'&candidate_id='.$cert_attest['candidate_id'].'&cert_id='.$cert_attest['cert_id'].'" id="delete_attest" >X </a>|<a href="'.base_url().'index.php/candidates_all/summary/'.$cert_attest['candidate_id'].'" target="_blank"> Profile </a> | <a href="javascript:;" onclick="create_visa('.$formdata['job_id'].','.$cert_attest['app_id'].','.$cert_attest['candidate_id'].');"> Visa Details</a>  </td>';
...
//CERT ATTEST
	function cert_attest()
	{
		...
		if($this->input->post('app_id')!='' && $this->input->post('candidate_id')!='' )
		{
			$this->jobs_sourcermodel->cert_attest();	
	...
	function create_invoice($id=null)
	{
		...
		{
			$this->jobs_sourcermodel->create_invoice();			
			redirect('jobs_sourcer/manage/'.$id);
		}
		$this->data['invoice_list']=$this->jobs_sourcermodel->invoice_generated($id);
		...
		$this->load->view('include/header',$this->data);
		$this->load->view('jobs_sourcer/create_invoice',$this->data);	
		$this->load->view('include/footer',$this->data);	
	}
	...
	function create_invoice2()
	{
		...
		if($this->input->post('placement_id')!='' && $this->input->post('app_id')!='' && $this->input->post('candidate_id')!='' && $this->input->post('invoice_date')!='')
		{
			...
			$invoice_generated=$this->jobs_sourcermodel->invoice_generated($id);
		   ...
		   $html.=' <tr>
					  ...
					  <td width="11%">'.$client_candidate.'</td>
                       <td><a href="javascript:;"  data-url="'.base_url().'index.php/jobs_sourcer/delete_invoice/?job_id='.$formdata['job_id'].'&placement_id='.$invoice['placement_id'].'&invoice_id='.$invoice['invoice_id'].'"  id="delete_invoice_candidate" class="btn btn-danger btn-xs" >X</a></td></tr>';
...
// both invoice deletion and get the list for AJAX display
	...
	function get_invoice_list()
	{
		...
		if($this->input->get('placement_id')!='')
		{
				...
				$invoice_generated=$this->jobs_sourcermodel->invoice_generated($job_id);
		   ...
		   $html.=' <tr>
                      ...
                      <td width="11%">'.$invoice_status.'</td>
					   <td width="11%">'.$client_candidate.'</td>
                       <td><a href="'.base_url().'index.php/jobs_sourcer/create_invoice/'.$formdata['job_id'].'/?placement_id='.$invoice['placement_id'].'&invoice_id='.$invoice['invoice_id'].'"> &nbsp;Edit&nbsp;</a>|<a href="javascript:;"  data-url="'.base_url().'index.php/jobs_sourcer/delete_invoice/?job_id='.$formdata['job_id'].'&placement_id='.$invoice['placement_id'].'&invoice_id='.$invoice['invoice_id'].'"  id="delete_invoice_candidate" >Delete</a>|<a href="'.base_url().'index.php/candidates_all/summary/'.$invoice['candidate_id'].'" target="_blank">Profile</a></td></tr>';
...
// both invoice deletion and get the list for AJAX display
	...
	function delete_application($id=null)
	{
		...
		{
			$this->jobs_sourcermodel->delete_application($this->input->get('candidate_id'),$id);
			redirect('jobs_sourcer/manage/'.$id.'/?del=1');
		}
		exit();
	}
	...
	function delete_from_shortlist($id=null)
	{
		...
		if($this->input->get('app_id')!='' && $this->input->get('candidate_id')!=''  & $id!='')
		{
			$this->jobs_sourcermodel->delete_from_shortlist($this->input->get('candidate_id'),$id);
			redirect('jobs_sourcer/manage/'.$id.'/?del=1');
		}
		exit();
	}
	...
	function delete_interview_schedule($id=null)
	{
		...
		if($this->input->get('app_id')!='' && $this->input->get('candidate_id')!=''  & $id!='')
		{
			$this->jobs_sourcermodel->delete_interview_schedule($this->input->get('candidate_id'),$id);
			redirect('jobs_sourcer/manage/'.$id.'/?del=1');
		}
		exit();
	}
	...
	function delete_selected_candidate($id=null)
	{
		...
		if($this->input->get('app_id')!='' && $this->input->get('candidate_id')!=''  & $id!='')
		{
			$this->jobs_sourcermodel->delete_selected_candidate($this->input->get('candidate_id'),$id);
			redirect('jobs_sourcer/manage/'.$id.'/?del=1');
		}
		exit();
	}
	...
	function delete_offer_letter($id=null)
	{
		...
		if($this->input->get('app_id')!='' && $this->input->get('candidate_id')!=''  & $id!='')
		{
			$this->jobs_sourcermodel->delete_offer_letter($this->input->get('candidate_id'),$id);
			redirect('jobs_sourcer/manage/'.$id.'/?del=1');
		}
		exit();
	}
	...
	function delete_placed_candidate($id=null)
	{
		...
		if($this->input->get('app_id')!='' && $this->input->get('candidate_id')!=''  & $id!='')
		{
			$this->jobs_sourcermodel->delete_placed_candidate($this->input->get('candidate_id'),$id);
			redirect('jobs_sourcer/manage/'.$id.'/?del=1');
		}
		exit();
	}
	...
	function delete_candidate_invoice($id=null)
	{
		...
		{
			$this->jobs_sourcermodel->delete_candidate_invoice($this->input->get('placement_id'),$this->input->get('invoice_id'));
			redirect('jobs_sourcer/manage/'.$id.'/?del=1');
		}
		exit();
	}
	...
	function delete($id=null)
	{
		...
		{		
			...
			$id=$this->jobs_sourcermodel->delete_records($delete_rec);
			
		}elseif(is_array($this->input->post('checkbox')))
		{
			$id=$this->jobs_sourcermodel->delete_records($this->input->post('checkbox'));
		}
		if($id==true)
			redirect('jobs_sourcer/?rows='.$rows.'&del=1');
		else
			redirect('jobs_sourcer/?rows='.$rows.'&del=0');
	...
	function multidelete(){
		...
		if(count($id_arr)>0){
			$this->load->model('jobs_sourcermodel');
			$this->jobs_sourcermodel->delete_multiple_record($id_arr);
			redirect('jobs_sourcer/?multi=1');
		}
		else{
			redirect('jobs_sourcer');
		}
	...
	function manage_interview($id=null)
	{
		...
		if(!empty($id))
		{
			...
			$data['shortlisted']=$this->jobs_sourcermodel->get_shortlisted($id);
			//echo '<pre>';print_r($data['shortlisted']);exit;
			$data['interview_list']=$this->jobs_sourcermodel->get_interview_list($id);
		...
		$data["int_status_id"] = $this->interviewstatusmodel->get_model_list();
			...
			$this->load->view('include/header');
			$this->load->view('include/job_sidebar',$data);
			$this->load->view('jobs_sourcer/manage_interview',$data);	
			$this->load->view('include/footer');
		}else
		{
			redirect('jobs_sourcer');
	...
	function manage_offer($id=null)
	{
		...
		if(!empty($id))
		{
			...
			$data['candidates_selected']=$this->jobs_sourcermodel->candidates_selected($id);
			$data['offer_letters_issued']=$this->jobs_sourcermodel->offer_letters_issued($id);
			
			
			$this->load->view('include/header');
			$this->load->view('include/job_sidebar',$data);
			$this->load->view('jobs_sourcer/manage_offer',$data);	
			$this->load->view('include/footer');
		}else
		{
			redirect('jobs_sourcer');
	...
	function manage_invoice($id=null)
	{
		...
		if(!empty($id))
		{
			...
			$data['offer_accepted']=$this->jobs_sourcermodel->offer_accepted($id);
			$data['invoice_generated']=$this->jobs_sourcermodel->invoice_generated($id);
			$data['invoice_list2']=$this->jobs_sourcermodel->invoice_generated($id);
						
			$this->load->view('include/header');
			$this->load->view('include/job_sidebar',$data);
			$this->load->view('jobs_sourcer/manage_invoice',$data);	
			$this->load->view('include/footer');
		}else
		{
			redirect('jobs_sourcer');
	...
	function job_apps($id=null)
	{
		...
		if(!empty($id))
		{
			...
			$this->load->view('include/header',$this->data);
			$this->load->view('include/job_sidebar',$this->data);
			$this->load->view('jobs_sourcer/jobs_applied',$this->data);	
			$this->load->view('include/footer',$this->data);
		}else
		{
			redirect('jobs_sourcer');
	...
	function delete_applied_candidate()
	{
		...
		if($this->input->get('job_app_id')!='' && $this->input->get('candidate_id')!='')
		{
			...
			{					
					...
					$applied =$this->jobs_sourcermodel->get_candidate_list($job_id);
					$count =count($applied);
					$this->db->where('job_id', $job_id);
					$query=$this->db->get('pms_jobs');
					$formdata =$query->row_array();
					...
					foreach($applied as $candidate)
					{
						...
						$html.='<tr>
								  ...
								  <td width="25%"><a href="javascript:;"  data-url="'.base_url().'index.php/jobs_sourcer/shortlist2/?job_app_id='.$candidate['job_app_id'].'&candidate_id='.$candidate['candidate_id'].'&job_id='.$formdata['job_id'].'"  id="shortlisted_candidate" > Short List </a> | <a href="javascript:;"  data-url="'.base_url().'index.php/jobs_sourcer/delete_applied_candidate/?job_app_id='.$candidate['job_app_id'].'&candidate_id='. $candidate['candidate_id'].'&job_id='.$formdata['job_id'].'"  id="delete_applied_candidate" > Delete </a></td>
	...
	function delete_shortlisted_candidate()
	{
		...
		if($this->input->get('job_app_id')!='' && $this->input->get('candidate_id')!='')
		{
			...
			else
			{
					...
					$shortlisted =$this->jobs_sourcermodel->get_shortlisted($job_id);
					...
					foreach($shortlisted as $candidate){
						...
						$html.='<tr>
								  <td width="44%"><a href="'.base_url().'index.php/candidates_all/summary/'.$candidate['candidate_id'].'" target="_blank">'.$candidate['first_name'].' '.$candidate['last_name'].'</a></td>
								  <td width="31%">'.$candidate['skills'].'</td>          
								 <td width="25%"> <a href="javascript:;"  onclick="interview('.$candidate['candidate_id'].','.$candidate['job_app_id'].','.$formdata['job_id'].')" >Interview</a>|<a href="javascript:;"  data-url="'.base_url().'index.php/jobs_sourcer/delete_shortlisted_candidate/?job_app_id='.$candidate['job_app_id'].'&candidate_id='.$candidate['candidate_id'].'&job_id='.$formdata['job_id'].'"  id="delete_shortlisted_candidate" >Remove</a></td>
	...
	function delete_selectedcandidate()
	{
		...
		if($this->input->get('job_app_id')!='' && $this->input->get('candidate_id')!='')
		{
			...
			else
			{
					...
					$formdata =$query->row_array();
					//$this->jobs_sourcermodel->common_delete2($this->input->get('app_id'),$this->input->get('candidate_id'),'pms_job_apps_interviews');
					$candidates_selected =$this->jobs_sourcermodel->candidates_selected($job_id);
						 ...
						 <tbody >					
						  ...
						  foreach($candidates_selected as $selected){
							  ...
							  $html.='<tr>
										  ...
										  <td width="11%">&nbsp;</td>
										  <td width="11%">'.$selected['feedback'].'</td>
										  <td><a href="javascript:;"  data-url="'.base_url().'index.php/jobs_sourcer/delete_selectedcandidate/?job_app_id='.$selected['app_id'].'&candidate_id='.$selected['candidate_id'].'&job_id='.$formdata['job_id'].'"  id="delete_selected_candidate" >Remove </a>| <a href="javascript:;" onclick="issue_offer('.$formdata['job_id'].','.$selected['app_id'].','.$selected['candidate_id'].');" id="issue_offer"> Issue Offer </a>|<a href="'.base_url().'index.php/candidates_all/summary/'.$selected['candidate_id'].'" target="_blank"> Profile </a> </td>';
	...
	function delete_offercandidate()
	{
		...
		if($this->input->get('job_app_id')!='' && $this->input->get('candidate_id')!='')
		{
			...
			else
			{
					...
					$formdata =$query->row_array();
					//$this->jobs_sourcermodel->common_delete2($this->input->get('app_id'),$this->input->get('candidate_id'),'pms_job_apps_interviews');
					$data['offer_letters_issued']=$this->jobs_sourcermodel->offer_letters_issued($job_id);
					
					$offer_letters_issued =$this->jobs_sourcermodel->offer_letters_issued($job_id);
	...
	function add_to_job()
	{
		...
		if($this->input->get('job_id')!='' && $this->input->get('candidate_id')!='')
		{
				...
				$result = $this->jobs_sourcermodel->add_to_job();
	...
	function delete_shortlisted()
	{
		...
		if($this->input->get('job_app_id')!='' && $this->input->get('candidate_id')!='')
		{
			...
			if(!empty($result))
			{
					redirect('jobs_sourcer/shortlist/'.$this->input->get('job_id').'/?del=1');
			}			
			...
			else
			{
					...
					$result = $this->db->query(' DELETE FROM pms_job_apps_shortlisted WHERE (app_id ="'.$id.'" AND candidate_id ="'.$c_id.'")');
					redirect('jobs_sourcer/shortlist/'.$this->input->get('job_id').'/?del=2');
			}
		}
...
//CREATE TICKET
	function create_ticket()
	{
		...
		{
			$this->jobs_sourcermodel->create_ticket();
...
//onchange get function	
	...
	{		
		...
		{
			$data=array();
			$data["function_list"] = $this->jobs_sourcermodel->function_list_by_category($_POST['category_id']);
			$function	='';
