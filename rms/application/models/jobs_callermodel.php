<?php 
class Jobs_callermodel extends CI_Model {
		var $table_name='';
	...
	function record_count($searchterm,$job_status,$company_id,$job_priority) 
	{
		...
		$sql = "select count(*)as job_id from ".$this->table_name." a ";
		$cond = '';
	...
	function get_job($job_id)
    {
		...
		$query=$this->db->query("select * from pms_jobs where job_id=".$job_id." and job_id in (select job_id from pms_jobs_to_recruiter where admin_id=".$_SESSION['admin_session'].")");
	...
	function child_skill()
	{
		...
		//~ $id=1;
		$this->data['skillset']=$this->jobs_callermodel->get_child_skills($id);
...
//delete ticket FOLLOWUP
	...
	{
		if($job_id=='')return;
		$query = $this->db->query('select a.*,b.*,c.* from pms_candidate a inner join pms_job_apps b on a.candidate_id=b.candidate_id inner join pms_job_apps_shortlisted c on b.job_app_id=c.app_id where b.job_id='.$job_id);
		$dropdowns = $query->result_array();	
		return $dropdowns;
	}
