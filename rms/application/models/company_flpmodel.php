...
// values for graph
	...
	{
		$query=$this->db->query('select count(status) as total_count,( CASE WHEN status=0 THEN "Unknown" WHEN status=4 THEN "Not Qualified" WHEN status=1 THEN "Feed" WHEN status=2 THEN "Lead" WHEN status=3 THEN "Client" END)as status from pms_company group by status');
		return $query->result_array();
	}
	...
	{
		  $query=$this->db->query('select count(flp_status) as total_count,( CASE WHEN flp_status=0 THEN "Unknown" WHEN flp_status=1 THEN "Contact Updated &amp; Profile Sent" WHEN flp_status=2 THEN "Followed up - Call Later" WHEN flp_status=3 THEN "Positive Response - Need Follow up" WHEN flp_status=4 THEN "Meeting Arranged &amp; Proposal Sent" WHEN flp_status=5 THEN "Active Client - Have Business" WHEN flp_status=6 THEN "Inactive Client - Need Follow up" WHEN flp_status=7 THEN "No Need to Follow up" END)as flp_status from pms_company_followup group by flp_status');
		return $query->result_array();
	}
