...
//delete ticket FOLLOWUP
	...
	{
		if($job_id=='')return;
		$query = $this->db->query('select a.*,b.*,c.short_id,c.app_id,c.short_date,c.admin_id from pms_candidate a inner join pms_job_apps b on a.candidate_id=b.candidate_id inner join pms_job_apps_shortlisted c on b.job_app_id=c.app_id where b.job_id='.$job_id);
		$dropdowns = $query->result_array();	
		return $dropdowns;
	}
