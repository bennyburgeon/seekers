...
class Tasksmodel extends CI_Model{
	...
	function record_count($searchterm,$date_range,$priority,$status,$admin_id) 
	{
...
$sql	= "select count(*) as total_task from ".$this->task_table." a left join pms_task_status b on a.task_status_id=b.task_status_id left join pms_task_priority c on a.task_priority_id=c.task_priority_id left join pms_candidate d on a.candidate_id=d.candidate_id ";
	...
	function get_list($start,$limit,$searchterm,$sort_by,$date_range,$priority,$status,$admin_id) 
    {
	...
	function select_record($id)
	{
		...
		$query = $this->db->query("select a.*,b.*,c.* from pms_task_followup a inner join pms_task_status b ON a.task_status=b.task_status_id left join pms_admin_users c on a.user_id=c.admin_id where a.task_fl_id = ".$id);
		if ($query->num_rows() > 0)
		{
			return $query->row_array();
		}else
	...
	function get_followup_list($id){
		$query = $this->db->query("select a.*,b.*,c.* from pms_task_followup a left join pms_task_status b ON a.task_status=b.task_status_id
left join pms_admin_users c on a.user_id=c.admin_id where a.task_id = $id order by a.task_fl_id   ASC ");
		return $query->result_array();
	}
