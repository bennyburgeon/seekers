...
class Mytasksmodel extends CI_Model{
	...
	function record_count($searchterm,$date_range,$priority,$status) 
	{
		...
		$cond='';
		$cond .=" a.admin_id=" . $_SESSION['admin_session'];
	...
	function select_record($id)
	{
		...
		$query = $this->db->query("select a.*,b.*,c.* from pms_task_followup a left join pms_task_status b ON a.task_status=b.task_status_id left join pms_admin_users c on a.user_id=c.admin_id where a.task_fl_id = ".$id);
		if ($query->num_rows() > 0)
		{
			return $query->row_array();
		}else
	...
	function get_followup_list($id){
		$query = $this->db->query("select a.*,b.*,c.* from pms_task_followup a left join pms_task_status b ON a.task_status=b.task_status_id left join pms_admin_users c on a.user_id=c.admin_id where a.task_id = $id order by a.task_fl_id   ASC ");
		return $query->result_array();
	}
