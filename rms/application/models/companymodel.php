...
class companymodel extends CI_Model {
	...
	function record_count($searchterm,$ind_id,$flp_status,$company_priority,$company_rating,$status,$flp_next_date,$date_added,$user_id,$unassigned,$city_id,$building_location,$state_id, $country_id) 
	{
	...
	if($ind_id!='')
	{
		...
		{
			if($cond!=''){
				$cond.=" and a.city_id in (select city_id from pms_city ct inner join pms_state st on ct.state_id=st.state_id where st.country_id=" . $country_id.")";
			} 
			else{
				$cond.=" a.city_id in (select city_id from pms_city ct inner join pms_state st on ct.state_id=st.state_id where st.country_id=" . $country_id.")";
			}  
		} 
	...
	function get_list($start,$limit,$searchterm,$ind_id,$flp_status,$sort_by,$company_priority,$company_rating,$status,$flp_next_date,$date_added,$user_id,$unassigned,$city_id,$building_location,$state_id, $country_id)
	{
		$sql="select a.*,b.firstname as created_by, ct.city_name, st.state_name, cn.country_name,ind.job_cat_name, ";
		$sql.="(select flp_date from pms_company_followup where company_id=a.company_id order by flp_id desc limit 0,1) as flp_date,";
		$sql.="(select count(flp_id) from pms_company_followup where company_id=a.company_id) as total_flp,";
		$sql.="(select flp_next_date from pms_company_followup where company_id=a.company_id order by flp_id desc limit 0,1) as flp_next_date,";
		$sql.="(select flp_notes from pms_company_followup where company_id=a.company_id order by flp_id desc limit 0,1) as flp_notes,";
		$sql.="(select flp_status from pms_company_followup where company_id=a.company_id and flp_status>0 order by flp_id desc limit 0,1 ) as flp_status";
		$sql.=",(select firstname from pms_admin_users ad inner join pms_company_followup cpl on ad.admin_id=cpl.admin_id where cpl.company_id=a.company_id order by cpl.flp_id desc limit 0,1) as firstname,";
		$sql.="(select count(*) as total_jobs from pms_jobs jb where jb.company_id=a.company_id) as total_jobs ";

		$sql.=" from ".$this->table_name." a left join pms_admin_users b on a.user_id=b.admin_id ";	
		$sql.=" left join pms_job_category ind on a.ind_id=ind.job_cat_id ";	
		$sql.=" left join pms_city ct on a.city_id=ct.city_id ";	
		$sql.=" left join pms_state st on ct.state_id=st.state_id ";	
		$sql.=" left join pms_country cn on st.country_id=cn.country_id ";	
	...
	function add_calls()
	{
		...
		{
			$data['flp_status']= $this->input->post('flp_status');
		}else
	...
	{
			$sql=" select a.*,b.* from pms_company_followup a inner join pms_admin_users b on a.admin_id=b.admin_id where a.company_id=".$company_id." order by a.flp_id desc limit 0,20	";
			$query = $this->db->query($sql);
			return $query->result_array();	
	}

	function get_last_call($company_id)
	{
			$sql=" select a.*,b.* from pms_company_followup a inner join pms_admin_users b on a.admin_id=b.admin_id where a.company_id=".$company_id." order by a.flp_id desc limit 0,1";
			$query = $this->db->query($sql);
			return $query->result_array();	
	}
	...
	function update_record($id=NULL)
	{
		...
		if($this->upload_file_name!='')$data['company_logo']= $this->upload_file_name;
	...
	{
		$query = $this->db->query("select distinct a.country_id, concat(a.country_name,' [+ ',intl_dial_prefix,' ',intl_code,']')as country_name from pms_country a order by country_name asc");
		$dropdowns = $query->result();
  ...
  function state_list_by_city($country_id='')
    {
		if($country_id !='')
			$query=$this->db->query("select a.*,b.* from pms_state a inner join pms_city b ON a.state_id=b.state_id where a.country_id=".$country_id." order by a.state_name asc");
		else
			$query=$this->db->query("select a.*,b.* from pms_state a inner join pms_city b ON a.state_id=b.state_id order by a.state_name asc");
	 ...
	 function insert_csv_records($data)
    {
        ...
        $id=$this->db->insert_id();
        //$this->db->query('update pms_company set user_id='.$_SESSION['admin_session']);
		
		if($admin_id >0)
		{
			$data=array('company_id' => $id, 'admin_id' => $admin_id);
			$this->db->insert('pms_company_to_recruiter', $data);
			$id=$this->db->insert_id();
		}
        return $id;
