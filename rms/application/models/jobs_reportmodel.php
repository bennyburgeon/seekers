...
class Jobs_reportmodel extends CI_Model {
	...
	function record_count($admin_id,$company_id,$searchterm) 
	{
		...
		$sql="SELECT count(DISTINCT a.job_id) as total_rec FROM `pms_jobs` a inner join pms_company b on a.company_id=b.company_id inner join pms_jobs_to_recruiter c on a.job_id=c.job_id";	
		$cond = " job_status=1 ";
