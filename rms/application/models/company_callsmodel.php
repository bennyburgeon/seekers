<?php 
class Company_callsmodel extends CI_Model {
	...
	function record_count($searchterm,$ind_id,$flp_status,$company_priority,$company_rating,$status,$flp_next_date) 
	{
	
	$sql = "select count(*)as company_id from ".$this->table_name ." a ";
	$cond = " a.status =2 and company_id in (select company_id from pms_company_to_recruiter where admin_id=".$_SESSION['admin_session'].") ";
	...
	{
		if($cond!=''){
			$cond.=" and a.company_id in (select company_id from pms_company_to_recruiter where admin_id=" . $_SESSION['admin_session'].") ";
		} 
		else{
			$cond.=" a.company_id in (select company_id from pms_company_to_recruiter where admin_id=" .$_SESSION['admin_session'].") ";
		}  
	} 
	...
	{
			$sql=" select a.* from pms_company a where a.company_id=".$company_id;
			$query = $this->db->query($sql);
			return $query->row_array();	
	}
