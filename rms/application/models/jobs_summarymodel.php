<?php 
class Jobs_summarymodel extends CI_Model {
	var $table_name='';
	...
	function record_count($admin_id,$company_id) 
	{
	
		$sql="SELECT count(a.job_id) as total_rec FROM `pms_jobs` a inner join pms_company b on a.company_id=b.company_id ";	
		$cond = "";
	...
	function get_list($start,$limit,$admin_id,$sort_by,$company_id)
	{
		
		$sql='SELECT a.*,b.company_name,(select count(job_app_id) from pms_job_apps where job_id=a.job_id)as total_apps ';
		...
		$sql.=' FROM pms_jobs a inner join pms_company b on b.company_id=a.company_id ';	
		...
		$sql.=" order by job_post_date desc limit ".$start.",".$limit;
