<?php 
class Tasksprioritymodel extends CI_Model {
    ...
    {
		$this->table_name='pms_task_priority';
    }
	 function record_count($searchterm) 
	{
	
	$sql = "select count(*)as task_priority_id from ".$this->table_name;
	$cond = '';
	...
	$cond =" task_priority_name like '%" . $searchterm . "%'";
	} 
	} 
	...
	$sql.=" order by task_priority_name ".$sort_by." limit ".$start.",".$limit;
	$query = $this->db->query($sql);
	...
	function update_record()
	{
       ...
       $this->db->where('task_priority_id', $this->input->post('task_priority_id'));
	...
	/*function delete($id=null)
	{
		...
		$query = $this->db->query("select * from ".$this->table_name." where task_priority_id=".$id);
		if ($query->num_rows() > 0)
		{
			$row = $query->row_array();

			//if($this->is_used($id)==true)return $row['task_priority_name'];
			
			
		}		
				
		$this->db->where('task_priority_id', $id);
		$this->db->delete('pms_task_priority'); 
	...
	{
		$master_tables = array(array('table'=>'pms_tasks','key' => 'task_priority_id','Module'=>'Tasks'));
		$is_related = FALSE;
	...
	{
		$this->db->query('task_priority_name', $name);
		if($id!='')	$this->db->where('task_priority_id !=', $id);		
