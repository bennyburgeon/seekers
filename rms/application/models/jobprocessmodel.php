...
class Jobprocessmodel extends CI_Model 
{
	...
	{
		$query = $this->db->query('delete from pms_job_apps where candidate_id='.$candidate_id.' and job_app_id='.$job_app_id.' and job_id='.$job_id);
		return 0;
	}
	...
	function select_candidate($data,$app_id)
	{
		$this->db->query("delete from pms_job_apps_selected where app_id=".$app_id);
		$id=$this->db->insert('pms_job_apps_selected', $data);

		$this->db->query("update pms_job_apps set app_status_id=5 where job_app_id=".$app_id);
		
		$data=array(
		'int_status'          => 1,
		);

		$this->db->where('job_app_id', $app_id);
	   	$this->db->update('pms_job_apps_interviews', $data);
...
//REJECT OFFER
	...
	function issue_offer($data,$app_id)
	{ 
		$this->db->query("delete from pms_job_apps_offerletter where app_id=".$app_id);
		$this->db->query("update pms_job_apps set app_status_id=6 where job_app_id=".$app_id);
...
//CERT ATTESTATTION
	...
	function accept_offer($data,$app_id)
	{
		$this->db->query("delete from pms_job_apps_placement where app_id=".$app_id);
		$this->db->insert('pms_job_apps_placement', $data);
		
		$this->db->query("update pms_job_apps set app_status_id=7 where job_app_id=".$app_id);
	...
	function create_invoice($data,$app_id,$placement_id)
	{
		$this->db->query("delete from pms_job_apps_invoice where placement_id=".$placement_id);
		$this->db->insert('pms_job_apps_invoice', $data);
		
		$this->db->query("update pms_job_apps set app_status_id=12 where job_app_id=".$app_id);
		return 0;
	}

//create visa
	function create_visa($data,$app_id)
	{
		$this->db->query("delete from pms_job_apps_visa where app_id=".$app_id);
...
//create visa doc
	function create_doc($data,$job_app_id)
	{
		$this->db->query("delete from pms_job_apps_document where app_id=".$job_app_id);
		$this->db->insert('pms_job_apps_document', $data);		
		return 0;
	}

	function move_to_docs($job_app_id)
	{
		$data=array(
				'app_status_id'     => 2,
				'app_status_id'        => 1,
		...
		$this->db->where('job_app_id', $job_app_id);
	   	$this->db->update('pms_job_apps', $data);	
		return 0;
	}
	
//create medical
	function create_medical($data,$app_id)
	{
		$this->db->query("delete from pms_job_apps_medical where app_id=".$app_id);
...
//delete ticket FOLLOWUP
	...
	{
		if($job_id=='')return;
		$query = $this->db->query('select DISTINCT a.username,a.first_name,a.lead_source,a.linkedin_url,a.last_name,a.mobile,a.candidate_id,a.cv_file,b.applied_on,b.job_app_id,b.job_id,c.firstname,b.current_ctc,b.notice_period,b.total_experience,b.expected_ctc,b.visa_status,b.gcc_experience,
		(select count(*) from pms_job_apps_calls where candidate_id=a.candidate_id and app_id=b.job_app_id)as total_calls,
		(select call_date from pms_job_apps_calls where candidate_id=a.candidate_id and app_id=b.job_app_id order by app_call_id desc limit 0,1)as last_call_date,
		(select call_notes from pms_job_apps_calls where candidate_id=a.candidate_id and app_id=b.job_app_id  order by app_call_id desc limit 0,1)as last_call_note,
		(select cur_job_status from pms_job_apps_calls where candidate_id=a.candidate_id and app_id=b.job_app_id  order by app_call_id desc limit 0,1)as cur_job_status,
		b.app_status_id from pms_candidate a inner join pms_job_apps b on a.candidate_id=b.candidate_id left join pms_admin_users c on b.admin_id=c.admin_id left join pms_candidate_job_search d on a.candidate_id=d.candidate_id where b.job_id='.$job_id.' and b.app_status_id=1 order by a.first_name asc');
	...
	{
		if($job_id=='')return;
		$query = $this->db->query("select a.*,b.*,c.short_id,c.app_id,c.short_date,d.* from pms_candidate a inner join pms_job_apps b on a.candidate_id=b.candidate_id inner join pms_job_apps_shortlisted c on b.job_app_id=c.app_id inner join pms_jobs d on b.job_id=d.job_id where b.job_id=".$job_id." and c.short_id in (".$short_id.")");
		$dropdowns = $query->result_array();	
		return $dropdowns;
	}
	function update_shortlisted_status($short_id)
	{
		if($job_id=='')return;
	...
	function manage_rejection($job_app_id,$data)
	{

	   	$this->db->insert('pms_job_apps_rejection', $data);
	...
	{
		if($job_id=='')return;
		$query = $this->db->query('select a.*,b.*,c.* from pms_candidate a inner join pms_job_apps b on a.candidate_id=b.candidate_id inner join pms_job_apps_shortlisted c on b.job_app_id=c.app_id where b.app_status_id=1 and b.app_status_id >=3 and b.job_id='.$job_id.' order by a.first_name');
		$dropdowns = $query->result_array();	
		return $dropdowns;
	}			
...
//GET CERT ATTEST
	...
	{
		$id=$this->db->insert('pms_job_apps_cert', $data);
		return $id;
	}
