...
class Candidateallmodel extends CI_Model {
	...
	function record_count($search_name,$search_email,$search_mobile,$cur_employer, $cur_job_status,$lead_source,$job_folder_id,$job_cat_id,$func_id,$desig_id, $skill_id, $level_id,$course_id,$spcl_id, $exp_years_from, $exp_years_to, $salary_from, $salary_to, $marital_status, $visa_type, $gender, $driving_license_country, $nationality, $country_id, $state_id, $city_id, $age_from, $age_to, $lang_id) 
	{
		...
		{
			if($cond!='')
				$cond.=" and a.nationality=". $nationality;
			else
				$cond.=" a.nationality=". $nationality;
		}
		
		if($driving_license_country!='')
		{
			if($cond!='')
				$cond.=" and a.driving_license_country=". $driving_license_country;
			else
				$cond.=" a.driving_license_country=". $driving_license_country;
		}
		
		if($gender!='')
		{
			if($cond!='')
				$cond.=" and a.gender=". $gender;
			else
		...
		{
			if($cond!='')
				$cond.=" and a.marital_status=". $marital_status;
			else
				$cond.=" a.marital_status=". $marital_status;
		}
		
		if($visa_type !='')
		{
			if($cond!='')
				$cond.=" and a.visa_type_id=". $visa_type;
			else
				$cond.=" a.visa_type_id=". $visa_type;
		}			
	 	
		if($cur_employer!='')
		{
			if($cond!='')
				$cond.=" and a.candidate_id in (select candidate_id from pms_candidate_job_profile where organization like '%" .$cur_employer."%')";
			else
				$cond.="  a.candidate_id in (select candidate_id from pms_candidate_job_profile where organization like '%" .$cur_employer."%')";
		}
		
		if($country_id !='' && $state_id =='' && $city_id=='')
		{
			if($cond!='')
				$cond.=" and a.city_id in (select city_id from pms_city where state_id in (select state_id from pms_state where country_id=".$country_id."))";
			else
				$cond.=" a.city_id in (select city_id from pms_city where state_id in (select state_id from pms_state where country_id=".$country_id."))";
		}elseif($country_id !='' && $state_id !='' && $city_id=='')
		{
			if($cond!='')
				$cond.=" and a.city_id in (select city_id from pms_city where state_id=".$state_id.")";
			else
				$cond.=" a.city_id in (select city_id from pms_city where state_id=".$state_id.")";
		}elseif($country_id !='' && $state_id !='' && $city_id!='')
		{
			if($cond!='')
				$cond.=" and a.city_id=".$state_id;
			else
				$cond.=" a.city_id=".$state_id;
		}
		...
		{
			if($cond!='')
				$cond.=" and a.candidate_id in (select candidate_id from pms_candidate_job_search where expected_ctc >='" .$salary_from."') ";
			else
				$cond.=" a.candidate_id in (select candidate_id from pms_candidate_job_search where expected_ctc >='" .$salary_from."') ";
		}if($salary_from=='' && $salary_to!='')
		{
			if($cond!='')
				$cond.=" and a.candidate_id in (select candidate_id from pms_candidate_job_search where expected_ctc <='" .$salary_to."') ";
			else
				$cond.=" a.candidate_id in (select candidate_id from pms_candidate_job_search where expected_ctc <='" .$salary_to."') ";
		}if($salary_from!='' && $salary_to!='')
		{
			if($cond!='')
				$cond.=" and a.candidate_id in (select candidate_id from pms_candidate_job_search where expected_ctc >='" .$salary_from."' and expected_ctc <='".$salary_to."') ";
			else
				$cond.=" a.candidate_id in (select candidate_id from pms_candidate_job_search where expected_ctc >='" .$salary_from."' and expected_ctc <='".$salary_to."') ";
		}

			//level
		if($exp_years_from!='' && $exp_years_to=='')
		{
			if($cond!='')
				$cond.=" and a.candidate_id in (select candidate_id from pms_candidate_job_search where total_experience >='" .$exp_years_from."') ";
			else
				$cond.=" a.candidate_id in (select candidate_id from pms_candidate_job_search where total_experience >='" .$exp_years_from."') ";
		}if($exp_years_from=='' && $exp_years_to!='')
		{
			if($cond!='')
				$cond.=" and a.candidate_id in (select candidate_id from pms_candidate_job_search where total_experience <='" .$exp_years_to."') ";
			else
				$cond.=" a.candidate_id in (select candidate_id from pms_candidate_job_search where total_experience <='" .$exp_years_to."') ";
		}if($exp_years_from!='' && $exp_years_to!='')
		{
			if($cond!='')
				$cond.=" and a.candidate_id in (select candidate_id from pms_candidate_job_search where total_experience >='" .$salary_from."' and total_experience <='".$exp_years_to."') ";
			else
				$cond.=" a.candidate_id in (select candidate_id from pms_candidate_job_search where total_experience >='" .$salary_from."' and total_experience <='".$exp_years_to."') ";
		}
		...
		{
			if($cond!='')
				$cond.=" and a.candidate_id in (select candidate_id from pms_candidate_to_skills where skill_id=" .$skill_id.") ";
			else
				$cond.=" a.candidate_id in (select candidate_id from pms_candidate_to_skills where skill_id=" .$skill_id.") ";
		}				
		...
		{
			if($cond!='')
				$cond.=" and (TIMESTAMPDIFF(YEAR, date_of_birth, CURDATE())) >= " .$age_from;
			else
				$cond.=" (TIMESTAMPDIFF(YEAR, date_of_birth, CURDATE())) >= " .$age_from;
		}		
		...
		{
			if($cond!='')
				$cond.=" and (TIMESTAMPDIFF(YEAR, date_of_birth, CURDATE())) <= " .$age_to;
			else
				$cond.=" (TIMESTAMPDIFF(YEAR, date_of_birth, CURDATE())) <= " .$age_to;
		}		
		...
		{
			if($cond!='')
				$cond.=" and a.candidate_id in (select candidate_id from pms_cand_lang where lang_id=" .$lang_id.") ";
			else
				$cond.=" a.candidate_id in (select candidate_id from pms_cand_lang where lang_id=" .$lang_id.") ";
		}
	...
	function get_list($start, $limit, $sort_by, $sort_param, $search_email, $search_name, $search_mobile, $cur_employer, $cur_job_status,$lead_source,$job_folder_id,$job_cat_id,$func_id,$desig_id, $skill_id, $level_id,$course_id,$spcl_id,$exp_years_from,$exp_years_to,$salary_from, $salary_to, $marital_status, $visa_type, $gender, $driving_license_country, $nationality, $country_id, $state_id, $city_id, $age_from, $age_to, $lang_id)
	{
		$sql="SELECT a.candidate_id, a.cur_job_status , a.first_name, a.last_name, a.username,a.reg_date,a.lead_opportunity, a.mobile, a.mobile_prefix, a.photo,a.status_updated_on,";
		$sql.="(SELECT iy.job_cat_name FROM `pms_candidate_job_profile` ix inner join pms_job_category iy on ix.job_cat_id=iy.job_cat_id where ix.candidate_id=a.candidate_id order by ix.from_date desc limit 0,1 ) as industry_name,  ";	

	
	$sql.='(select count(*) from pms_job_apps_calls where candidate_id=a.candidate_id)as total_calls,';
	$sql.='(select call_date from pms_job_apps_calls where candidate_id=a.candidate_id order by app_call_id desc limit 0,1)as last_call_date,';
	$sql.='(select call_notes from pms_job_apps_calls where candidate_id=a.candidate_id order by app_call_id desc limit 0,1)as last_call_note,';
	
	$sql.=" (select concat(feedback_general,feedback_language,feedback_education,feedback_salary,feedback_skills,feedback_domain,feedback_industry,feedback_projects,feedback_social,feedback_concepts,feedback_activities,feedback_vision,feedback_job_change,feedback_attitude,feedback_personality,feedback_interaction,feedback_team_work,feedback_corporate_exposure) as consultant_feedback from pms_candidate_job_search where candidate_id=a.candidate_id limit 0,1 ) as consultant_feedback, ";
			
		
		$sql.="(SELECT fy.func_area FROM `pms_candidate_job_profile` fx inner join pms_job_functional_area fy on fx.func_id=fy.func_id where fx.candidate_id=a.candidate_id order by fx.from_date desc limit 0,1 ) as func_area,  ";
		
		$sql.="(SELECT dx.desig_name FROM `pms_candidate_job_profile` mx inner join pms_designation dx on mx.desig_id=dx.desig_id where mx.candidate_id=a.candidate_id order by mx.from_date desc limit 0,1 ) as designation,  ";

		
			//$sql.="(SELECT dx.designation FROM `pms_candidate_job_profile` dx  where dx.candidate_id=a.candidate_id order by dx.from_date desc limit 0,1 ) as designation,  ";
		
		
	$sql.=" cty.city_name as location  ";
		
		
		$sql.=" FROM ".$this->table_name." a left join pms_city cty on a.city_id=cty.city_id ";
		$cond='';
...
//designation		
		...
		$sql.=" order by ".$sort_param." ".$sort_by." limit ".$start.",".$limit;		
	...
	function cur_job_status_list()
	{
		...
		$finalDropDown=array();
		$dropDownList=array('' => 'Select Job Status');
	...
	function get_notes_list($candidate_id)
	{
		$query = $this->db->query('SELECT a.*,b.firstname, cjs.cur_job_status_name  from pms_job_apps_calls a left join pms_admin_users b on a.admin_id=b.admin_id left join pms_cur_job_status cjs on a.cur_job_status=cjs.cur_job_status  where a.candidate_id='.$candidate_id.' order by a.app_call_id desc');
		if($query->num_rows()>0)
		{
...
function select_aplication_visa($candidate_id)
    ...
    {
        $query=$this->db->query("select a.*,b.job_cat_name,c.*,d.* from pms_candidate_job_profile a left join pms_job_category b on a.job_cat_id=b.job_cat_id left join pms_job_functional_area c on a.func_id=c.func_id left join pms_designation d on a.desig_id=d.desig_id where a.candidate_id=".$candidate_id." order by a.present_job asc, a.from_date asc");
		return $query->result_array();
	}	
...
$this->db->query("update pms_candidate_files set file_name='no_photo.png',file_type='no_photo.png' where file_name='".$row['photo']."' and candidate_id=".$id);
	...
	{
		return array('' => 'Gender', '1' => 'Male', '0' => 'Female');	
	}
	
	function marital_status_list()
	{
		return array('' => 'Marital Status', '1' => 'Married', '0' => 'Single');	
	}
	
	function driving_license_country_list()
	{
		$query = $this->db->query('select distinct country_id, country_name from pms_country where country_id in (select distinct driving_license_country from pms_candidate) order by country_name asc');
		$dropdowns = $query->result();
		$dropDownList['']='Drv. Lic. Issued.';
		foreach($dropdowns as $dropdown)
		{
			 $dropDownList[$dropdown->country_id] = $dropdown->country_name;
		}
	...
	function exp_years_to()
	{
	 ...
	 if($this->input->post('date_of_birth')!='')$age = $this->get_age($this->input->post('date_of_birth'));
		...
		$this->db->insert('pms_candidate', $data);
        $id = $this->db->insert_id();
		if($this->input->post('cur_ctc')!='' || $this->input->post('expected_ctc')!='' || $this->input->post('notice_period')!='' || $this->input->post('total_exp')!='')
			{
...
//delte andd insert languages 
    ...
    function update_job_vacancy($candidate_id, $job_id)
	{
		if($job_id > 0)
		{
			$this->add_to_job($candidate_id, $job_id);
		}		
		return $job_id;		
	}
    ...
    function get_single_profile($candidateId)
	{
        ...
        $query=$this->db->query("select a.*,b.visa_type,(select country_name from pms_country nt where nt.country_id=a.nationality)as nationality_name,(select country_name from pms_country dl where dl.country_id=a.driving_license_country)as driving_license_country_name,(select country_name from pms_country pl where pl.country_id=a.passport_nationality)as passport_nationality_name from ".$this->table_name." a left join pms_job_visa_type b on a.visa_type_id=b.visa_type_id where a.candidate_id=".$candidateId);
		return $query->row_array();
...
//EDIT PASSPORT NUMBER AND TYPE	
	...
	function get_single_record($candidateId){
		...
		$query=$this->db->query("select a.*, cj.cur_job_status_name, b.visa_type,(select country_name from pms_country nt where nt.country_id=a.nationality)as nationality_name,(select city_name from pms_city cl where cl.city_id=a.city_id)as current_location_name,(select country_name from pms_country dl where dl.country_id=a.driving_license_country)as driving_license_country_name,(select country_name from pms_country pl where pl.country_id=a.passport_nationality)as passport_nationality_name from ".$this->table_name." a left join pms_job_visa_type b on a.visa_type_id=b.visa_type_id left join pms_cur_job_status cj on a.cur_job_status=cj.cur_job_status where a.candidate_id=".$candidateId);
		return $query->row_array();
	}
...
//GET ALL LANGUAGES
   function get_language_candidate()
   {
		$query = $this->db->query('SELECT lang_id, lang_name FROM pms_languages  ORDER BY lang_name');
		$dropDownList=array();
		$result = $query->result();
		...
		{
			$dropDownList[$dropdown->lang_id] = $dropdown->lang_name;
		}
		return $dropDownList;
   }
...
//GET JOB SEARCH DETAILS
   ...
   function get_all_skills($desig_id)
   {
		$query = $this->db->query('SELECT a.skill_id, a.skill_name FROM pms_candidate_skills a  ORDER BY a.skill_name ASC');
		
		$dropdowns = $query->result();
		$skill_result=array('' => 'Select Skill');
		foreach($dropdowns as $dropdown)
		{
			 $skill_result[$dropdown->skill_id] = $dropdown->skill_name;
		}
...
//GET ALL PARENT SKILL
   ...
   {
	  $query=$this->db->query("select * from pms_candidate_skills order by skill_name asc");
	  return $query->result_array();
	}
...
//function for category and functional role in summary	//INSERT functional  area and get
	...
	{ 
		...
		{ 
			$this->db->query("delete from pms_candidate_to_industry where candidate_id=".$candidate_id);
		...
		{ 
			$this->db->query("delete from pms_candidate_to_funcional where candidate_id=".$candidate_id);
		...
		{ 
			$this->db->query("delete from pms_candidate_to_designation where candidate_id=".$candidate_id);
	...
	function get_cat_list($candidate_id)
	{
		 $query=$this->db->query("SELECT a.job_cat_id, a.job_cat_name FROM  pms_job_category a inner join pms_candidate_to_industry b on a.job_cat_id=b.job_cat_id  where b.candidate_id=".$candidate_id." order by a.job_cat_name asc");
		return $query->result_array();
	}

	function get_function_list($candidate_id)
	{
		 $query=$this->db->query("SELECT a.func_id, a.func_area FROM  pms_job_functional_area a inner join pms_candidate_to_funcional b on a.func_id=b.func_id  where b.candidate_id=".$candidate_id." order by a.func_area asc");
		return $query->result_array();
	}

	function get_designation_list($candidate_id)
	{
		 $query=$this->db->query("SELECT a.desig_id, a.desig_name FROM  pms_designation a inner join pms_candidate_designation b on a.desig_id=b.desig_id  where b.candidate_id=".$candidate_id." order by a.desig_name asc");
		return $query->result_array();
	}
