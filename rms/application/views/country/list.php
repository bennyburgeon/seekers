...
<div class="alert alert-success alert-dismissable">
<button aria-hidden="true" data-dismiss="alert" class="close" type="button"></button>
<strong>Error!! Country name exists in related modules.</strong>
</div>
...
<form name="form1" method="post" id="form1" action="<?php  echo $this->config->site_url();?>/country/multidelete?rows=<?php echo $rows;?>" >

<div class="right-btns">
<a href="<?php echo base_url();?>index.php/country/add" class="attach-subs tools"><img src="<?php echo base_url('assets/images/plus.png');?>">Add New</a>
...
<table class="tool-table">
    <tbody>
        <form id="searchForm">
            <tr>
                <td><input class="form-control" type="text" name="searchterm" id="search_term" placeholder="Search Your Country" style="width: 185px;"></td>
...
<table class="tool-table new">
    <thead>
        <tr>
            ...
            <th>
            <a href="<?php echo $this->config->site_url();?>/country?sort_by=<?php if($sort_by=='asc'){echo 'desc';}else{echo 'asc';}?>&searchterm=<?php echo $searchterm;?>&rows=<?php echo $rows;?>">Country Name</a>
    ...
    ?>
        <tr>
            <td align="center"><input type="checkbox" name="checkbox[]"  class="checkboxes" value="<?php echo $result['country_id']?>" ></td>
            <td><?php echo $result['country_name']?></td>
            <td>
            <?php if($result['status']==1){?>
            <span class="label label-success">
            <a href="<?php echo $this->config->site_url();?>/country/changestat/<?php echo $result['country_id']?>?stat=0">Active</a></span>
            <?php } ?>
            <?php if($result['status']==0){?>
            <span class="label label-danger">
            <a href="<?php echo $this->config->site_url();?>/country/changestat/<?php echo $result['country_id']?>?stat=1">Disabled</a></span>
            <?php } ?>
            </td>
            <td>
            <a href="<?php echo base_url();?>index.php/country/edit/<?php echo $result['country_id']?>" class="views" 
            title="Edit"><img src="<?php echo base_url('assets/images/edits.png');?>"></a>
            <a href="<?php echo base_url();?>index.php/country/delete/<?php echo $result['country_id']?>" class="views" 
            title="Delete" onclick="return confirm('Are you sure you want to delete?')"><img src="<?php echo base_url('assets/images/deletes.png');?>"></a>
...
$(document).ready(function()
{
	...
	$("#search").click(function(){
		...
		window.location.href = '<?php echo $this->config->site_url();?>/country?searchterm='+searchterm;
	});
	$("#sel_limit1").change(function(){
		var limits = $(this).find(":selected").val();
		window.location.href = '<?php echo $this->config->site_url();?>/country?limit='+limits;
	});
