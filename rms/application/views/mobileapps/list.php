...
<form name="form1" method="post" id="form1" action="<?php  echo $this->config->site_url();?>/mobileapps/multidelete?rows=<?php echo $rows;?>" >


<div class="right-btns">
<a href="<?php echo base_url();?>index.php/mobileapps/add" class="attach-subs tools"><img src="<?php echo base_url('assets/images/plus.png');?>">Add New</a>
...
<tr class="odd gradeX">
	...
	<div class="checker">
    	<span>
    		<input type="checkbox" name="checkbox[]" class="checkboxes" value="<?php echo $result['obj_id']?>" >
        </span>
	</div>
</td>
								<td>
									 <?php echo $result["obj_title"]; ?>
								</td>
								<td>
									
									<?php echo $result["obj_contact_name"]; ?> 
								</td>
								<td><?php echo $result["obj_contact_phone"]; ?> </td>
								<td><?php echo $result["obj_contact_email"]; ?></td>
								<td>
                                   
                                    <?php if($result['obj_status']=='1'){?>
                                    <a href="<?php  echo $this->config->site_url();?>/mobileapps/changestat/<?php echo $result['obj_id']?>?obj_status=0"> <span class="label label-sm label-success">Active</span></a>
                                     <?php }elseif($result['obj_status']=='0'){?>
									<a href="<?php  echo $this->config->site_url();?>/mobileapps/changestat/<?php echo $result['obj_id']?>?obj_status=1" ><span class="label label-sm label-danger">Blocked </span></a>
                                     <?php } ?>
			...
			<a href="<?php echo base_url();?>index.php/mobileapps/edit/<?php echo $result['obj_id']?>" class="views" title="Edit"><img src="<?php echo base_url('assets/images/edits.png');?>"></a>
...
$(document).ready(function()
{
	...
	$("#search").click(function(){
		...
		window.location.href = '<?php echo $this->config->site_url();?>/mobileapps?searchterm='+searchterm;
	});
	$("#sel_limit1").change(function(){
		var limits = $(this).find(":selected").val();
		window.location.href = '<?php echo $this->config->site_url();?>/mobileapps?limit='+limits;
	});
