...
<input type="button" class="attach-subs" value="Save & Continue" id="save_candidate3" style="width: 153px;">
...
$( document ).ready(function() {
   ...
   $('#save_candidate3').click(function(){
		...
		if(isContactValid) {
			...
			$.ajax({
				type: "post",
				url: "<?php echo site_url('register/addEducationDetail'); ?>"+'/'+candidateId,
				cache: false,				
				data: dataStringprop,
				success: function(json){
					try{		
						...
						if(ret['STATUS']==1) {
                            ...
                            var site_url = "<?php echo site_url('register/loadJobhtml'); ?>" +'/'+ candidateId;
...
var dataStringprop = $("#candidate_form3").serialize();
	$.ajax({
				type: "post",
				url: "<?php echo site_url('register/skip_step4'); ?>"+'/'+candidateId,
				cache: false,				
				data: dataStringprop,
				success: function(json){
					try{		
						...
						if(ret['STATUS']==1) {
                            ...
                            var site_url = "<?php echo site_url('register/loadJobhtml'); ?>" +'/'+ id;
                            $("#step1").load(site_url, function() {
                                //alert("success step2");
                            });
						}
