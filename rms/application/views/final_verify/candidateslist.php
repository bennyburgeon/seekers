...
<!-- 
<div class="actions">
<a href="<?php echo site_url('final_verify/import_csv');?>" class="btn btn-default btn-circle">
...
-->
<br>
<form id="searchForm" method="get" action="<?php echo $this->config->site_url()?>/final_verify/">
    ...
    <table class="tool-table new">
        <thead>
            <tr role="row" class="heading">
                ...
                <th><a href="<?php echo $this->config->site_url()?>/final_verify?sort_by=<?php if($sort_by=='asc'){echo 'desc';}else{echo 'asc';};?>&limit=<?php echo $limit;?>&search_name=<?php echo $search_name;?>&search_email=<?php echo $search_email;?>&search_mobile=<?php echo $search_mobile;?>&rows=<?php echo $rows;?>">Candidate Name</a></th>
                
                <th><a href="<?php echo $this->config->site_url()?>/final_verify?sort_by=<?php if($sort_by=='asc'){echo 'desc';}else{echo 'asc';};?>&limit=<?php echo $limit;?>&search_name=<?php echo $search_name;?>&search_email=<?php echo $search_email;?>&search_mobile=<?php echo $search_mobile;?>&rows=<?php echo $rows;?>">Email</a></th>
                
                <th>Reg Date</th>
                <th><a href="<?php echo $this->config->site_url()?>/final_verify?sort_by=<?php if($sort_by=='asc'){echo 'desc';}else{echo 'asc';};?>&limit=<?php echo $limit;?>&search_name=<?php echo $search_name;?>&search_email=<?php echo $search_email;?>&search_mobile=<?php echo $search_mobile;?>&rows=<?php echo $rows;?>">Mobile</a></th>
        ...
        <tbody>
			...
			<?php if($records!=NULL){ $i=0; foreach($records as $result){ $i+=1; ?>            
            	<tr class="odd gradeX">
                    ...
                    <a href="<?php echo base_url();?>index.php/final_verify/summary/<?php echo $result['candidate_id']?>" class="views" title="View">Manage</a>&nbsp;|&nbsp;
                    <a href="<?php echo base_url();?>index.php/final_verify/final_verification/<?php echo $result['candidate_id']?>" class="views" title="Final Verification">Final Verification</a>
...
$(document).ready(function()
{
	...
	$("#sel_limit1").change(function(){
		...
		window.location.href = '<?php echo $this->config->site_url();?>/final_verify?limit='+limits+'&search_name='+search_name+'&search_email='+search_email+'&search_mobile='+search_mobile+'&reg_status='+reg_status;
	});
	 ...
	 {  // triggred submit
		...
		if(count_checked >0) {
			if($('#admin_id').val() == 0){
				...
				$.ajax({
					type:"POST",
					url: "<?php echo $this->config->site_url();?>/final_verify/assignAdmin",
					data:{ 
					...
					success: function(msg) {
						...
						window.location='<?php echo $this->config->site_url();?>/final_verify';
						}
						else{
	...
	$("#btn_change_status").click(function()
	 {  // triggred submit
		...
		$.ajax({
			type:"POST",
			url: "<?php echo $this->config->site_url();?>/final_verify/change_status",
			data:{ 
					'selectedArr' : selectedArr,
