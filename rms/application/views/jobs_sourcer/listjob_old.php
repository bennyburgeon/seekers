  ...
  <div class="row">
    <div class="col-sm-12">
      ...
      <div class="table-tech specs">
        <div class="right-btns"> <a href="<?php echo base_url();?>index.php/jobs_sourcer/add" class="attach-subs tools"><img src="<?php echo base_url('assets/images/plus.png');?>">Add New</a> 
          <!-- 
<a href="#" id="deleteall" class="attach-subs tools"><img src="<?php echo base_url('assets/images/deletes_new.png');?>">Delete</a>

-->: </div>
        <form id="searchForm" method="post" action="<?php  echo $this->config->site_url();?>/jobs_sourcer/">
          <table class="tool-table">
            <tbody>
              <tr>
                ...
                <td width="56"><input type="submit" value="Search"></td>
              </tr>
            </tbody>
          </table>
        </form>
        ...
        <form name="form1" method="post" id="form1" action="<?php  echo $this->config->site_url();?>/jobs_sourcer/multidelete?rows=<?php echo $rows;?>" >
          <table class="tool-table new">
            <thead>
              <tr role="row" class="heading">
                <th width="100"><div class="checker"><span>Job ID<!-- <input type="checkbox" class="group-checkable" id="selectall">--></span></div></th>
                <th width="585"><a href="<?php echo $this->config->site_url()?>/jobs_sourcer?sort_by=<?php if($sort_by=='asc'){echo 'desc';}else{echo 'asc';};?>&searchterm=<?php echo $searchterm;?>
        &rows=<?php echo $rows;?>">Job Title</a></th>
                <th width="120">Posted On</th>
		...
		foreach($records as $result){ ?>
              <tr class="odd gradeX">
                <td><div class="checker"> <span> 
                    ...
                    SEEK-<?php echo $result['job_id']?> </span> </div></td>
                <td><a href="<?php echo base_url();?>index.php/jobs_sourcer/manage/<?php echo $result['job_id']?>" class="views" title="Manage Job"><?php echo $result['job_title']?></a><br>
                  <?php //echo $result['company_name']?>  Openings =<a href="#" class="btn btn-info btn-xs"><?php echo $result['vacancies']?></a> | Total Apps=<a href="#" class="btn btn-primary btn-xs"><?php echo $result['total_apps']?></a> || Rejection= <a href="#" class="btn btn-warning btn-xs"><?php echo $result['total_rejected']?></a></td>
                <td><?php echo $result['job_post_date']?></td>
                <td><?php echo $result['job_expiry_date']?></td>
                
                <!--<a href="<?php //echo base_url();?>index.php/jobs_sourcer/edit/<?php //echo $result['job_id']?>" class="views" title="Edit">Edit</a> -->
                  <!-- 
             <a href="<?php echo base_url();?>index.php/jobs_sourcer/delete/<?php echo $result['job_id']?>" class="views" title="Delete" onclick="return false;confirm('Are you sure you want to delete?');"><img src="<?php echo base_url('assets/images/deletes.png');?>"></a> --> 
                  <td><a href="<?php echo base_url();?>index.php/jobs_sourcer/manage/<?php echo $result['job_id']?>" class="views" title="Manage Job">Manage</a> </td>
              </tr>
              <?php
	}}else{?>
              <tr>
      ...
      <div class="modal-header">
        ...
        <div class="modal-body">
          <div class="col-md-15" id="assignment_history">
            <form class="form-horizontal form-bordered"  method="post" action="<?php echo $this->config->site_url();?>/jobs_sourcer/save_assignment" id="assignment_form" name="assignment_form">
              ...
              <table class="hori-form">
                <tbody>
                  ...
                  data-url="<?php echo $this->config->site_url();?>/jobs_sourcer/save_assignment" />
                      </span></td>
                  </tr>
                </tbody>
              </table>
...
$(document).ready(function()
{
	...
	$("#search").click(function(){
		...
		window.location.href = '<?php echo $this->config->site_url();?>/jobs_sourcer?searchterm='+searchterm;
	});
	$("#sel_limit1").change(function(){
		var limits = $(this).find(":selected").val();
		window.location.href = '<?php echo $this->config->site_url();?>/jobs_sourcer?limit='+limits;
	});
...
function assign_requirement(job_id)
{
	...
	$('#data_holder').html('Loading..................');	
	 $.ajax({			
			type: 'POST',
			url:"<?php echo base_url(); ?>index.php/jobs_sourcer/assign_requirement/?job_id="+job_id,
