...
<div class="table-tech specs hor">
  ...
  <form class="form-horizontal form-bordered"  method="post" id="candidate_form5" name="candidate_form5" action="<?php echo $this->config->site_url();?>/candidates/editfiles" enctype="multipart/form-data"> 
...
{ 
	$.ajax({
	   ...
	   url: '<?php echo base_url(); ?>index.php/candidates/show_cv_photo/',
...
//BEGIN FILE DETAILS
	   ...
	   $('#candidate_form5').submit(function(evt){
		...
		//if(isContactValid) {
			...
			$.ajax({
				type: "post",
				url: "<?php echo $this->config->site_url();?>/candidates/editfiles",
                data:formData,
                cache:false,
                contentType: false,
                processData: false,
 ...
 //onchange of job_category
	...
	$('#job_cat_id').change(function() 
	{
		...
		if($('#job_cat_id').val()=='')return;
			$.ajax({
			  type: 'POST',
			  url: '<?php echo $this->config->site_url();?>/candidates/getfunction/',
