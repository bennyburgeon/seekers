...
<tbody>
      ...
      <form action="<?php echo $this->config->site_url();?>/my_company/update?<?php echo $query_string;?>" class="form-horizontal form-bordered"  method="post" id="frmentry" name="frmentry" onSubmit="return validate();" enctype="multipart/form-data"> 
    ...
    <td colspan="2"> <input type="text" id="company_name" name="company_name" value="<?php echo $formdata['company_name'];?>" placeholder=" Enter Company Name" class="form-control" /></td>
        ...
        <tr>
    <td>Attach Logo</td>
    <td> <?php echo form_upload(array('name'=>'company_logo','class'=>'smallinput', ''));?>
    </td>
    </tr>
...
<tr>
    ...
    <a href="<?php echo $this->config->site_url();?>/my_company?<?php echo $query_string;?>" class="attach-subs subs">Cancel</a>
    </span>
    </td>
    </tr>
    </form>
...
{	
	if($('#company_name').val()=='')
	{
		alert('Please enter my_company name');
		$('#company_name').focus();
		return false;
	}
/*	
	if($('#type_id').val()=='0')
	{
		alert('Please select my_company type');
		$('#type_id').focus();
		return false;
	}	
