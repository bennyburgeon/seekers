...
<div class="table-tech specs hor">
  ...
  <form class="form-horizontal form-bordered"  method="post" id="" name=""   action="<?php echo site_url('candidates/process_completed'); ?>/<?php echo $candidate_id ?>" > 
  <input type="hidden" name="candidate_id" value="<?php echo $candidate_id ?>">
...
//BEGIN category and functional area
   $('#edit_category').click(function(){ 
		...
		var candidateId = '<?php echo $candidate_id ?>';
			$.ajax({
				type: "post",
				url: "<?php echo site_url('candidates/editCategory'); ?>"+'/'+candidateId,
				cache: false,				
				data: dataStringprop,
...
//BEGIN SKILLS AND CERTIFICATE
   $('#edit_skills').click(function(){ 
		...
		var candidateId = '<?php echo $candidate_id ?>';
			$.ajax({
				type: "post",
				url: "<?php echo site_url('candidates/editSkills'); ?>"+'/'+candidateId,
				cache: false,				
				data: dataStringprop,
				success: function(json){ 
					try{		
...
$('#update_lang').click(function(){ 
		...
		var candidateId = '<?php echo $candidate_id ?>';
			$.ajax({
				type: "post",
				url: "<?php echo site_url('candidates/add_lang_details'); ?>"+'/'+candidateId,
				cache: false,				
				data: dataStringprop,
				success: function(json){
					try{		
 ...
 //onchange of job_category
	...
	$('#multiple_category').change(function() 
	{
		...
		if($('#multiple_category').val()=='')return;
			$.ajax({
			  type: 'POST',
			  url: '<?php echo $this->config->site_url();?>/candidates/getfunction_multiple/',
