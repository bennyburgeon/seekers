...
<table width="100%" border="1">
  <tbody>
    ...
    <tr>
      <td  width="60%" align="left" valign="top"><form class="form-horizontal form-bordered"  method="post" action="<?php echo $this->config->site_url();?>candidates_own/update_job_vacancy" id="job_assignment_form" name="assignment_form">
...
?>
          ...
          <?php } ?>
          &nbsp;<?php echo $val['job_title'];?> ( <?php echo "<span style='color:#3396d8;'>".$val['company_name']."</span>" ?> ) <br>
          <?php } ?>
          <br>
          <input type="button" value="Add To These Jobs" data-url="<?php echo $this->config->site_url();?>candidates_own/update_job_vacancy" id="save_job_app_button" >
        </form></td>
      <td align="left" valign="top"  width="40%"> Full Name:
        ...
        <br>
        Registered On:
        <?php  echo ($candidate_profile['reg_date']!='0000-00-00' && $candidate_profile['reg_date']!='') ? date('d-m-Y', strtotime($candidate_profile['reg_date'])) : '';?>
        <br>
        Profile Updated On:
        <?php  echo ($candidate_profile['status_updated_on']!='0000-00-00' && $candidate_profile['status_updated_on']!='') ? date('d-m-Y', strtotime($candidate_profile['status_updated_on'])) : '';?>
        <br>
        <br>
      ...
      ?>
        CTC Updated on:
        <?php  echo ($job_search['ctc_updated_on']!='0000-00-00' && $job_search['ctc_updated_on']!='') ? date('d-m-Y', strtotime($job_search['ctc_updated_on'])) : '';?>
        <br>
        Total Experience:
