<form class="form-horizontal form-bordered" action="<?php echo site_url('candidates_own/update_job_details'); ?>"  method="post" id="update_job_form" name="update_job_form" >
  ...
  <table class="hori-form">
    <tbody>
      ...
      <tr>
        <td>Job Location</td>
        <td><input class="form-control hori " type="text" name="job_location" value="<?php echo $formdata['job_location'];?>" id="job_location"></td>
      </tr>
      <tr>
        <td>Industry</td>
        <td><?php echo form_dropdown('job_cat_id',  $industries_list, $formdata['job_cat_id'],'class="form-control" id=""');?></td>
      </tr>
      <tr>
        <td>Department</td>
        <td><?php echo form_dropdown('func_id',  $functional_list, $formdata['func_id'],'class="form-control" id=""');?></td>
      </tr>
      <tr>
        <td>Designation</td>
        <td><?php echo form_dropdown('desig_id',  $designation_list, $formdata['desig_id'],'class="form-control" id=""');?></td>
      </tr>
         <tr>
                      <td>Technologies Used</td>
                      <td><input class="form-control hori" type="text" name="technology_used" value="<?php echo $formdata['technology_used'];?>"  id="technology_used"></td>
                    </tr>
      <tr>
        <td>Responsibilities</td>
        <td><textarea class="form-control hori" name="responsibility" id="responsibility"><?php echo $formdata['responsibility'];?></textarea></td>
      </tr>
    ...
    <tr>
      ...
      <td><label class="radio-inline">
          <input type="radio" name="present_job" id="present_job" value="1" <?php if($formdata['present_job']==1){?> checked <?php } ?> >
          Yes</label>
        <label class="radio-inline">
          <input type="radio" name="present_job" id="present_job" value="0" <?php if($formdata['present_job']==0){?> checked <?php } ?> >
          No</label></td>
    </tr>
    <tr>
      <td colspan="2"><span class="click-icons">
       ...
       <!-- <input type="button" class="attach-subs" onClick="window.location='<?php echo site_url('candidates_own/summary'); ?>/'" value="Cancel" id="cancel_update_job_profile" style="width:180px;">-->
        </span></td>
    </tr>
    </tbody>
...
$('#job_cat_id_prof_edit').change(function() {
	...
	if($('#job_cat_id_prof_edit').val()=='')return;
		...
		$.ajax({
		  type: 'POST',
		  url: '<?php echo $this->config->site_url();?>candidates_own/getdepartment/',
...
$('#func_id_prof_edit').change(function() {
	...
	if($('#func_id_prof_edit').val()=='')return;
		...
		$.ajax({
		  type: 'POST',
		  url: '<?php echo $this->config->site_url();?>candidates_own/getdesignation/',
