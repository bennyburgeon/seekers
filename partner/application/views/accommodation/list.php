...
<form name="form1" method="post" id="form1" action="<?php  echo $this->config->site_url();?>/accommodation/multidelete?rows=<?php echo $rows;?>" >


<div class="right-btns">
<a href="<?php echo base_url();?>index.php/accommodation/add" class="attach-subs tools"><img src="<?php echo base_url('assets/images/plus.png');?>">Add New</a>
...
<tr class="odd gradeX">
	...
	<div class="checker">
    	<span>
    		<input type="checkbox" name="checkbox[]" class="checkboxes" value="<?php echo $result['co_work_id']?>" >
        </span>
	</div>
</td>
								<td>
									 <?php echo $result["co_work_title"]; ?>
								</td>
								<td><?php echo $result["address"]; ?></td>
								<td><?php if($result["vacancy_status"]==1)echo 'Yes';else echo 'No';?></td>                                 
								<td><?php echo $result["monthly_rent"]; ?></td>								
								<td><?php echo $result["contact_name"]; ?></td>
								<td>                    
                                    <?php if($result['list_status']=='1'){?>
                                    <a href="<?php  echo $this->config->site_url();?>/accommodation/changestat/<?php echo $result['co_work_id']?>?list_status=0"> <span class="label label-sm label-success">Active</span></a>
                                     <?php }elseif($result['list_status']=='0'){?>
									<a href="<?php  echo $this->config->site_url();?>/accommodation/changestat/<?php echo $result['co_work_id']?>?list_status=1" ><span class="label label-sm label-danger">Blocked </span></a>
                                     <?php } ?>
			...
			<a href="<?php echo base_url();?>index.php/accommodation/edit/<?php echo $result['co_work_id']?>" class="views" title="Edit"><img src="<?php echo base_url('assets/images/edits.png');?>"></a>
...
$(document).ready(function()
{
	...
	$("#search").click(function(){
		...
		window.location.href = '<?php echo $this->config->site_url();?>/accommodation?searchterm='+searchterm;
	});
	$("#sel_limit1").change(function(){
		var limits = $(this).find(":selected").val();
		window.location.href = '<?php echo $this->config->site_url();?>/accommodation?limit='+limits;
	});
