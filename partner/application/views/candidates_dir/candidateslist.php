  ...
  <div class="row">
    <div class="col-sm-12">
      ...
      <div class="table-tech specs">
        ...
        <a href="<?php echo base_url();?>candidates_dir/add" class="attach-subs tools"><img src="<?php echo base_url('assets/images/plus.png');?>">Add New</a> || <a href="<?php echo base_url(); ?>candidates_dir" id="clear_search" class="btn btn-primary btn-xs">Clear Search Filters</a> </div>
            ...
            <tbody>
              <tr>
                ...
                <td colspan="2">Payment Status:
                  <input type="radio" name="payment_status" value="1" <?php if($payment_status==1) echo 'checked'; ?>>Paid
                  <input type="radio" name="payment_status" value="2" <?php if($payment_status==2) echo 'checked'; ?>>Unpaid </td>
              </tr>
              <tr>
    ...
    ?>
              <tr class="odd gradeX">
                ...
                <td><?php if($result['photo']!=''){?>
                  ...
                  <?php } ?></td>
                <td><a href="<?php echo base_url();?>candidates_dir/summary/<?php echo $result['candidate_id']?>" target="_blank" class="views" title="View"> <strong><?php echo $result['first_name']?></strong></a> || <?php echo $result['username'];?> || <strong><?php echo $result['mobile'];?></strong> 
           ...
           --> 
                  ...
                  <a href="javascript:;" title="Update & View Note" onclick="add_candidate_notes(<?php echo $result['candidate_id'];?>);"  data-url="<?php echo base_url(); ?>index.php/jobs/get_all_notes/?candidate_id=<?php echo $result['candidate_id'];?>"  id="add_candidate_notes" class="btn btn-<?php if($result['total_notes']>0)echo 'info';else echo 'danger';?> btn-xs"> Add Notes</a>
                   ...
                   || <a href="javascript:;" title="Shortlist Candidates, see the list of jobs" onclick="connect_with_jobs(<?php echo $result['candidate_id'];?>);"  id="manage_shortlists" class="btn btn-info btn-xs">Shortlist</a>
                    ...
                    || <a href="javascript:;" title="Update Consultant's Feedback" onclick="add_consultant_feedback(<?php echo $result['candidate_id'];?>);"  data-url="<?php echo base_url(); ?>jobs/add_consultant_feedback/?candidate_id=<?php echo $result['candidate_id'];?>"  id="add_consultant_feedback" class="btn btn-<?php if($result['consultant_feedback']!='')echo 'info';else echo 'danger';?> btn-xs"> Consultant's Feedback</a>&nbsp;&nbsp;</span>
                  </td>
...
function add_candidate_notes(candidate_id){
	...
	$('#show_all_notes').html('');	
	 $.ajax({			
			type: 'POST',
			url:"<?php echo base_url(); ?>candidates_dir/get_all_notes/"+candidate_id,
			method: "POST",
  			data: { candidate_id : candidate_id},
		    dataType: "html",
			success: function(data) 
...
function open_client_cv_doc(candidate_id)
{
	$('#show_client_cv_doc').html('');	
	 $.ajax({			
			type: 'POST',
			url:"<?php echo base_url(); ?>candidates_dir/client_cv_doc/?view="+candidate_id,
			method: "POST",
  			data: { candidate_id : candidate_id},
		    dataType: "html",
			success: function(data) 
...
function pre_screening_interview(candidate_id)
{
	...
	$('#pre_screening_test').html('Loading..................');	
	 $.ajax({
			type: 'POST',
			url:"<?php echo $this->config->site_url()?>candidates_dir/pre_screening",
			data: $('#job_assignment_form').serialize(),
			method: "POST",
  			data: { candidate_id : candidate_id,},
		    dataType: "html",
