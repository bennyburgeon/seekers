...
<section class="bot-sep">
  <div class="section-wrap">
    ...
    <div class="alert alert-success alert-dismissable">
      <button aria-hidden="true" data-dismiss="alert" class="close" type="button"></button>
      <strong>Sucess !</strong> &nbsp;&nbsp;Details added successfully. </div>
    <?php } 
    ...
    <div class="row">
      <div class="col-sm-12">
        ...
        <div class="table-tech specs">
          <div class="right-btns"> <a href="<?php echo base_url();?>company/add?<?php echo $query_string;?>" class="attach-subs tools"><img src="<?php echo base_url('assets/images/plus.png');?>">Add New</a> || <a href="<?php echo base_url();?>company/import_csv" class="attach-subs tools"><img src="<?php echo base_url('assets/images/plus.png');?>">Import CSV</a> <a href="<?php echo base_url(); ?>company" id="clear_search" class="btn btn-primary btn-xs">Clear Search Filters</a> </div>
          <form id="searchForm" action="<?php echo base_url(); ?>company" method="get" name="searchForm">
            <table width="47%" class="tool-table">
              <tbody>
                <tr>
                  <td width="17%"><input class="form-control" type="text" name="searchterm" id="search_term" placeholder="Company Name" value="<?php echo $searchterm != '' ? $searchterm: '' ;?>"     style="width: 200px;">
                  <td width="17%"><input class="form-control" type="text" name="search_designation" id="search_designation" placeholder="Designation" value="<?php echo $search_designation != '' ? $search_designation: '' ;?>"     style="width: 200px;"></td>
                  <td width="9%"><?php echo form_dropdown('ind_id', $industry_list , $ind_id,'style="width:200px;" class="form-control"  id="ind_id" ');?></td>
...
-->
                  ...
                  <td width="11%"><a href="#" class="se-reset"><img src="<?php echo base_url('assets/images/search.png');?>" id="search"></a></td>
                </tr>
                <!-- 
<tr>
  ...
  <td><?php echo form_dropdown('user_id', $bde_list , $user_id,'  id="user_id" ');?></td>
            ...
            <table class="tool-table new">
              <thead>
                <tr role="row" class="heading">
                  <th width="24"><div class="checker"><span>
                  ...
                  <th width="408">Contact Details</th>
                  <th width="138">Action</th>
                </tr>
              </thead>
              <tbody>
                <?php 		
if($records!=NULL)
{
	foreach($records as $result){ ?>
                <tr class="odd gradeX">
                  ...
                  <td><a href="<?php echo base_url();?>company/edit/<?php echo $result['company_id']?>" class="views" title="Edit This Company"><?php echo $result['company_name']?></a><br>
                    Link: <a target="_blank" href="<?php echo $this->config->item('home_url'); ?>candidates?ch=<?php echo $result['company_hash']; ?>"><?php echo $this->config->item('home_url'); ?>candidates?ch=<?php echo $result['company_hash']; ?></a><br>
                    ...
                    Flp Date: <a href="javascript:void();" class="btn btn-success btn-xs"><?php echo ($result['flp_date']!='0000-00-00' && $result['flp_date']!='') ? date('d-m-Y', strtotime($result['flp_date'])) : '';?></a> ||  Next Date: <a href="javascript:void();" class="btn btn-warning btn-xs"><?php echo ($result['flp_next_date']!='0000-00-00' && $result['flp_next_date']!='') ? date('d-m-Y', strtotime($result['flp_next_date'])) : '';?></a> || Upd By:
                    <p class="btn btn-primary btn-xs"> <?php echo $result['firstname']?></p>
                    <br>
                    <br>
                    Lead Status:
                    <p class="btn btn-primary btn-xs">
                      <?php if($result['flp_status']==1)echo 'We Have Openings';?>
                      <?php if($result['flp_status']==2)echo 'No Openings';?>
                      <?php if($result['flp_status']==3)echo 'Call after a month';?>
                      <?php if($result['flp_status']==4)echo 'Already have vendor';?>
                      <?php if($result['flp_status']==5)echo 'We have in house team';?>
                      <?php if($result['flp_status']==6)echo 'Became Client';?>
                      <?php if($result['flp_status']==7)echo 'Do not Disturb';?>
                    </p>
                    || Created By:<?php echo $result['created_by']?>|| Total Jobs : <?php echo $result['total_jobs']?></td>
                  <td> Email:<a href="mailto:<?php echo $result['contact_email']?>?subject=test%20subject" class="btn btn-warning btn-xs"><?php echo $result['contact_email']?></a><br>
                    ...
                    Designation: <?php echo $result['designation']?><br>
                    <?php 
				$host_ar=parse_url($result['company_website']);

			?>
                    <a href="<?php if(isset($host_ar['path']) && $host_ar['path']!='')echo 'http://'.$host_ar['path']; if(isset($host_ar['host']) && $host_ar['host']!='') echo ''.$host_ar['host'];?>" target="_blank"><?php echo $result['company_website'];?></a> <br>
                    <a href="javascript:;" title="Get All Contacts" onclick="get_all_contacts(<?php echo $result['company_id'];?>);"  id="get_all_contacts" class="btn btn-info btn-xs"> Show All Contacts</a></td>
                  <td>
                  <a href="javascript:;" title="Make Calls" onclick="add_calls(<?php echo $result['company_id'];?>);"  data-url="<?php echo base_url(); ?>company/add_calls/?company_id=<?php echo $result['company_id'];?>"  id="add_calls" class="btn btn-info btn-xs"> Calls</a>
                  
                   || <a href="<?php echo base_url(); ?>company/edit/<?php echo $result['company_id'];?>" title="Edit"   id="edit_company" class="btn btn-info btn-xs"> Edit</a> 
                  <br>
                    <br>
                    <a href="<?php echo base_url(); ?>company/sentmail?ch=<?php echo $result['company_hash']; ?>" class="btn btn-info btn-xs" title="Send Email to Client">Send to Client</a> 
                    
                    <!--<a href="javascript:;" title="Assign this to BDE" onclick="assign_requirement(<?php echo $result['company_id'];?>);"  data-url="<?php echo base_url(); ?>company/assign_requirement/?job_id=<?php echo $result['company_id'];?>"  id="assign_requirement" class="btn btn-info btn-xs"> Assign</a>-->
                      ...
                      <form class="form-horizontal form-bordered"  method="post" action="<?php echo $this->config->site_url();?>company/add_calls" id="calls_form" name="calls_form">
                        ...
                        <table class="hori-form">
                          <tbody>
                            <tr>
                              <td>Call Status</td>
                              <td><input id="flp_status" type="radio" name="flp_status" value="1"  />
                                We Have Openings<br>
                                <input id="flp_status" type="radio" name="flp_status" value="2"/>
                                No Openings<br>
                                <input id="flp_status" type="radio" name="flp_status" value="3"/>
                                Call after a month <br>
                                <input id="flp_status" type="radio" name="flp_status" value="4"/>
                                Already have vendor <br>
                                <input id="flp_status" type="radio" name="flp_status" value="5"/>
                                We have in house team <br>
                                <input id="flp_status" type="radio" name="flp_status" value="6"/>
                                Became Client <br>
                                <input id="flp_status" type="radio" name="flp_status" value="7"/>
                                Do not Disturb </td>
                            </tr>
                            <tr>
                              <td>Lead Status</td>
...
function get_calls_history(company_id){
	...
	$('#show_followup_history').html('');	
	 $.ajax({			
			type: 'POST',
			url:"<?php echo base_url(); ?>company/get_calls_history/?company_id=<?php echo $result['company_id'];?>",
...
function assign_requirement(company_id)
{
	...
	$('#data_holder').html('Loading..................');	
	 $.ajax({			
			type: 'POST',
			url:"<?php echo base_url(); ?>company/assign_requirement/?company_id="+company_id,
...
function get_all_contacts(company_id){
	...
	$('#show_all_contacts').html('');	
	 $.ajax({			
			type: 'POST',
			url:"<?php echo base_url(); ?>company_calls/get_all_contacts/?company_id=<?php echo $result['company_id'];?>",
			data: $('#calls_form').serialize(),
			method: "POST",
  			data: { company_id : company_id },
		    dataType: "html",
