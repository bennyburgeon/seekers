  ...
  <div class="section-wrap">
    <div class="row">
      <ul class="page-breadcrumb breadcrumb">
        ...
        <li><a href="<?php echo $this->config->site_url();?>my_jobs">Jobs</a> <i class="icon-angle-right"></i> </li>
      ...
      <div class="col-sm-12">
        ...
        <div class="table-tech specs hor">
          <table class="hori-form">
            <tbody>
            <form action="<?php echo $this->config->site_url();?>my_jobs/update" class="form-horizontal" enctype="multipart/form-data" method="post" id="frmctype" name="frmentry" onSubmit="return validate();">
              <?php echo form_hidden('job_id', $formdata['job_id']);?>
              <input type="hidden" id="work_duration" name="work_duration" value="<?php echo $formdata['work_duration'];?>" placeholder="Work Duration in Days" class="form-control hori" />
                <input type="hidden" id="hourly_rate" name="hourly_rate" value="<?php echo $formdata['hourly_rate'];?>" placeholder="Hourly Rate" class="form-control hori" />
              ...
              <tr>
                ...
                <td>Qualification</td>
                 <td>
				<?php echo form_dropdown('level_id', $education, $formdata['level_id'],'class="form-control hori"');?>
                </td>
              </tr>
                <tr>
                <td>Course Name</td>
             ...
             <tr>
               <tr>
                ...
                <td><?php echo $this->ckeditor->editor('desired_profile',$formdata['desired_profile']);?></td>
              </tr>
                 <tr>
                <td>Work Level</td>
             ...
             <tr>
                <td>Skills</td>
                <td>
               <input type="text" id="job_skill_name" name="job_skill_name" value="<?php echo $formdata['job_skill_name'];?>" placeholder="Job Skills" class="form-control hori" />
                ...
                <tr>
                <td>Job Skills</td>
                <td><select class="form-control hori" onchange="myFunction();" id="parent"  name="parent">
            ...
            <!-- <tr>
                ...
                <tr>
                <td>Product name</td>
                <td><?php echo form_dropdown('product_id', $product_list, $formdata['product_id'],'class="form-control hori" id="product_id"');?></td>
              </tr>
             ...
             <!-- <tr>
              ...
              <tr>
                ...
                <td> <input style="width:100%; height: 38px; border-color: #d6d7d4;" type="text" readonly name="job_post_date" id="job_post_date" class="smallinput datepicker" value="<?php if($formdata['job_post_date']!='0000-00-00' && $formdata['job_post_date']!='')echo date('Y-m-d', strtotime($formdata['job_post_date']));?>" placeholder="Job Post Date">
                </td>
              </tr>
              ...
              <tr>
                ...
                <td>
                <input style="width:100%; height: 38px; border-color: #d6d7d4;" type="text" readonly name="job_update_date" id="job_update_date" class="smallinput datepicker" value="<?php if($formdata['job_update_date']!='0000-00-00' && $formdata['job_update_date']!='')echo date('Y-m-d', strtotime($formdata['job_update_date']));?>" placeholder="Job Update Date">
                </td>
              </tr>
              ...
              <tr>
                ...
                <td>
                <input style="width:100%; height: 38px; border-color: #d6d7d4;" type="text" readonly name="job_expiry_date" id="job_expiry_date" class="smallinput datepicker" value="<?php if($formdata['job_expiry_date']!='0000-00-00' && $formdata['job_expiry_date']!='')echo date('Y-m-d', strtotime($formdata['job_expiry_date']));?>" placeholder="Job Expiry Date">
                </td>
              </tr>
              <tr>
                <td>Expected join date</td>
                <td>
                <input style="width:100%; height: 38px; border-color: #d6d7d4;" type="text" readonly name="exp_join_date" id="exp_join_date" class="smallinput datepicker" value="<?php if($formdata['exp_join_date']!='0000-00-00' && $formdata['exp_join_date']!='')echo date('Y-m-d', strtotime($formdata['exp_join_date']));?>" placeholder="Expected Join Date">
                </td>
              </tr>
...
<script type="text/javascript">
	...
	if($('#country_id').val()=='')return;
		...
		$.ajax({
		  type: 'POST',
		  url: '<?php echo $this->config->site_url();?>my_jobs/getstate/',
	...
	if($('#state_id').val()=='')return;
		...
		$.ajax({
		  type: 'POST',
		  url: '<?php echo $this->config->site_url();?>my_jobs/getcity/',
...
<script type="text/javascript">
	...
	if($('#job_cat_id').val()=='')return;
		...
		$.ajax({
		  type: 'POST',
		  url: '<?php echo $this->config->site_url();?>my_jobs/get_functional_by_industry/',
	...
	if($('#func_id').val()=='')return;
		...
		$.ajax({
		  type: 'POST',
		  url: '<?php echo $this->config->site_url();?>my_jobs/get_designation_by_functional/',
...
$('#company_id').change(function() {
	...
	jQuery('#product_id').append('<option value="">Select Product</option');

			
	if($('#company_id').val()=='')return;

		$.ajax({
		  type: 'POST',
		  url: '<?php echo $this->config->site_url();?>my_jobs/getproduct/',
		  data: { company_id: $('#company_id').val()},
		  dataType: 'json',
		 
		  beforeSend:function(){
				jQuery('#product_id').html('');
				jQuery('#product_id').append('<option value="">Loading...</option');
		  },
		  ...
		  success:function(data){
			  ...
			  {
				  jQuery('#product_id').html('');
				  $.each(data.product_list, function (index, value) 
				  {
					  if(index=='')
						 jQuery('#product_id').append('<option value="'+ index +'" selected="selected">'+ value +'</option');
					 else
						 jQuery('#product_id').append('<option value="'+ index +'">'+ value +'</option');
				 });
		  ...
		  error:function(){
				alert('Problem with server. Pelase try again');
				jQuery('#product_id').html('');
				jQuery('#product_id').append('<option value="">Select State</option');
		  }
		});	
});
</script>