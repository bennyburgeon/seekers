...
class Jobsmodel extends CI_Model 
{
	...
	{
		$sql="select a.*,(select count(job_app_id) from pms_job_apps where job_id=a.job_id)as total_apps,(select count(job_app_id) from pms_job_apps where job_id=a.job_id and app_status_id=2)as total_rejected,b.*,c.package_name from ".$this->table_name." a left join pms_company b on a.company_id=b.company_id left join pms_packages c on a.package_id=c.package_id ";
		...
		foreach ($data as $key => $val)
 		{
			...
			if($query->num_rows()==0)
			{
				$data=array(
						...
						'cover_letter'=> '' ,
						'vendor_id'   => $_SESSION['admin_session'],
						'app_status_id'=> 1
						);
