...
$( document ).ready(function() {
   ...
   $('#save_candidate1').click(function(){
		...
		var candidateId = '<?php echo $candidate_id ?>';
			$.ajax({
				type: "post",
				url: "<?php echo site_url('register/addCandidateDetail'); ?>"+'/'+candidateId,
				cache: false,				
				data: dataStringprop,
				success: function(json){
					try{		
						...
						if(ret['STATUS']==1) {
                            ...
                            var site_url = "<?php echo site_url('register/loadPassporthtml'); ?>" +'/'+ candidateId;
...
$('#country_id').change(function() {
	...
	if($('#country_id').val()=='')return;
		...
		$.ajax({
		  type: 'POST',
		  url: '<?php echo $this->config->site_url();?>/register/getstate/',
...
$('#state_id').change(function() {
	...
	if($('#state_id').val()=='')return;
		...
		$.ajax({
		  type: 'POST',
		  url: '<?php echo $this->config->site_url();?>/register/getcity/',
...
$('#city_id').change(function() {
	...
	if($('#state_id').val()=='')return;
		...
		$.ajax({
		  type: 'POST',
		  url: '<?php echo $this->config->site_url();?>/register/getlocation/',
...
var dataStringprop = $("#candidate_form1").serialize();
	$.ajax({
				type: "post",
				url: "<?php echo site_url('register/skip_step2'); ?>"+'/'+candidateId,
				cache: false,				
				data: dataStringprop,
				success: function(json){
					try{		
						...
						if(ret['STATUS']==1) {
                            ...
                            var site_url = "<?php echo site_url('register/loadPassporthtml'); ?>" +'/'+ id;
                            $("#step1").load(site_url, function() {
                                //alert("success step2");
                            });
						}
