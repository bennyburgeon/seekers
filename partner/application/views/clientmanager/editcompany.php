...
<tbody>
      ...
      <form action="<?php echo $this->config->site_url();?>/clientmanager/update" class="form-horizontal form-bordered"  method="post" id="frmentry" name="frmentry" onSubmit="return validate();" enctype="multipart/form-data"> 
...
<tr>
    ...
    <a href="<?php echo $this->config->site_url();?>/clientmanager" class="attach-subs subs">Cancel</a>
    </span>
    </td>
    </tr>
    </form>
...
{	
	if($('#company_name').val()=='')
	{
		alert('Please enter clientmanager name');
		$('#company_name').focus();
		return false;
	}
/*	
	if($('#type_id').val()=='0')
	{
		alert('Please select clientmanager type');
		$('#type_id').focus();
		return false;
	}	
...
$('#country_id').change(function() {
	...
	if($('#country_id').val()=='')return;
		...
		$.ajax({
		  type: 'POST',
		  url: '<?php echo $this->config->site_url();?>/clientmanager/getstate/',
...
$('#state_id').change(function() {
	...
	if($('#state_id').val()=='')return;
		...
		$.ajax({
		  type: 'POST',
		  url: '<?php echo $this->config->site_url();?>/clientmanager/getcity/',
