...
<form name="form1" method="post" id="form1" action="<?php  echo $this->config->site_url();?>/freelance/multidelete?rows=<?php echo $rows;?>" >


<div class="right-btns">
<a href="<?php echo base_url();?>index.php/freelance/add" class="attach-subs tools"><img src="<?php echo base_url('assets/images/plus.png');?>">Add New</a>
...
<tr class="odd gradeX">
	...
	<div class="checker">
    	<span>
    		<input type="checkbox" name="checkbox[]" class="checkboxes" value="<?php echo $result['part_id']?>" >
        </span>
	</div>
</td>
								<td>
									 <?php echo $result["part_title"]; ?>
								</td>
								<td>
								...
								<td><?php echo $result["time_allowed"]; ?></td>
								
								<td><?php echo $result["meeting_venue"]; ?></td>
								<td>
                                   
                                    <?php if($result['part_status']=='1'){?>
                                    <a href="<?php  echo $this->config->site_url();?>/freelance/changestat/<?php echo $result['part_id']?>?part_status=0"> <span class="label label-sm label-success">Active</span></a>
                                     <?php }elseif($result['part_status']=='0'){?>
									<a href="<?php  echo $this->config->site_url();?>/freelance/changestat/<?php echo $result['part_id']?>?part_status=1" ><span class="label label-sm label-danger">Blocked </span></a>
                                     <?php } ?>
			...
			<a href="<?php echo base_url();?>index.php/freelance/edit/<?php echo $result['part_id']?>" class="views" title="Edit"><img src="<?php echo base_url('assets/images/edits.png');?>"></a>
...
$(document).ready(function()
{
	...
	$("#search").click(function(){
		...
		window.location.href = '<?php echo $this->config->site_url();?>/freelance?searchterm='+searchterm;
	});
	$("#sel_limit1").change(function(){
		var limits = $(this).find(":selected").val();
		window.location.href = '<?php echo $this->config->site_url();?>/freelance?limit='+limits;
	});
