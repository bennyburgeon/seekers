...
<?php if(isset($formdata['profile_list']['1'])){ ?>
        ...
        <table class="hori-form">
        	<tbody>
                <tr>
                    ...
                    <td> 
 						<input class="form-control datepicker hori " readonly="readonly" type="text" value="<?php echo $formdata['job_date'];?>" name="job_date" id="datepickfrom1"  
                ...
                <tr>
                    <td>Current CTC</td>
                    <td><input class="form-control hori" value="<?php echo $formdata['current_ctc'];?>" type="text" name="current_ctc" value="" id="current_ctc"> </td>
                </tr>
                
                <tr>                
                    <td>Expected CTC</td>
                    <td><input class="form-control hori" value="<?php echo $formdata['expected_ctc'];?>" type="text" name="expected_ctc" value="" id="expected_ctc"></td>
...
<h2>Job Details</h2>
    <table class="hori-form">    
        <tbody>
                ...
                <tr>
                     <td>Industry</td>
                     <td> <?php echo form_dropdown('job_cat_id',  $industry_list, '','class="form-control job-field js-example-basic-single" id="job_cat_id" style="width:100%;"');?> </td>
                </tr>
                
                <tr>
                     <td>Function/Role</td>
                     <td> <?php echo form_dropdown('func_id',  $functional_list, '','class="form-control job-field js-example-basic-single" id="func_id" style="width:100%;"');?> </td>
                </tr>
                ...
                <tr>
                    <td>From Date</td>
                    <td><input class="form-control hori datepicker job-field" type="text" name="from_date" id="datepickfrom" readonly="readonly"
                    value="<?php echo $formdata['from_date'];?>" placeholder="Enter From Date Date YYYY-MM-DD"></td>
                </tr>
                <tr>
                    <td>To Date</td>
                    <td><input class="form-control hori datepicker job-field" type="text" readonly="readonly" name="to_date" id="datepickto" value=""
...
<div class="table-tech specs hor">
  ...
  <form class="form-horizontal form-bordered"  method="post" id="candidate_form5" name="candidate_form5" enctype="multipart/form-data"> 
    ...
    <table class="hori-form">
        <tbody>
                ...
                <tr>
                    <td colspan="2">
                    <span class="click-icons">
                    <input type="submit" class="attach-subs" value="Save" id="save_file"  style="width:180px;">
...
<form class="form-horizontal form-bordered"  method="post" id="candidate_form51" name="candidate_form51" enctype="multipart/form-data"> 
 
    <table class="hori-form">
        <tbody>
                <tr>
                    <td>Do you completed data entry?</td>
                    <td><input id="profile_completion" type="radio" name="profile_completion" value="3"   />Yes &nbsp;
                    <input type="radio" name="profile_completion" value="2" id="profile_completion"  checked="checked" />No 
                    &nbsp;&nbsp;</td>
                </tr>
                <tr>
                    <td colspan="2">
                    <span class="click-icons">
                    <input type="button" class="attach-subs" value="Done" id="complete_status" style="width:180px;">
...
function profile_complete() 
{
	...
	$.ajax({
		type: "post",
		url: "<?php echo site_url('final_verify/check_profile_complete'); ?>"+'/'+candidateId,
...
//CONTACT DETAILS VALIDATION AND SUBMIT
   ...
   $('#edit_candidate1').click(function(){
		...
		var candidateId = '<?php echo $candidate_id ?>';
			$.ajax({
				type: "post",
				url: "<?php echo site_url('final_verify/editCandidateDetail'); ?>"+'/'+candidateId,
				cache: false,				
				data: dataStringprop,
				success: function(json){
					try{		
...
//BEGIN PASSPORT DETAILS
   ...
   $('#edit_candidate2').click(function(){ 
		...
		var candidateId = '<?php echo $candidate_id ?>';
			$.ajax({
				type: "post",
				url: "<?php echo site_url('final_verify/editPassportDetail'); ?>"+'/'+candidateId,
				cache: false,				
				data: dataStringprop,
				success: function(json){
					try{		
...
//BEGIN SKILLS AND CERTIFICATE
   $('#edit_candidate7').click(function(){ 
		...
		var candidateId = '<?php echo $candidate_id ?>';
			$.ajax({
				type: "post",
				url: "<?php echo site_url('final_verify/editSkillCertificateDetail'); ?>"+'/'+candidateId,
				cache: false,				
				data: dataStringprop,
				success: function(json){ 
					try{		
...
*/	    return true;
   ...
   $('#save_candidate3').click(function(){
		...
		if(isContactValid) {
			...
			$.ajax({
				type: "post",
				url: "<?php echo site_url('final_verify/addEducationDetail'); ?>"+'/'+candidateId,
				cache: false,
				data: dataStringprop,
				success: function(json){
					try{		
...
//BEGIN PANNING JOB DEATILS
   ...
   $('#edit_candidate8').click(function(){
		...
		if(isContactValid) {
			...
			$.ajax({
				type: "post",
				url: "<?php echo site_url('final_verify/editJobChangeDetail'); ?>"+'/'+candidateId,
				cache: false,
				data: dataStringprop,
				success: function(json){
					try{		
...
//BEGIN DELETE EDUCATION
	...
	{ 
		...
		if(isconfirm){
			$.ajax({
				type: "post",
				url: "<?php echo site_url('final_verify/deleteEducationDetail'); ?>"+'/'+candidateId,
				cache: false,				
				data: { edu_id:edu_id},
				success: function(json){
					try{		
...
//BEGIN JOB DETAILS
   ...
   $('#edit_candidate4').click(function(){
		...
		if(isContactValid) {
			...
			$.ajax({
				type: "post",
				url: "<?php echo site_url('final_verify/addJobDetail'); ?>"+'/'+candidateId,
				cache: false,				
				data: dataStringprop,
				success: function(json){
					try{		
...
//BEGIN DELETE JOB
	...
	{ 
		...
		if(isconfirm){
			$.ajax({
				type: "post",
				url: "<?php echo site_url('final_verify/deleteJobDetail'); ?>"+'/'+candidateId,
				cache: false,				
				data: { job_id:job_id},
				success: function(json){
					try{		
...
//BEGIN FILE DETAILS
	$('#candidate_form5').submit(function(evt){
		evt.preventDefault();
		var $this = $('#candidate_form5');  
		var candidateId = '<?php echo $candidate_id ?>';
     	var formData = new FormData(this);
        $.ajax({
			
			type: 'POST',
			url: "<?php echo site_url('final_verify/editfiles'); ?>"+'/'+candidateId,
			data:formData,
			cache:false,
			contentType: false,
			processData: false,
		...
		/*$(this).ajaxSubmit({success:function(data){ 
			...
			if(($.trim(data)=='Add title')||($.trim(data) == 'Invalid file format')||($.trim(data) == 'Choose file')||($.trim(data) == 'Image file size max 1 MB')||($.trim(data) == 'failed')){
				...
				window.location.href = '<?php echo $this->config->site_url();?>/final_verify/';
			}	
		}*/
		});
});
...
//BEGIN DELETE PHOTO
	...
	{ 
		...
		$.ajax({
			type: "post",
			url: "<?php echo site_url('final_verify/photo_delete'); ?>"+'/'+candidateId,
			cache: false,				
...
//BEGIN DELETE CV
	...
	{ 
		...
		$.ajax({
			type: "post",
			url: "<?php echo site_url('final_verify/cv_delete'); ?>"+'/'+candidateId,
			cache: false,				
...
$('#country_id').change(function() {
	...
	if($('#country_id').val()=='')return;
		...
		$.ajax({
		  type: 'POST',
		  url: '<?php echo $this->config->site_url();?>/final_verify/getstate/',
...
$('#state_id').change(function() {
	...
	if($('#state_id').val()=='')return;
		...
		$.ajax({
		  type: 'POST',
		  url: '<?php echo $this->config->site_url();?>/final_verify/getcity/',
...
$('#city_id').change(function() {
	...
	if($('#state_id').val()=='')return;
		...
		$.ajax({
		  type: 'POST',
		  url: '<?php echo $this->config->site_url();?>/final_verify/getlocation/',
	...
	$('#level_id').change(function() 
	{
		...
		if($('#level_id').val()=='')return;
			$.ajax({
			  type: 'POST',
			  url: '<?php echo $this->config->site_url();?>/final_verify/getcourses/',
 ...
 $('#organization').autocomplete({ 
  ...
  source: function(request, response){ 
   ...
   if( previous_request ) {
    ...
    $element.data('jqXHR',$.ajax({url:'<?php echo $this->config->site_url();?>/final_verify/get_all_organisation', type: "GET", data:{organization: $('#organization').val()},dataType: "json", success:function(data) 																																							   {
		 if (data == null) 
		 {
 ...
 $('#designation').autocomplete({ 
  ...
  source: function(request, response){ 
   ...
   if( previous_request ) {
    ...
    $element.data('jqXHR',$.ajax({url:'<?php echo $this->config->site_url();?>/final_verify/get_all_designation', type: "GET", data:{designation: $('#designation').val()},dataType: "json", success:function(data) {
     if (data == null) {
      $(".ui-autocomplete").hide();
     } else {
      response(data);
 ...
 $('#responsibility').autocomplete({ 
  ...
  source: function(request, response){ 
   ...
   if( previous_request ) {
    ...
    $element.data('jqXHR',$.ajax({url:'<?php echo $this->config->site_url();?>/final_verify/get_all_responsibility', type: "GET", data:{responsibility: $('#responsibility').val()},dataType: "json", success:function(data) {
     if (data == null) {
      $(".ui-autocomplete").hide();
     } else {
      response(data);
