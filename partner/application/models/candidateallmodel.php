...
class Candidateallmodel extends CI_Model {
	...
	function record_count($search_email,$search_name,$search_mobile,$cur_job_status,$branch_id,$skills,$level_id,$course_id,$spcl_id,$exp_years,$lead_source,$job_folder_id) 
	{
		$sql="select count(a.candidate_id)as total_rec  from pms_candidate a LEFT JOIN pms_candidate_to_skills_primary c on a.candidate_id=c.candidate_id LEFT JOIN pms_candidate_education d on a.candidate_id=d.candidate_id LEFT JOIN pms_candidate_job_search g on a.candidate_id=g.candidate_id";
		$cond='';

		if($exp_years!='')
		{
			if($cond!='')
				$cond.=" and g.total_experience >=". $exp_years ;
			else
				$cond ="  g.total_experience >=". $exp_years ;
...
//course		
	...
	function get_list($start,$limit,$search_email,$search_name,$search_mobile,$sort_by,$cur_job_status,$branch_id,$skills,$level_id,$course_id,$spcl_id,$exp_years,$lead_source,$job_folder_id)
	{
		$sql="SELECT a.candidate_id, a.first_name, a.last_name, a.username,a.reg_date,a.lead_opportunity, a.mobile,c.skill_id,a.photo FROM ".$this->table_name." a LEFT JOIN pms_candidate_to_skills_primary c on a.candidate_id=c.candidate_id LEFT JOIN pms_candidate_education d on a.candidate_id=d.candidate_id LEFT JOIN pms_candidate_job_search g on a.candidate_id=g.candidate_id";
		$cond='';
...
function select_aplication_visa($candidate_id)
    ...
    {
		$query = $this->db->query('SELECT a.*,b.admin_id,b.firstname as admin_name,c.first_name as candidate FROM pms_candidate_messages a left join pms_admin_users b on a.admin_id=b.admin_id left join pms_candidate c on a.candidate_id=c.candidate_id where a.candidate_id='.$candidate_id.' order by message_id desc');
		...
		{
			$message_list[]=array('message_id' => $main['message_id'],'message_title' => $main['message_title'],'message_text' => $main['message_text'],'message_date' => $main['message_date'],'message_time' => $main['message_time'],'message_status' => $main['message_status'],'admin_id' => $main['admin_id'],'admin_name' => $main['admin_name'],'candidate' => $main['candidate']);
		}
		return $message_list;		
	}	
...
$this->db->query("update pms_candidate_files set file_name='no_photo.png',file_type='no_photo.png' where file_name='".$row['photo']."' and candidate_id=".$id);
	...
	function notice_days_list()
	{
		$dropDownList[0]='0 Days';
		for($i=1;$i<=60;$i++)
		{
			 $dropDownList[$i] = $i.' Days';
		}
		return $dropDownList;
	}
	...
	function ctc_list()
	{
		$dropDownList[0]='0 Lacs';
		for($i=1;$i<=60;$i++)
		{
			 $dropDownList[$i * 100000] = $i.' Lacs';
		}
		return $dropDownList;
	}
