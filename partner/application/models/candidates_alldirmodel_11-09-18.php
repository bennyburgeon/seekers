...
class Candidates_alldirmodel extends CI_Model {
	...
	function get_list($start,$limit,$search_email,$search_name,$search_mobile,$sort_by,$cur_job_status,$reg_status,$skills,$level_id,$course_id,$spcl_id,$exp_years,$lead_source,$job_folder_id,$job_cat_id,$func_id, $desig_id, $city_id, $pref_city_id, $expected_ctc_from, $expected_ctc_to, $notice_period_from, $notice_period_to, $total_experience_from, $total_experience_to)
	{
		...
		$sql.=" FROM ".$this->table_name." a LEFT JOIN pms_candidate_job_search b on a.candidate_id=b.candidate_id left join pms_curernt_job_status c on a. 	cur_job_status=c.cur_job_status inner join pms_company_temp ct on a.candidate_id=ct.candidate_id";
...
//STOP	
	...
	function add_company($data,$candidate_id)
	{
...
function select_short_company($candidate_id)
    {
        $query=$this->db->query("select * from pms_company_temp where candidate_id=".$candidate_id." order by company_id desc");
		return $query->result_array();
	}
	
	function delete_short_company($candidate_id, $company_id)
    {
	...
	function update_candidates_all_dir($data, $company_id, $candidate_id)
	{
		$this->db->where('company_id', $company_id);
		$this->db->where('candidate_id', $candidate_id);
