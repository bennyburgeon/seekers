<?php 
class Trainingcoursesmodel extends CI_Model {
	var $table_name='';
    ...
    {
		$this->table_name='pms_trainingcenter_courses';
		$this->upload_file_name='';
    }

	function record_count($searchterm,$international,$level_study) 
	{
		$sql="select count(*)as course_id from pms_trainingcenter_courses a left join pms_education_level b on a.level_study=b.level_id ";
		$cond	= '';
    ...
    {
		$sql="select a.*, b.level_name from ".$this->table_name." a left join pms_education_level b on a.level_study=b.level_id ";
	...
	function get_international($course_id)
	{
		...
		{	
			$query = $this->db->query("select course_id,course_name from ".$this->table_name." where level_study=".$level['level_id']." and course_id not in (select course_id from pms_trainingcenter_to_courses where course_id=".$course_id." ) order by course_name ");
	...
	function get_international_current($course_id)
	{
		...
		foreach($levels_list as $levels => $level)
		{	
		$query = $this->db->query("select  a.course_id,a.course_name from pms_trainingcenter_courses a inner join pms_trainingcenter_to_courses b on a.course_id=b.center_id where b.course_id=".$course_id." and a.level_study=".$level['level_id']." order by a.course_name");
	...
	function insert_record()
    {
		...
		if (isset($_FILES['course_info_attach']) && is_uploaded_file($_FILES['course_info_attach']['tmp_name'])) 
		{            
	   ...
	   $this->db->update($this->table_name, $data);
		...
		{            
			...
			if ($this->upload->do_upload('course_info_attach'))
			{
				$data =  $this->upload->data();	
				$this->upload_file_name=$data['file_name'];
				$query = $this->db->query("select course_info_attach from pms_trainingcenter_courses where course_id=".$id);
				if ($query->num_rows() > 0)
				{
					$row = $query->row_array();
			...
			$query = $this->db->query("update pms_trainingcenter_courses set course_info_attach='".$this->upload_file_name."' where course_id=".$id);
	...
	function fill_training_centers()
	{
		$query = $this->db->query('select distinct center_id, center_name from  pms_trainingcenter order by center_name asc');
		$dropdowns = $query->result();
		$dropDownList['']='Select Training Center';
		foreach($dropdowns as $dropdown)
		{
			 $dropDownList[$dropdown->center_id] = $dropdown->center_name;
		}
	...
	{
		$query = $this->db->query('select course_id, course_name from pms_trainingcenter_courses where level_study='.$level_study.'  and international='.$int_val.' order by course_name asc');
		$dropdowns = $query->result();
	...
	function get_course_info($course_id)
	{
		...
		$query = $this->db->query('select a.*,b.* from pms_trainingcenter_courses a left join pms_education_level b on a.level_study=b.level_id where a.course_id='.$course_id);
		$row = $query->row_array();
		return $row;
	}
	...
	function delete_records($delete_rec=array())	
	{
		if(is_array($delete_rec))
		{
			 foreach ($delete_rec as $key => $val)
 				{
					$query = $this->db->query("select course_info_attach from pms_trainingcenter_courses courses where course_id=".$val);
					if ($query->num_rows() > 0)
					{
						$row = $query->row_array();
						if(file_exists('uploads/'.$row['course_info_attach']) && $row['course_info_attach']!='')
					...
					$this->db->delete('pms_trainingcenter_courses'); 
				}
			return true;
		}else
		{
