<?php 
class Candidatectcmodel extends CI_Model {
	var $table_name='';
	...
	function record_count($search_email,$search_name,$search_mobile,$cur_job_status,$reg_status,$skills,$level_id,$course_id,$spcl_id,$exp_years,$lead_source,$job_folder_id) 
	{
...
//course		
	...
	function get_list($start,$limit,$search_email,$search_name,$search_mobile,$sort_by,$cur_job_status,$reg_status,$skills,$level_id,$course_id,$spcl_id,$exp_years,$lead_source,$job_folder_id)
	{
		$sql="SELECT a.candidate_id, a.first_name, a.last_name, a.username,a.reg_date,a.lead_opportunity, a.mobile,a.photo,a.reg_status,b.current_ctc,b.expected_ctc,b.notice_period,b.total_experience,b.ctc_updated_on,c.cur_job_status_name ";
		...
		$sql.=", (select concat(feedback_education,feedback_industry,feedback_skills,feedback_salary,feedback_general) as consultant_feedback from  pms_candidate_job_search cf where cf.candidate_id=a.candidate_id limit 0,1) as consultant_feedback ";
		...
		$sql.=" order by a.reg_status asc, total_job_apps asc, b.ctc_updated_on ".$sort_by." limit ".$start.",".$limit;		
  ...
  function fill_bands()
   ...
   function education_deatils($candidate_id)
   {
	...
	{
		$data = array();		
		$query=$this->db->query("select a.candidate_id,a.avail_date,b.shift_name,b.shift_id,c.shift_status_id from pms_candidate_available_dates a left join pms_shifts b on a.shift_id=b.shift_id left join pms_client_shifts c on a.avail_id=c.avail_id where a.candidate_id=".$candidate_id." and month(a.avail_date)=".$month." and year(a.avail_date)=".$year);		
		$dropDownList = array();
		$admin_list = $query->result();		
		
		foreach($admin_list as $dropdown)
		{
			$dropDownList[$dropdown->avail_date] =array('candidate_id' => $dropdown->candidate_id, 'avail_date' => $dropdown->avail_date, 'shift_id' => $dropdown->shift_id, 'shift_name' => $dropdown->shift_name, 'shift_status_id' => $dropdown->shift_status_id);
		}
	...
	function update_shift_vacancy($candidate_id,$avail_date,$month,$year,$shift_id)
	{
	...
	function update_shift_vacancy_month($candidate_id,$month,$year)
	{
		$this->db->where('candidate_id', $candidate_id);
