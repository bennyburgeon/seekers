...
class ticketstatusmodel extends CI_Model {
	...
	function record_count($searchterm) 
	{
		...
		$sql	= "select count(*)as ticket_status_id from ".$this->table_name;
		$cond	= '';
		...
		if($searchterm!='')
		{
			...
			//$cond.=" and connum=".$connum;
				...
				$cond =" ticket_status_name like '%" . $searchterm . "%'";
			}	
		} 
	...
	function get_list($start,$limit,$searchterm,$sort_by)
    {
		...
		$sql.=" order by ticket_status_name ".$sort_by." limit ".$start.",".$limit;
		$query = $this->db->query($sql);
