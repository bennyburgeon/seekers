...
class companymodel extends CI_Model {
	...
	function record_count($searchterm,$search_designation,$ind_id,$flp_status,$company_priority,$company_rating,$status,$flp_next_date,$date_added,$user_id,$unassigned,$city_id,$building_location) 
	{
	...
	{
		if($cond!=''){
			$cond.=" and a.designation like '%" . $search_designation . "%'";
		} 
		else{
			$cond=" a.designation like '%" . $search_designation . "%'";
		}  
	} 
	...
	function get_list($start,$limit,$searchterm,$search_designation,$ind_id,$flp_status,$sort_by,$company_priority,$company_rating,$status,$flp_next_date,$date_added,$user_id,$unassigned,$city_id,$building_location)
	{
	...
	if($flp_next_date!='')
	{
		...
		$sql.=" order by a.company_name asc limit ".$start.",".$limit;
  ...
  function state_list_by_city($country_id='')
	 ...
	 function insert_csv_records($data)
    {
		...
		$data=array('company_id' => $id, 'admin_id' => $_SESSION['vendor_session']);
