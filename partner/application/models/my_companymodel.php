<?php 
class My_companymodel extends CI_Model {
	var $table_name='';
	...
	function get_list($start,$limit,$searchterm,$ind_id,$flp_status,$sort_by,$company_priority,$company_rating,$status,$flp_next_date,$date_added,$user_id,$unassigned,$city_id,$building_location)
	{
		...
		$sql.=" ,(select flp_status from pms_company_followup where company_id=a.company_id order by flp_id desc limit 0,1) as flp_status ";
		$sql.=" ,(select count(*) as total_jobs from pms_jobs jb where jb.company_id=a.company_id) as total_jobs ";
		$sql.=" from ".$this->table_name." a inner join pms_company_to_recruiter cr on a.company_id=cr.company_id ";	
		
		$cond = "";	
		$cond = " cr.admin_id=".$_SESSION['vendor_session'];	
	   ...
	   $this->db->update($this->table_name, $data);
		...
		{            
			...
			if ($this->upload->do_upload('company_logo'))
			{
				$data =  $this->upload->data();	
				$this->upload_file_name=$data['file_name'];
				$query = $this->db->query("select company_logo from pms_company where company_id=".$id);
				if ($query->num_rows() > 0)
				{
					$row = $query->row_array();
					if(file_exists('uploads/company_logo/'.$row['company_logo']) && $row['company_logo']!='')
					unlink('uploads/company_logo/'.$row['company_logo']);
				}
			$query = $this->db->query("update pms_company set company_logo='".$this->upload_file_name."' where company_id=".$id);
			}
		}
