<?php 
class Candidates_alldirmodel extends CI_Model {
	var $table_name='';
	...
	function record_count($search_email,$search_name,$search_mobile,$cur_job_status,$reg_status,$skills,$level_id,$course_id,$spcl_id,$exp_years,$lead_source,$job_folder_id,$job_cat_id,$func_id, $desig_id, $city_id, $pref_city_id, $expected_ctc_from, $expected_ctc_to, $notice_period_from, $notice_period_to, $total_experience_from, $total_experience_to) 
	{
	...
	function get_list($start,$limit,$search_email,$search_name,$search_mobile,$sort_by,$cur_job_status,$reg_status,$skills,$level_id,$course_id,$spcl_id,$exp_years,$lead_source,$job_folder_id,$job_cat_id,$func_id, $desig_id, $city_id, $pref_city_id, $expected_ctc_from, $expected_ctc_to, $notice_period_from, $notice_period_to, $total_experience_from, $total_experience_to)
	{
		...
		$sql.=", (select count(candidate_id) as total_shortlists from pms_company_temp pct where pct.candidate_id=a.candidate_id) as total_shortlists ";
		...
		$cond=' a.candidate_id in(select candidate_id from pms_company_temp)';
		//$cond=" (g.current_ctc ='' or g.expected_ctc ='' or g.notice_period ='' or g.total_experience ='') ";
          if($expected_ctc_from!='')
		{
			if($cond!='')
   ...
   {
   		$query = $this->db->query('select a.*,c.course_name,d.level_name,e.univ_name,f.country_name from pms_candidate_education a inner join pms_candidate b on a.candidate_id=b.candidate_id left join pms_courses c on a.course_id=c.course_id left join pms_education_level d on a.level_id=d.level_id left join pms_university e on a.univ_id=e.univ_id left join pms_country f on a.edu_country=f.country_id where a.candidate_id='.$candidate_id.' order by a.edu_year');
		return $query->result_array();
   }   
    ...
    {
        $query=$this->db->query("select a.*,b.job_cat_name,c.* from pms_candidate_job_profile a left join pms_job_category b on a.job_cat_id=b.job_cat_id left join pms_job_functional_area c on a.func_id=c.func_id  where a.candidate_id=".$candidate_id);
		return $query->result_array();
	}	
...
//STOP	
	...
	function update_company($data,$candidate_id)
	{
		$this->db->query("delete from pms_company_temp where candidate_id=".$candidate_id);
		$this->db->insert('pms_company_temp', $data);
		return;		
	}
	
function select_company_lists($candidate_id)
    {
        $query=$this->db->query("select a.* from pms_company_temp a inner join pms_candidate b on a.candidate_id=b.candidate_id where a.candidate_id=".$candidate_id." order by company_id desc");
		return $query->result_array();
	}
	...
	function delete_company($candidate_id, $company_id)
    {
		...
		$this->db->delete('pms_company_temp');
		return 0;
	}	
	
	function update_company_lists($data,$company_id,$candidate_id)
	{
		$this->db->where('company_id', $company_id);
		$this->db->where('candidate_id', $candidate_id);
		$this->db->update('pms_company_temp', $data);				
		return 0;
	}
