<?php 
class Tasksmodulemodel extends CI_Model {
    ...
    {
		$this->table_name='pms_task_module';
    }
	
	 function record_count($searchterm) 
 {
 
  $sql = "select count(*)as task_module_id from ".$this->table_name;
  $cond = '';
  ...
  if($searchterm!='')
  {
   ...
   //$cond.=" and connum=".$connum;
    ...
    $cond =" task_module_name like '%" . $searchterm . "%'";
   } 
  } 
 ...
 function get_list($start,$limit,$searchterm,$sort_by)
  ...
  $sql.=" order by task_module_name ".$sort_by." limit ".$start.",".$limit;
  $query = $this->db->query($sql);
  return $query->result_array();
	...
	function update_record()
	{
       ...
       $this->db->where('task_module_id', $this->input->post('task_module_id'));
	...
	/*function delete($id=null)
	{
		...
		$query = $this->db->query("select * from ".$this->table_name." where task_module_id=".$id);
		if ($query->num_rows() > 0)
		{
			$row = $query->row_array();

			//if($this->is_used($id)==true)return $row['task_module_name'];
			
			
		}		
				
		$this->db->where('task_module_id', $id);
		$this->db->delete('pms_task_module'); 
	...
	{
		$master_tables = array(array('table'=>'pms_tasks','key' => 'task_module_id','Module'=>'Tasks'));
		$is_related = FALSE;
	...
	{
		$this->db->query('task_module_name', $name);
		if($id!='')	$this->db->where('task_module_id !=', $id);		
