...
class Seminars extends CI_controller
 {
	function __construct()
	{
	   ...
	   if(!isset($_SESSION['vendor_session']) || $_SESSION['vendor_session']=='')redirect('logout');
		...
		$this->data['cur_page_name']=config_item('page_title').' Seminars ';
		$this->data['current_page_head']='Seminars';
		$this->data['page'] = 'Seminars';
		$this->data['module_head'] = 'Manage Seminars';
	...
	function editor($path,$width) {
     ...
     function index()	 
	 {
	    ...
	    $this->data['module_action'] = 'List All Seminars';
		$this->data["page_head"]= "Manage Seminars";
		...
		$this->data['total_rows']= $this->seminarsmodel->record_count($searchterm);
		...
		$config['base_url'] = $this->config->item('base_url')."seminars/?sort_by=$sort_by&limit=$limit&searchterm=$searchterm$query_str";
		...
		// paging ends here
		$this->data["records"] = $this->seminarsmodel->walkin_list($start,$limit,$searchterm,$sort_by);
		//print_r($this->data["records"]);
		//exit();
		$this->data["rows"]=$start;
		$this->data["sort_by"]=$sort_by;
		$this->data["searchterm"]=$searchterm;
		$this->data['pahe_head'] = 'Manage Seminars';
		$this->load->view("include/header",$this->data);
		$this->load->view("seminars/list",$this->data);
	...
	function add()
	{
		...
		$this->data['page_head'] = 'Add Seminars';

		if($this->input->post("walkin_title"))
		{ 
			$this->form_validation->set_rules("walkin_title","Seminars Title","required");
			//$this->form_validation->set_rules("event_type","Event Type","required");
			if ($this->form_validation->run() == TRUE)
			{ 
				$id = $this->seminarsmodel->insert_record();
				redirect('seminars/?ins=1');
			}
			$this->data["formdata"] = array(
		...
		$this->data["job_list"] = $this->seminarsmodel->get_job_list();
		
			 $path = '../js/ckfinder';
		    $width = '700px';
		    $this->editor($path, $width);
		$this->load->view("include/header");
		$this->load->view("seminars/add",$this->data);
		$this->load->view("include/footer");
	}
	
	function edit($id='')
	{
		$this->data['page_head'] = 'Edit Seminars';
		if(!empty($id))
		{
			$this->data["formdata"] = $this->seminarsmodel->single_admin($id);
		}
		$this->data["interview_type_list"] = array(
				"1"                => "Telephonic",				
				"2"               => "HR",
		...
		$this->load->view("include/header",$this->data);
		$this->load->view("seminars/edit",$this->data);
		$this->load->view("include/footer",$this->data);
	}
	...
	{
		$this->data['page_head'] = 'Edit Seminars';
		$this->data['module_action'] = 'Update Seminars';
		$this->data["interview_type_list"] = array(
				"1"                => "Telephonic",				
				"2"               => "HR",
				"3"               => "Face to Face",
		...
		{ 
			...
			if ($this->form_validation->run() == TRUE)
			{ 
				$this->seminarsmodel->update_record();
				redirect('seminars/?upd=1');
			}
			else
			{
			$this->data["formdata"] = array(
	...
	function delete($id='')
	{
		if($id!='')
		{
			$this->seminarsmodel->delete_record($id);
			redirect('seminars/?del=1');
		}
			if($this->input->post("delete_rec"))
			{
				 foreach ($this->input->post("delete_rec") as $key => $val)
					{
						$this->seminarsmodel->delete_record($val);
					}
				redirect('seminars/?del=1');
			}
			else
	...
	function multidelete(){
		...
		if(count($id_arr)>0){
			$this->load->model('seminarsmodel');
			$this->seminarsmodel->delete_multiple_record($id_arr);
			redirect('seminars/?multi=1');
		}
		else{
			redirect('seminars');
		}
	...
	{
		if($id=='')redirect('seminars');
		if($this->input->get('int_status')=='')redirect('seminars');
	...
	{ 
		...
		if($query->num_rows()==0) return true;
			...
			$this->form_validation->set_message('check_dups',"Seminars title already exists");
			return false;
		}
	}
