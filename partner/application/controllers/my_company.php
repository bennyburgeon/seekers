...
class My_company extends CI_Controller {
	...
	function __construct()
	{
		...
		$this->load->model('my_companymodel');
	...
	{	$this->load->library('pagination');
		...
		$this->data['total_rows']= $this->my_companymodel->record_count($searchterm,$ind_id,$flp_status,$company_priority,$company_rating,$status,$flp_next_date,$date_added,$user_id,$unassigned,$city_id,$building_location);
		...
		$this->data["records"] = $this->my_companymodel->get_list($start,$limit,$searchterm,$ind_id,$flp_status,$sort_by,$company_priority,$company_rating,$status,$flp_next_date,$date_added,$user_id,$unassigned,$city_id,$building_location);

		$this->data["industry_list"] = $this->my_companymodel->industry_list_search();
		...
		*/	
		...
		$this->data["admin_list"]=$this->my_companymodel->admin_list();
		$this->data["bde_list"]=$this->my_companymodel->bde_list();

		$this->data["building_loc_list"] = $this->my_companymodel->building_loc_list($state_id);
		
		$this->data["city_list"] = $this->citymodel->city_list($state_id);
		$this->data["state_list"] = $this->statemodel->state_list($country_id);	
		$this->data["country_list"] = $this->my_companymodel->country_list();
		...
		$this->load->view('my_company/companylist',$this->data);
    ...
    {    
		...
		{	
			...
			{         
				...
				if ($this->upload->do_upload('userfile'))
				{
					...
					while (!feof($file)) 
					{
						...
						else
						{
							...
							//	unset($company_row['country_name']);
								$my_company[] = $company_row;
							}
						}
					}
					
					
					for($i=0;$i<count($my_company);$i++)
					{
						if(isset($my_company[$i]['city_id']) && $my_company[$i]['city_id'])
						{
							$query = $this->db->query("select city_id from pms_city where city_name='".$my_company[$i]['city_id']."'");					
							if ($query->num_rows() > 0)
							{
								$row = $query->row_array();
								$row1=$my_company[$i];
					...
					for($i=0;$i<count($my_company);$i++)
					{
						if(isset($my_company[$i]['admin_id']) && $my_company[$i]['admin_id'])
						{
							$query = $this->db->query("select admin_id from pms_admin_users  where username='".$my_company[$i]['admin_id']."'");					
							if ($query->num_rows() > 0)
							{
								$row = $query->row_array();
								$row1=$my_company[$i];
					...
					*/
					...
					{
						$data = $my_company[$i];
						$this->my_companymodel->insert_csv_records($data);
					}
					redirect('my_company');
				}
			}
		...
		$this->load->view('include/header',$this->data);
		$this->load->view('my_company/upload-csv',$this->data);
		$this->load->view('include/footer',$this->data);		
    }
	...
	function add()
	{
		...
		$this->data["type_list"] = $this->my_companymodel->type_list();
		$this->data["industry_list"] = $this->my_companymodel->industry_list();
		...
		{
			$this->form_validation->set_rules('company_name', 'my_company name', 'required');

			if ($this->form_validation->run() == TRUE)
			{
				$id=$this->my_companymodel->insert_record();
				redirect('my_company/?ins=1&'.$_SERVER['QUERY_STRING']);
			}
		...
		$this->load->view('include/header',$this->data);
		$this->load->view('my_company/addcompany',$this->data);				
		$this->load->view('include/footer',$this->data);
...
// edxit and update pages here 
	...
	function edit($id=null)
	{
		...
		if(!empty($id))
		{
			...
			$this->data['formdata']=$this->my_companymodel->get_single_company($id);			
		    ...
		    $this->editor($path, $width);
			$this->load->view('include/header',$this->data);
			$this->load->view('my_company/editcompany',$this->data);				
			$this->load->view('include/footer',$this->data);
		}
	}
	...
	function update()
	{
		...
		if($this->input->post('company_name'))
		{
			...
			$this->form_validation->set_rules('company_name', 'Company Name', 'required');
				...
				if ($this->form_validation->run() == TRUE)
				{
					$id=$this->my_companymodel->update_record($this->input->post('company_id'));
					redirect('my_company/?upd=1&'.$_SERVER['QUERY_STRING']);
				}else{
	...
	{
		redirect('my_company/?delete=stop');
			if(!empty($id))
			{
				
				if($this->is_related($id)){

					redirect('my_company/?del=2');
				}else{
	...
	function add_calls()
	{
		...
		{
			$job_id=$this->my_companymodel->add_calls();	
			
			if($this->input->post('create_task')!='')
			{
				$task_id=$this->my_companymodel->add_task();
			}
	...
	{
		$this->data["records"] = $this->my_companymodel->get_calls_history($this->input->post('company_id'));
		$output_str='
	...
	function add_jobs()
	{
		...
		if($this->input->post('company_id')!='')
		{
			$job_id=$this->my_companymodel->add_jobs();	
			$response = array(
			    'data' => '',
				'status'=>'success',
			);
	...
	function assign_requirement()
	{
		...
		$output_str='<table width="100%" border="1" cellpadding="3" cellspacing="3">
			  ...
			  $this->data["records"] = $this->my_companymodel->get_assignment_recruiter($this->input->post('company_id'));
	...
	function save_assignment()
	{
		...
		if($this->input->post('company_id')!='')
		{
				$company_name=$this->my_companymodel->get_company_name($this->input->post('company_id'));
				...
				{	
					...
					{
						 $return_val = $this->my_companymodel->add_recruiter_to_company($this->input->post('company_id'),$val);
						 if($comments[$val]!='')
						 {
							 $data = array(
							...
							$return_val = $this->my_companymodel->add_task_assignment($data);
						 }
					}
				}
	...
	public function getstate()
	{
		...
		{
			$data=array();
			$data["state_list"] = $this->my_companymodel->state_list_by_city($_POST['country_id']);
	...
	{ 
		...
		{
			$data=array();
			$data["city_list"] = $this->my_companymodel->city_list_by_state($_POST['state_id']);
	...
	function multiple_assignment()
	{
		...
		if(is_array($id_arr))
		{
			foreach ($id_arr as $key => $val)
			{
				...
				$this->my_companymodel->add_recruiter_to_company($val,$this->input->post('user_id'));
			}
		}
		redirect('my_company/?assigned=1');
	}		
}
?>
