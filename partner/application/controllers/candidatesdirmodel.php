<?php 
class Candidatesdirmodel extends CI_Model {
	var $table_name='';
	...
	function record_count($search_email,$search_name,$search_mobile,$cur_job_status,$reg_status,$skills,$level_id,$course_id,$spcl_id,$exp_years,$lead_source,$job_folder_id,$job_cat_id,$func_id, $desig_id, $city_id, $pref_city_id, $expected_ctc_from, $expected_ctc_to, $notice_period_from, $notice_period_to, $total_experience_from, $total_experience_to, $payment_status)  
	{
		$sql="select count(a.candidate_id)as total_rec  from pms_candidate a LEFT JOIN pms_candidate_job_search b on a.candidate_id=b.candidate_id ";
	
		$cond='';
		//$cond=" (g.current_ctc ='' or g.expected_ctc ='' or g.notice_period ='' or g.total_experience ='') ";

          if($expected_ctc_from!='')
		{
			if($cond!='')
				$cond.=" and b.expected_ctc >=". $expected_ctc_from;
			else
				$cond ="  b.expected_ctc >=". $expected_ctc_from;			
		}
		if($expected_ctc_to!='')
		{
			if($cond!='')
				$cond.=" and b.expected_ctc <=". $expected_ctc_to;
			else
				$cond ="  b.expected_ctc <=". $expected_ctc_to;			
		}
		
		if($notice_period_from!='')
		{
			if($cond!='')
				$cond.=" and b.notice_period >=". $notice_period_from;
			else
				$cond ="  b.notice_period >=". $notice_period_from;			
		}
		if($notice_period_to!='')
		{
			if($cond!='')
				$cond.=" and b.notice_period <=". $notice_period_to;
			else
				$cond ="  b.notice_period <=". $notice_period_to;			
		}
		
		if($total_experience_from!='')
		{
			if($cond!='')
				$cond.=" and b.total_experience >=". $total_experience_from;
			else
				$cond ="  b.total_experience >=". $total_experience_from;			
		}
		if($total_experience_to!='')
		{
			if($cond!='')
				$cond.=" and b.total_experience <=". $total_experience_to;
			else
				$cond ="  b.total_experience <=". $total_experience_to;			
		}
		...
		{
			if($cond!='')
				$cond.=" and b.total_experience >=". $exp_years ;
			else
				$cond ="  b.total_experience >=". $exp_years;			
		}
		...
		{			
			if($cond!='')
				$cond.=" and a.candidate_id in (select candidate_id from pms_candidate_to_industry where job_cat_id=".$job_cat_id.") ";
			else
				$cond.=" a.candidate_id in (select candidate_id from pms_candidate_to_industry where job_cat_id=".$job_cat_id.") ";			
		}
		
		if($func_id!='')
		{			
			if($cond!='')
				$cond.=" and a.candidate_id in (select candidate_id from pms_candidate_to_funcional where func_id=".$func_id.") ";
			else
				$cond.=" a.candidate_id in (select candidate_id from pms_candidate_to_funcional where func_id=".$func_id.") ";			
		}

		if($desig_id!='')
		{			
			if($cond!='')
				$cond.=" and a.candidate_id in (select candidate_id from pms_candidate_to_designation where desig_id=".$desig_id.") ";
			else
				$cond.=" a.candidate_id in (select candidate_id from pms_candidate_to_designation where desig_id=".$desig_id.") ";			
		}

		if($pref_city_id!='')
		{			
			if($cond!='')
				$cond.=" and a.candidate_id in (select candidate_id from pms_candidate_to_location where city_id=".$pref_city_id.") ";
			else
				$cond.=" a.candidate_id in (select candidate_id from pms_candidate_to_location where city_id=".$pref_city_id.") ";			
		}
		...
		{
			if($cond!='')
				$cond.=" and a.payment_status like '%".$payment_status."%' ";
			else
				$cond.=" a.payment_status like '%".$payment_status."%' ";
		}
	...
	function get_list($start,$limit,$search_email,$search_name,$search_mobile,$sort_by,$cur_job_status,$reg_status,$skills,$level_id,$course_id,$spcl_id,$exp_years,$lead_source,$job_folder_id,$job_cat_id,$func_id, $desig_id, $city_id, $pref_city_id, $expected_ctc_from, $expected_ctc_to, $notice_period_from, $notice_period_to, $total_experience_from, $total_experience_to, $payment_status)
	{
		$sql="SELECT a.candidate_id, a.first_name, a.last_name, a.username,a.reg_date,a.lead_opportunity, a.mobile,a.photo,a.reg_status,a.status_updated_on,a.client_cv_file, a.cv_file, b.current_ctc,b.expected_ctc,b.notice_period,b.total_experience,b.ctc_updated_on,c.cur_job_status_name ";
		...
		$sql.=", (select jp.organization from pms_candidate_job_profile jp where jp.candidate_id=a.candidate_id order by jp.present_job desc, jp.from_date desc limit 0,1) as current_organization ";
		...
		$sql.=", (select count(candidate_id) as total_job_profile from pms_candidate_job_profile jp where jp.candidate_id=a.candidate_id) as total_job_profile ";
		
		$sql.=", (select count(candidate_id) as total_edu from pms_candidate_education edu where edu.candidate_id=a.candidate_id) as total_edu ";
		...
		$sql.=", (select job_cat_name from pms_job_category jbc inner join pms_candidate_to_industry cti on jbc.job_cat_id=cti.job_cat_id where cti.candidate_id=a.candidate_id limit 0,1) as candidate_industry ";

		$sql.=", (select func_area from pms_job_functional_area fua inner join pms_candidate_to_funcional cfa on fua.func_id=cfa.func_id where cfa.candidate_id=a.candidate_id limit 0,1) as candidate_functional ";

		$sql.=", (select desig_name from pms_designation ds inner join pms_candidate_to_designation cds on ds.desig_id=cds.desig_id where cds.candidate_id=a.candidate_id limit 0,1) as candidate_role ";
					
		$sql.=" FROM ".$this->table_name." a LEFT JOIN pms_candidate_job_search b on a.candidate_id=b.candidate_id left join pms_curernt_job_status c on a. 	cur_job_status=c.cur_job_status ";
		...
		$sql.=" order by a.candidate_id desc, a.candidate_id desc limit ".$start.",".$limit;		
	...
	{	
		...
		$query = $this->db->query("select file_type,file_id from pms_candidate_files where candidate_id='".$id."'");
		if ($query->num_rows() > 0)
		{
			$row = $query->result_array();
		...
		$this->db->query("delete from pms_admin_candidates where candidate_id='".$id."'");	
		// delete from candidate adress
		$this->db->query("delete from pms_candidate_address where candidate_id='".$id."'");
		// delete from VISA approvals list
		$this->db->query("delete from pms_candidate_visa_approval where candidate_id='".$id."'");
		// delete from applications 
		$this->db->query("delete from pms_candidate_applications where candidate_id='".$id."'");
		// delete from cv files 
		$this->db->query("delete from pms_candidate_cvfile where candidate_id='".$id."'");
		// delete from education
		$this->db->query("delete from pms_candidate_education where candidate_id='".$id."'");
		// delete from candidate files uploaded
		$this->db->query("delete from pms_candidate_files where candidate_id='".$id."'");
		// delete from candidate follow up
		$this->db->query("delete from pms_candidate_followup where candidate_id='".$id."'");
		// delete from interviews
		$this->db->query("delete from pms_candidate_interviews where candidate_id='".$id."'");
		// delete from job profile
		$this->db->query("delete from pms_candidate_job_profile where candidate_id='".$id."'");
		// delete from notes 
		$this->db->query("delete from pms_candidate_notes where candidate_id='".$id."'");
		// delete from survey result
		$this->db->query("delete from pms_candidate_survey_result where candidate_id='".$id."'");
		// delete from certification
		$this->db->query("delete from pms_candidate_to_certification where candidate_id='".$id."'");
		// delete from skills 
		$this->db->query("delete from pms_candidate_to_skills where candidate_id='".$id."'");	
		// delete from skills primary 
		$this->db->query("delete from pms_candidate_to_skills_primary where candidate_id='".$id."'");				

		// delete from contracts 
		$this->db->query("delete from pms_candidate_contract where candidate_id='".$id."'");				

		// delete from job apps 
		$this->db->query("delete from pms_job_apps where candidate_id='".$id."'");		
		...
		// delete from domain 
		$this->db->query("delete from pms_candidate_otp where candidate_id='".$id."'");

		// delete from domain 
		$this->db->query("delete from pms_candidate_profile_assessment where candidate_id='".$id."'");

	// delete from domain 
		$this->db->query("delete from pms_candidate_projects where candidate_id='".$id."'");

		// delete from domain 
		$this->db->query("delete from pms_candidate_job_search where candidate_id='".$id."'");						

		// delete from domain 
		$this->db->query("delete from pms_candidate_language where candidate_id='".$id."'");			

		// delete from domain 
		$this->db->query("delete from pms_candidate_marks where candidate_id='".$id."'");			

		// delete from domain 
		$this->db->query("delete from pms_job_apps_cert where candidate_id='".$id."'");			

		// delete from domain 
		$this->db->query("delete from pms_candidate_visa_approval where candidate_id='".$id."'");			

		// delete from domain 
		$this->db->query("delete from pms_candidate_to_skills_secondary where candidate_id='".$id."'");			

		// delete from domain 
		$this->db->query("delete from pms_candidate_to_domain where candidate_id='".$id."'");			
		
		// delete ticket folllow ups
		$this->db->query("delete from pms_tickets_followup where ticket_id in (select ticket_id from pms_tickets where candidate_id='".$id."')");		
		// delete from tickets 
		$this->db->query("delete from pms_tickets where candidate_id='".$id."'");
		// delete from sms email history
		$this->db->query("delete from pms_email_sms_history where candidate_id='".$id."'");
		// delete from candidate designation.
		$this->db->query("delete from pms_candidate_to_designation where candidate_id='".$id."'");
		// delete from candidate messages.
		$this->db->query("delete from pms_candidate_messages where candidate_id='".$id."'");		
		// delete from actual candidate database.
		$this->db->query("delete from pms_candidate where candidate_id='".$id."'");
		return;
	}
   ...
   {
   		$query = $this->db->query('select a.*,f.country_name from pms_candidate_education a left join pms_country f on a.edu_country=f.country_id where a.candidate_id='.$candidate_id.' order by a.edu_year');
		return $query->result_array();
   }   
...
//  suggestion_logic start from here
    ...
    {
        //$query=$this->db->query("select a.*,b.*,c.*,d.*,e.*,f.* from pms_candidate_education a left join pms_education_level b on a.level_id=b.level_id left join pms_courses c on a.course_id=c.course_id left join pms_specialisation d on a.spcl_id=d.spcl_id left join pms_university e on a.univ_id=e.univ_id left join pms_course_type f on a.course_type_id=f.course_type_id  where candidate_id=".$candidate_id);
        
         $query=$this->db->query("select a.* from pms_candidate_education a where a.candidate_id=".$candidate_id." order by a.edu_year asc");
		return $query->result_array();
	}
...
function create_program_name($course_id)
   ...
   function interview_status_list()
	...
	function insert_csv_levels($data) 
	{
		$this->db->insert('pms_candidate_education', $data);
		return 1;
    }
	
	function insert_csv_pre_jobdata($data1) 
	{
		$this->db->insert('pms_candidate_job_search', $data1);
		return 1;
    }
	...
	function insert_csv_address($data) 
	{
...
//delte andd insert languages 
	...
	function update_role_designation_old($candidate_id, $desig_id)
	{
...
//STOP	
	...
	function qualification_level($candidate_id)
    {
        $query=$this->db->query("SELECT a.level_id,a.level_name FROM `pms_education_level` a inner join pms_candidate_to_edu_level b on a.level_id=b.level_id where b.candidate_id=".$candidate_id." order by a.level_name");
		$dropdowns = $query->result_array();
		$survey_result=array();
		foreach($dropdowns as $key => $val)
		{
	...
	function update_qualification_level($candidate_id, $level_id)
	{	
			$this->db->where('candidate_id',$candidate_id);
	...
	function get_pre_screening($candidate_id)
	{
	
		$query=$this->db->query("select a.first_name, a.mobile, a.payment_status, b.*,c.username, d.company_name FROM pms_candidate a left join pms_job_pre_screening b on a.candidate_id=b.candidate_id left join pms_admin_users c on b.admin_id=c.admin_id left join pms_company d on d.company_id=b.company_id where a.candidate_id=".$candidate_id);
		return $query->row_array();
	}
	
		
	
	function admin_recruiters_list()
	{
		$query = $this->db->query('select distinct admin_id, firstname from pms_admin_users order by firstname');
		$dropdowns = $query->result();
		$dropDownList['']='Select Recruiter Name';
		foreach($dropdowns as $dropdown)
		{
			 $dropDownList[$dropdown->admin_id] = $dropdown->firstname;
		}
	...
	{
		$query = $this->db->query('select distinct company_id, company_name from pms_company order by company_name');
		$dropdowns = $query->result();
	...
	function add_pre_screening($data){
	
		$this->db->where('candidate_id', $data['candidate_id']);			
		$this->db->delete('pms_job_pre_screening');
		$this->db->insert('pms_job_pre_screening',$data);
		return 1;
	}
	
	function update_pre_screening($data, $pre_screen_id, $candidate_id)
	{
			$this->db->where('pre_screen_id', $pre_screen_id);
			$this->db->where('candidate_id', $candidate_id);
			$this->db->update('pms_job_pre_screening',$data);
	}
	
	function update_payment_status($data, $candidate_id)
	{
