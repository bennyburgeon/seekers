...
class Jobarea extends CI_Controller {
	...
	{	
		...
		$config['base_url'] = $this->config->item('base_url')."jobarea/?sort_by=$sort_by&searchterm=$searchterm$query_str&job_cat_id=".$job_cat_id;
	...
	{	
		...
		if($this->input->post('func_area'))
		{
			...
			{				
				$id=$this->jobareamodel->insert_record();
			...
			if($this->input->post('func_area'))
			{
				...
				$this->form_validation->set_rules('func_area_dup', 'Functional area', 'callback_check_dups');
					...
					{						
						...
						$this->load->view('include/header');
						$this->load->view('jobarea/editjobarea',$data);	
						$this->load->view('include/footer');
					}
			}else
			{
