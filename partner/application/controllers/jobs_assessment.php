...
class Jobs_assessment extends CI_Controller {
	...
	{	
		...
		$this->data["functional"] = $this->jobs_assessmentmodel->fill_functional();
		...
		$this->data["job_location"] = $this->jobs_assessmentmodel->job_location();
	...
	function edit($id=null)
	{
		...
		$this->data["job_location"] = $this->jobs_assessmentmodel->job_location();
		$this->data["jobtype"] = $this->jobs_assessmentmodel->jobtype_list();
		$this->data['skill_list']=$this->jobs_assessmentmodel->get_parent_skills();
		$this->data['cerifications']=$this->jobs_assessmentmodel->get_cert();
		$this->data['domain']=$this->jobs_assessmentmodel->get_domain();
		...
		{
			$this->data["functional"] = $this->jobs_assessmentmodel->function_list_by_category($data['formdata']['job_cat_id']);
		}
		if($this->data['formdata']['job_skills'] !=0)
		{
		 $this->data['childskill']=$this->jobs_assessmentmodel->get_child_skills($data['formdata']['job_skills']);
		}
		*/
		...
		$path = '../js/ckfinder';
			...
			$this->load->view('jobs_assessment/editjob',$this->data);	
	...
	function manage($id=null)
	{
		...
		if(!empty($id))
		{
			...
			$this->data['formdata']= $this->jobs_assessmentmodel->get_job_complete($id);
			
			if(count($this->data['formdata'])==0)redirect('jobs_assessment');
			
			$this->data['company_info']=$this->jobs_assessmentmodel->get_company_info($this->data['formdata']['company_id']);
	...
	function send_shortlisted()
	{
		...
		{
			$job_details=$this->jobs_assessmentmodel->get_job($this->input->post('job_id'));
			$user_details=$this->jobs_assessmentmodel->get_user_info($_SESSION['vendor_session']);
...
//INSTRUCTION AND GUIDE LINES
	...
	function search_candidate($id=null)
	{
		...
		$this->data["interview_type"] = $this->interviewtypemodel->get_type_list();
		$this->data["int_status_id"] = $this->interviewstatusmodel->get_model_list();
		$this->data['job_folders']=$this->jobs_assessmentmodel->get_job_folders();
...
//		$headers.= "From: ".$email_array['from_name']." <".$email_array['email_from'].">\r\n";		
	...
	function add_candidate($job_id)
	{
		...
		{ 
			...
			{
				$this->jobs_assessmentmodel->addcandidate(array('candidate_id' => $id),$this->input->post('job_id'));
			}
