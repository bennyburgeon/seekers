<?php 
class Candidates_skill_summary extends CI_Controller {
	...
	function __construct()
	{
		...
		$this->load->model('candidates_skill_summarymodel');
	...
	{	
		...
		$this->data['total_rows']= $this->candidates_skill_summarymodel->record_count($this->data['search_name']);
		...
		$config['base_url'] = $this->config->item('base_url')."candidates_skill_summary/?sort_by=".$this->data['sort_by']."&limit=".$this->data['limit']."&search_name=".$this->data['search_name'];
		...
		$this->data["records"] = $this->candidates_skill_summarymodel->get_list($this->data['start'],$this->data['limit'],$this->data['search_name'],$this->data['sort_by']);
		
			
		$this->data['page_head'] = 'Candidates Skills based Report';				

	
		$this->load->view('include/header',$this->data);
		$this->load->view('candidates_skill_summary/list',$this->data);
	...
	public function sentmail($id = '',$username = '')
	{
		...
		$headers='From:'.$this->config->item('websites').'hr Solutions \r\n';
		$headers.='Reply-To:'.$this->config->item('websites').'hr.in\r\n';
		$headers.='X-Mailer: PHP/' . phpversion().'\r\n';
		$headers.= 'MIME-Version: 1.0' . "\r\n";
		$headers.= 'Content-type: text/html; charset=iso-8859-1 \r\n';
		mail($row['username'], $subject, $mail_body, $headers);
			
			redirect('candidates_skill_summary/');
			exit();
