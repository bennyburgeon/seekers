<?php 
class Education_summary extends CI_Controller {
	...
	function __construct()
	{
		...
		$this->load->model('educationsummary_model');
	...
	{	
		...
		$this->data['total_rows']= $this->educationsummary_model->record_count($this->data['search_name']);
		...
		$config['base_url'] = $this->config->item('base_url')."education_summary/?sort_by=".$this->data['sort_by']."&limit=".$this->data['limit']."&search_name=".$this->data['search_name'];
		...
		$this->data["records"] = $this->educationsummary_model->get_list($this->data['start'],$this->data['limit'],$this->data['search_name'],$this->data['sort_by']);
		...
		$this->load->view('education_summary/list',$this->data);
	...
	public function sentmail($id = '',$username = '')
	{
		...
		//echo "SELECT * FROM pms_candidate where md5(candidate_id) ='".$id."'" ;
		$row = $query->row_array();
		...
		$mail_body='<!DOCTYPE html>
			...
			<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="main-bg" style="background:#dcdcdc;">
			  ...
			  <tr>
				  <td align="center" valign="bottom" style="font:Bold 15px Arial, Helvetica, sans-serif; color:#67bd3c; padding-top:18px;"><span style="color:#3b3b3b;">Call Us :</span> Phone:+91 999 567 1980 </td>
			  </tr>
			</table>
			<!--  Call Part End-->
					...
					<td align="left" valign="top" bgcolor="#67bd3c" style="background:#fff;  padding:12px 20px 12px 20px;" class="borter-inner-bottom"><table width="100%" border="0" cellspacing="0" cellpadding="0">
					  <tr>
						<td align="left" valign="top" style="font:Normal 12px Arial, Helvetica, sans-serif; color:#67bd3c; padding:8px 0px 16px 4px; line-height:22px;"><p><strong>Dear '.$row['first_name'].',</strong><br />
							<p>
						  	Please click here to update your personal details:<br>http://<?php echo $this->config->item('websites'); ?>hr.in/login?candidate_id='.$id.'
						  </p>
						  <p>Thank You ! <br>
		...
		$headers='From: <?php echo $this->config->item('websites'); ?>hr \r\n';
		$headers.='Reply-To: athira@bournham.in\r\n';
		$headers.='X-Mailer: PHP/' . phpversion().'\r\n';
		$headers.= 'MIME-Version: 1.0' . "\r\n";
		$headers.= 'Content-type: text/html; charset=iso-8859-1 \r\n';
		mail($row['username'], $subject, $mail_body, $headers);
			
			redirect('education_summary/');
			exit();
