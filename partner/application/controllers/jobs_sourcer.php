...
class Jobs_sourcer extends CI_Controller {
	...
	{	
		...
		$config['base_url'] = $this->config->item('base_url')."jobs_sourcer/?sort_by=$sort_by&searchterm=$searchterm$query_str&job_status=$job_status&company_id=$company_id&job_priority=$job_priority";
		...
		$this->data['page_head'] = 'Manage Jobs || Sourcer';
	...
	{	
		...
		$this->data["jobcategory"] = $this->jobs_sourcermodel->fill_jobcategory();
	...
	function edit($id=null)
	{
		...
		$data["company"] = $this->jobs_sourcermodel->fill_company();
		$data["jobindustry"] = $this->jobs_sourcermodel->fill_jobindustry();
		$data["jobcategory"] = $this->jobs_sourcermodel->fill_jobcategory();
		$data["functional"] = array('' => 'Select Functional Role');

		$data["education"] = $this->jobs_sourcermodel->fill_education();
		$data["salary"] = $this->jobs_sourcermodel->fill_salary();
		$data["worklevel"]= $this->jobs_sourcermodel->fill_worklevel();
		$data["nationality"] = $this->countrymodel->country_list();
		$data["jobtype"] = $this->jobs_sourcermodel->jobtype_list();
		$data['skill_list']=$this->jobs_sourcermodel->get_parent_skills();
		$data['cerifications']=$this->jobs_sourcermodel->get_cert();
		$data['domain']=$this->jobs_sourcermodel->get_domain();
		...
		{
			$data["functional"] = $this->jobs_sourcermodel->function_list_by_category($data['formdata']['job_cat_id']);
		}
		if($data['formdata']['job_skills'] !=0)
		{
		 $data['childskill']=$this->jobs_sourcermodel->get_child_skills($data['formdata']['job_skills']);
		}
		...
		$path = '../js/ckfinder';
			...
			$this->editor($path, $width);
			$this->load->view('include/header');
			$this->load->view('jobs_sourcer/editjob',$data);	
			$this->load->view('include/footer');
	}	
	...
	function manage($id=null)
	{
		...
		if(!empty($id))
		{
			...
			$this->data['formdata']=$this->jobs_sourcermodel->get_job($id);
			if(count($this->data['formdata'])==0)redirect('jobs_sourcer?error=1');
	...
	function send_shortlisted()
	{
		...
		{			
			...
			$list_shortlisted=$this->jobs_sourcermodel->get_shortlisted_client($this->input->post('job_id'));
			...
			{
				$cv_hash=md5($val['candidate_id'].$val['job_app_id']);
...
// upcoming contracts
	function upcoming_contracts($id=null)
	{
		...
		//paging starts here 
		$config['base_url'] = $this->config->item('base_url')."jobs_sourcer/upcoming_contracts/".$id."/?";
		$config['page_query_string'] = TRUE;
		$config['total_rows'] = $this->data["total_rows"];
		$config['query_string_segment'] = 'rows';
		$config['per_page'] =50;
...
//INSTRUCTION AND GUIDE LINES
	...
	function search_candidate($id=null)
	{
		...
		//paging starts here 
		$config['base_url'] = $this->config->item('base_url')."jobs_sourcer/search_candidate/".$id."/?";
		$config['page_query_string'] = TRUE;
		$config['total_rows'] = $this->data["total_rows"];
		$config['query_string_segment'] = 'rows';
		$config['per_page'] =50;
	...
	function send_mass_mail($id=null)
	{
		...
		//paging starts here 
		$config['base_url'] = $this->config->item('base_url')."jobs_sourcer/send_mass_mail/".$id."/?";
		$config['page_query_string'] = TRUE;
		$config['total_rows'] = $this->data["total_rows"];
		$config['query_string_segment'] = 'rows';
		$config['per_page'] =50;
	...
	function shortlist_jquery()
	{
		...
		if($this->input->get('job_app_id')!='' && $this->input->get('candidate_id')!='')
		{
			...
			$this->jobs_sourcermodel->shortlist2();
			
			$shortlisted =$this->jobs_sourcermodel->get_shortlisted($id);
			$this->db->where('job_id', $id);
			$query=$this->db->get('pms_jobs');
          ...
          <th>Action</th>
					...
					foreach($shortlisted as $candidate){
						...
						$html.='<tr>
								  <td width="30%"><a href="'.base_url().'candidates_all/summary/'.$candidate['candidate_id'].'" target="_blank">'.$candidate['first_name'].' '.$candidate['last_name'].'</a></td>
								 <td width="30%">'.$candidate['mobile'].'</td>
								  <td width="30%">'.$candidate['username'].'</td>								  
								 <td width="10%"> <a href="javascript:;"  onclick="interview('.$candidate['candidate_id'].','.$candidate['job_app_id'].','.$formdata['job_id'].')" class="btn btn-primary btn-xs" >Interview</a>|<a href="javascript:;"  data-url="'.base_url().'jobs_sourcer/delete_shortlisted_candidate/?job_app_id='.$candidate['job_app_id'].'&candidate_id='.$candidate['candidate_id'].'&job_id='.$formdata['job_id'].'"  id="delete_shortlisted_candidate" class="btn btn-danger btn-xs">X</a></td>			
							   </tr>';		
					}
					
			$html .='<tr>
          <td colspan="4" align="right">               
   <p class="text-right" style="margin-top: 1px;"><a href="'.$this->config->site_url().'/jobs_sourcer/addcandidate/'.$formdata['job_id'].'" class="btn btn-primary">Send To Client</a></p></td></tr></tbody> </table></td> ';
	...
	function addinterview($id='null')
	{
		...
		{
			$job_id=$this->jobs_sourcermodel->add_interview();	
		...
		if($this->input->get('job_app_id')!='' && $this->input->get('candidate_id')!='')
		{
			...
			$this->load->view('jobs_sourcer/addinterview',$this->data);	
	...
	function addinterview_jqery()
	{
		...
		if($this->input->post('job_app_id')!='' && $this->input->post('candidate_id')!='' && $this->input->post('title')!='' )
		{
			...
			$html.=	'<tr>
						  ...
						  <td><a href="javascript:;"  data-url="'.base_url().'jobs_sourcer/edit_interview/?job_app_id='.$interview['job_app_id'].'&candidate_id='.$interview['candidate_id'].'&job_id='.$formdata['job_id'].'"  id="edit_interview" class="btn btn-primary btn-xs">Change</a> |<a href="javascript:;"  data-url="'.base_url().'jobs_sourcer/delete_interview_candidate/?job_app_id='.$interview['job_app_id'].'&candidate_id='.$interview['candidate_id'].'&job_id='.$formdata['job_id'].'"  id="delete_interview_candidate" class="btn btn-danger btn-xs">X </a> | <a href="'.base_url().'candidates_all/summary/'.$interview['candidate_id'].'" target="_blank" class="btn btn-info btn-xs"> Profile </a> | <a href="javascript:;"  onclick="select_candidate('.$interview['candidate_id'].','.$interview['job_app_id'].','.$formdata['job_id'].')" class="btn btn-primary btn-xs"> Select </a></td></tr>';
   ...
   <p class="text-right" style="margin-top: 1px;"><a href="'.$this->config->site_url().'/jobs_sourcer/addcandidate/'.$formdata["job_id"].'" class="btn btn-primary">Send To Client</a></p></td>          
      </tr>  </tbody> </table> ';
	...
	function delete_interview_candidate()
	{
		...
		if($this->input->get('job_app_id')!='' && $this->input->get('candidate_id')!='')
		{
			...
			else
			{
					...
					$interview_list=$this->jobs_sourcermodel->get_interview_list($job_id);
                        ...
                        <th width="26%">Action</th>
						  ...
						  foreach($candidates_selected as $selected){
							  ...
							  $html.='<tr>
										  ...
										  <td><a href="#" data-reveal-id="interview" data-animation="fade" class="btn btn-primary btn-xs">Change</a> | <a href="javascript:;"  data-url="'.base_url().'jobs_sourcer/delete_selectedcandidate/?job_app_id='.$selected['app_id'].'&candidate_id='.$selected['candidate_id'].'&job_id='.$formdata['job_id'].'"  id="delete_selected_candidate" class="btn btn-danger btn-xs">X </a>|<a href="'.base_url().'candidates_all/summary/'.$selected['candidate_id'].'" class="btn btn-info btn-xs" target="_blank"> Profile </a> | <a href="javascript:;" onclick="issue_offer('.$formdata['job_id'].','.$selected['app_id'].','.$selected['candidate_id'].');" id="issue_offer" class="btn btn-info btn-xs"> Issue Offer </a></td>';	
					 ...
					 foreach($contracts_ending as $contract)
					 {
						...
						$html2.='<tr>
                              ...
                              <td width="13%">'. date('d-m-Y',strtotime($contract['end_date'])).'</td>
									  <td width="13%"> <a href="javascript:;" data-url="'.base_url().'jobs_sourcer/add_to_job/?candidate_id='.$contract['candidate_id'].'&job_id='.$formdata['job_id'].'"  id="add_to_job"  > Add to Job </a></td>
									  </tr>';
		...
		if(!empty($get_cert_attest)){
			...
			$html2 =	'<td colspan="2" align="center" valign="top">
				...
				$html2.='<tr>
						  ...
						  <td><a href="#" data-reveal-id="interview" data-animation="fade">Change</a> | <a href="javascript:;"  data-url="'.base_url().'jobs_sourcer/delete_attest/?job_id='.$formdata['job_id'].'&app_id='.$cert_attest['app_id'].'&candidate_id='.$cert_attest['candidate_id'].'&cert_id='.$cert_attest['cert_id'].'" id="delete_attest" >X </a>|<a href="'.base_url().'candidates_all/summary/'.$cert_attest['candidate_id'].'" target="_blank"> Profile </a> | <a href="javascript:;" onclick="create_visa('.$formdata['job_id'].','.$cert_attest['app_id'].','.$cert_attest['candidate_id'].');"> Visa Details</a>  </td>';
		...
		if($this->input->post('placement_id')!='' && $this->input->post('app_id')!='' && $this->input->post('candidate_id')!='' && $this->input->post('invoice_date')!='')
		{
		   ...
		   $html.=' <tr>
					  ...
					  <td width="11%">'.$client_candidate.'</td>
                       <td><a href="javascript:;"  data-url="'.base_url().'jobs_sourcer/delete_invoice/?job_id='.$formdata['job_id'].'&placement_id='.$invoice['placement_id'].'&invoice_id='.$invoice['invoice_id'].'"  id="delete_invoice_candidate" class="btn btn-danger btn-xs" >X</a></td></tr>';
	...
	function get_invoice_list()
	{
		...
		if($this->input->get('placement_id')!='')
		{
		   ...
		   $html.=' <tr>
                      ...
                      <td width="11%">'.$invoice_status.'</td>
					   <td width="11%">'.$client_candidate.'</td>
                       <td><a href="'.base_url().'jobs_sourcer/create_invoice/'.$formdata['job_id'].'/?placement_id='.$invoice['placement_id'].'&invoice_id='.$invoice['invoice_id'].'"> &nbsp;Edit&nbsp;</a>|<a href="javascript:;"  data-url="'.base_url().'jobs_sourcer/delete_invoice/?job_id='.$formdata['job_id'].'&placement_id='.$invoice['placement_id'].'&invoice_id='.$invoice['invoice_id'].'"  id="delete_invoice_candidate" >Delete</a>|<a href="'.base_url().'candidates_all/summary/'.$invoice['candidate_id'].'" target="_blank">Profile</a></td></tr>';
			...
			{					
					...
					foreach($applied as $candidate)
					{
						...
						$html.='<tr>
								  ...
								  <td width="25%"><a href="javascript:;"  data-url="'.base_url().'jobs_sourcer/shortlist2/?job_app_id='.$candidate['job_app_id'].'&candidate_id='.$candidate['candidate_id'].'&job_id='.$formdata['job_id'].'"  id="shortlisted_candidate" > Short List </a> | <a href="javascript:;"  data-url="'.base_url().'jobs_sourcer/delete_applied_candidate/?job_app_id='.$candidate['job_app_id'].'&candidate_id='. $candidate['candidate_id'].'&job_id='.$formdata['job_id'].'"  id="delete_applied_candidate" > Delete </a></td>
			...
			else
			{
					...
					foreach($shortlisted as $candidate){
						...
						$html.='<tr>
								  <td width="44%"><a href="'.base_url().'candidates_all/summary/'.$candidate['candidate_id'].'" target="_blank">'.$candidate['first_name'].' '.$candidate['last_name'].'</a></td>
								  <td width="31%">'.$candidate['skills'].'</td>          
								 <td width="25%"> <a href="javascript:;"  onclick="interview('.$candidate['candidate_id'].','.$candidate['job_app_id'].','.$formdata['job_id'].')" >Interview</a>|<a href="javascript:;"  data-url="'.base_url().'jobs_sourcer/delete_shortlisted_candidate/?job_app_id='.$candidate['job_app_id'].'&candidate_id='.$candidate['candidate_id'].'&job_id='.$formdata['job_id'].'"  id="delete_shortlisted_candidate" >Remove</a></td>
						 ...
						 <tbody >					
						  ...
						  foreach($candidates_selected as $selected){
							  ...
							  $html.='<tr>
										  ...
										  <td width="11%">&nbsp;</td>
										  <td width="11%">'.$selected['feedback'].'</td>
										  <td><a href="javascript:;"  data-url="'.base_url().'jobs_sourcer/delete_selectedcandidate/?job_app_id='.$selected['app_id'].'&candidate_id='.$selected['candidate_id'].'&job_id='.$formdata['job_id'].'"  id="delete_selected_candidate" >Remove </a>| <a href="javascript:;" onclick="issue_offer('.$formdata['job_id'].','.$selected['app_id'].','.$selected['candidate_id'].');" id="issue_offer"> Issue Offer </a>|<a href="'.base_url().'candidates_all/summary/'.$selected['candidate_id'].'" target="_blank"> Profile </a> </td>';
