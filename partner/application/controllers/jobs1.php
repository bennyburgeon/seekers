...
//INSTRUCTION AND GUIDE LINES
	...
	function addcandidate($id=null)
	{
		...
		//paging starts here 
		$config['base_url'] = $this->config->item('base_url')."jobs/addcandidate/".$id."/?";
		$config['page_query_string'] = TRUE;
		$config['total_rows'] = $this->data["total_rows"];
		$config['query_string_segment'] = 'rows';
		$config['per_page'] =50;
          ...
          <th>Action</th>
					...
					foreach($shortlisted as $candidate){
						...
						$html.='<tr>
								 ...
								 <td width="10%"> <a href="javascript:;"  onclick="interview('.$candidate['candidate_id'].','.$candidate['job_app_id'].','.$formdata['job_id'].')" class="btn btn-primary btn-xs" >Interview</a>|<a href="javascript:;"  data-url="'.base_url().'jobs/delete_shortlisted_candidate/?job_app_id='.$candidate['job_app_id'].'&candidate_id='.$candidate['candidate_id'].'&job_id='.$formdata['job_id'].'"  id="delete_shortlisted_candidate" class="btn btn-danger btn-xs">X</a></td>			
							   </tr>';		
					}
