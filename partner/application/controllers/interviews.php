...
class Interviews extends CI_Controller {
	...
	function index(){
		...
		$config['base_url'] = $this->config->item('base_url')."interviews/?sort_by=$sort_by&searchterm=$searchterm$query_str";
	...
	function calendar(){
		...
		$this->data['page_head'] = 'Interview list';
		$this->load->view('include/header');
		$this->load->view('interview/calendar',$this->data);				
		$this->load->view('include/footer');		
	}	
	...
	function calendar_ci(){
		...
		$data=$this->interview_model->get_current_schedule(1,$this->data['cur_month'],$this->data['cur_year']);
		...
		$this->data['page_head'] = 'Interview List';
		//$this->load->view('include/header');
		$this->load->view('interview/ci_calendar',$this->data);				
		//$this->load->view('include/footer');		
	}	
	...
	function calendar_ci_open(){
		...
		$this->data['page_head'] = 'Interview List';
		$this->load->view('include/header');
		$this->load->view('interview/open_cal',$this->data);				
		$this->load->view('include/footer');		
	}	
		...
		if($this->input->post("avail_id")!='')
		{
			...
			if(is_array($avail_id))
			{
				...
				foreach ($avail_id as $key => $val)
 				{
					$this->interview_model->update_shift_vacancy($candidate_id,$val,$month,$year);
				}
