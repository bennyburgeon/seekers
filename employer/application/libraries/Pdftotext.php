...
class  Pdftotext 	extends PdfObjectBase
   {
	...
	private function  __load ( $filename, $contents, $user_password = false, $owner_password = false )
	   {
		...
		for  ( $i = 0, $count = count ( $matches [ 'object_id' ] ) ; $i  <  $count ; $i ++ )
		   {
			$object_number			=  $matches [ 'object_id' ] [$i] ;
			$object_data			=  $matches [ 'object'    ] [$i] ;
								...
								for  ( $i = 1 ; $i  <  $length ; $i ++ )
								   {
									...
									if  ( $text [$i]  !=  '\\' )
									...
									else 
									   {
										...
										if  ( $text [$i]  <  '0'  ||  $text [$i]  >  '7' )
											$ch1	=  $this -> __process_escaped_character ( $text [$i] ) ;
										else
										   {
											$oct		=  '' ;
											$digit_count	=  0 ;
									...
									if  ( $text [$i]  != '\\' )
									...
									else
									   {
										...
										if  ( $text [$i]  <  '0'  ||  $text [$i]  >  '7' )
											$ch2	=  $this -> __process_escaped_character ( $text [$i] ) ;
										else
										   {
											$oct		=  '' ;
											$digit_count	=  0 ;
									...
									if  ( $newchar  ==  '\\' )
									   {
										$newchar		=  $this -> __process_escaped_character ( $text [ $i + 2 ] ) ;
							...
							if  ( ! $handled )
							   {
									...
									// 10 and 13
										...
										// Escaped character
										if  ( $ch  <  '0'  ||  $ch  >  '7' )
											$ch		=  $this -> __process_escaped_character ( $ch ) ;
	...
	// __process_escaped_character -
	//	Text specified between parentheses, either as plain text or as codes using a Unicode CMAP, can be escaped through a backslash ;
	//	For example, closing parentheses will be expressed as "\)", newlines as "\n", etc.
	//	This function returns the appropriate code corresponding to the character after the backslash.
	private function  __process_escaped_character ( $ch )
	   {
		switch ( $ch ) 
		   {
		...
		switch ( strtolower ( $encryption_data_match [ 'mode' ] ) )
		   {
			...
			default :
				if  ( self::$DEBUG  >  1 )
					error ( new PdfToTextDecodingException ( "Unhandled encryption mode '{$encryption_data [ 'mode' ]}'", $encrypt_object_id ) ) ;
	...
	public function  Add ( $object_id, $font_definition )
	   {
		...
		$this -> Fonts [ $object_id ]	=  new  PdfTexterFont ( $object_id, $cmap_id, $font_type, $secondary_cmap_id ) ;
...
class  PdfTexterFont		extends PdfObjectBase
   {
	...
	public function  __construct ( $resource_id, $cmap_id, $font_type, $secondary_cmap_id = null )
	   {
		parent::__construct ( ) ;
		$this -> Id				=  $resource_id ;
	...
	public function  __construct ( $object_id, $definitions )
	   {
		...
		if ( $max_found_char_width  !=  $this -> HexCharWidth )
		   {
			if  ( PdfToText::$DEBUG )
