...
class Jobs extends CI_Controller {
	...
	{	
		...
		//print_r($_SESSION['company_session']);
		//exit();
	...
	{	
		...
		$this->data["job_cat_list"]         = $this->jobsmodel->fill_jobindustry();

		if($this->data["formdata"]["job_cat_id"]!='0' && $this->data["formdata"]["job_cat_id"]!='')
		{
			 $this->data["func_list"]=$this->jobsmodel->get_functional_by_industry($this->data["formdata"]["job_cat_id"]);
		}else{		
			$this->data["func_list"] = $this->data["func_list"] = $this->jobsmodel->all_func_list();
		}

		if($this->data["formdata"]["func_id"]!='0' && $this->data["formdata"]["func_id"]!='')
		{
			 $this->data["desig_list"]=  $this->jobsmodel->get_designation_by_function($this->data["formdata"]["func_id"]);
		}else{		
			$this->data["desig_list"] = $this->jobsmodel->all_designation();
		}	

		if($this->data["formdata"]["desig_id"]!='0' && $this->data["formdata"]["desig_id"]!='')
		{
			 $this->data["skill_list"]=  $this->jobsmodel->get_skills_by_designation($this->data["formdata"]["desig_id"]);
		}else{		
			$this->data["skill_list"] = $this->jobsmodel->all_skills();
		}
		...
		$this->data["nationality"]          = $this->jobsmodel->country_list();
		...
		if($this->input->post('job_title'))
		{
			...
			if ($this->form_validation->run() == TRUE)
			{
				...
				$job_skill=$this->jobsmodel->insert_skill_details($id,$this->input->post('skill_id'));
				...
				'show_company_details'=> $this->input->post('show_company_details'),
	...
	function edit($id=null)
	{
		...
		//print_r($this->data['job_skills']);exit();
	...
	public function get_designation_by_function()
	{
		...
		{
			$data=array();
			$data["desig_list"] = $this->jobsmodel->get_designation_by_function($_POST['func_id']);
			$data = array('success' => true, 'desig_list' => $data["desig_list"]);
		}else{
			$data=array();
			$data["desig_list"] = $this->jobsmodel->all_designation();
			$data = array('success' => true, 'desig_list' => $data["desig_list"]);
		}
	...
	public function get_skills_by_designation()
	{
		...
		{
			$data=array();
			$data["skill_list"] = $this->jobsmodel->get_skills_by_designation($_POST['desig_id']);
			$data = array('success' => true, 'skill_list' => $data["skill_list"]);
		}else{
			$data=array();
			$data["skill_list"] = $this->jobsmodel->all_skills();
			$data = array('success' => true, 'skill_list' => $data["skill_list"]);
		}
		echo json_encode($data);
	}	
	...
	function child_skill()
	{
		...
		//~ $id=1;
		$this->data['skillset']=$this->jobsmodel->get_child_skills();
	...
	function send_shortlisted()
	{
		...
		{
			$job_details=$this->jobsmodel->get_job($this->input->post('job_id'));
			$user_details=$this->jobsmodel->get_user_info($_SESSION['company_session']);
	...
	function client_cv()
	{
		...
		if($this->input->get('candidate_id')!='' && $this->input->get('job_app_id')!='')
		{
			...
			*/
			...
			redirect($this->config->item('client_cv_preview').'profile_rms?candidate_id='.md5($candidate_id).'&job_app_id='.$job_app_id);
//			echo $html_profile;
//			exit();
		}else
		{
...
//INSTRUCTION AND GUIDE LINES
	...
	function search_candidate($id=null)
	{
		...
		$this->data['job_cat_list']       =$this->jobsmodel->industry_list();
		$this->data['func_area_list']     =$this->jobsmodel->functional_list();
		$this->data['desig_list']         =$this->jobsmodel->desig_list();
			...
			if($this->input->post('candidate_source')==1)
			{
				...
				{				
					...
					foreach($applicants_list as $key => $job_app)
					{
						...
						$job_app['admin_id']=$_SESSION['company_session'];			
...
//DELETE TICKET DETAILS
	...
	{		
		...
		{
			$data["func_list"] = $this->jobsmodel->function_list_by_category($_POST['job_cat_id']);
			$data = array('success' => true, 'func_list' => $data["func_list"]);
		}else
		{
			$data["func_list"] = $this->jobsmodel->all_func_list();
			$data = array('success' => true, 'func_list' => $data["func_list"]);
		}
		echo json_encode($data);		
	}
