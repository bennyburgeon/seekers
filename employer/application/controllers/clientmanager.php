...
class Clientmanager extends CI_Controller 
{
	...
	function __construct()
	{
		...
		//$_SESSION['company_session']=1;
		$this->load->model('statemodel');
		$this->load->model('countrymodel');
		$this->load->model('citymodel');
		$this->load->model('clientmanagermodel');
	...
	{	$this->load->library('pagination');
		...
		$this->data['total_rows']= $this->clientmanagermodel->record_count($searchterm,$ind_id,$flp_status,$company_priority,$company_rating,$status,$flp_next_date,$date_added,$user_id,$unassigned);
		...
		$config['base_url'] = $this->config->item('base_url')."clientmanager/?sort_by=$sort_by&searchterm=$searchterm$query_str&ind_id=".$ind_id."&flp_status=$flp_status&company_priority=$company_priority&company_rating=$company_rating&status=$status&flp_next_date=$flp_next_date&date_added=$date_added&user_id=$user_id&unassigned=$unassigned";
		...
		$this->data["records"] = $this->clientmanagermodel->get_list($start,$limit,$searchterm,$ind_id,$flp_status,$sort_by,$company_priority,$company_rating,$status,$flp_next_date,$date_added,$user_id,$unassigned);

		$this->data["industry_list"] = $this->clientmanagermodel->industry_list_search();
		...
		*/	
		...
		$this->data["admin_list"]=$this->clientmanagermodel->admin_list();
		$this->data["bde_list"]=$this->clientmanagermodel->bde_list();
		...
		$this->load->view('clientmanager/companylist',$this->data);
    ...
    {    
					...
					while (!feof($file)) 
					{
						...
						else
						{
							if(!empty($company_data))
							{
								...
								$clientmanager[] = $company_row;
							}
						}
					}
					
					for($i=0;$i<count($clientmanager);$i++)
					{
						$data = $clientmanager[$i];
						$this->clientmanagermodel->insert_csv_records($data);
					}
		...
		$this->load->view('include/header',$this->data);
		$this->load->view('clientmanager/upload-csv',$this->data);
		$this->load->view('include/footer',$this->data);		
    }
	...
	function add()
	{
		...
		$this->data["type_list"] = $this->clientmanagermodel->type_list();
		$this->data["industry_list"] = $this->clientmanagermodel->industry_list();
		...
		$this->data["country_list"] = $this->clientmanagermodel->country_list();
		
		if($this->input->post('company_name'))
		{
			$this->form_validation->set_rules('company_name', 'clientmanager name', 'required');

			if ($this->form_validation->run() == TRUE)
			{
				$id=$this->clientmanagermodel->insert_record();
				redirect('clientmanager/?ins=1');
			}
		...
		$this->load->view('include/header',$this->data);
		$this->load->view('clientmanager/addcompany',$this->data);				
		$this->load->view('include/footer',$this->data);
...
// edxit and update pages here 
	...
	function edit($id=null)
	{
		...
		if(!empty($id))
		{
			...
			$this->data['formdata']=$this->clientmanagermodel->get_single_company($id);			
		    ...
		    $this->editor($path, $width);
			$this->load->view('include/header',$this->data);
			$this->load->view('clientmanager/editcompany',$this->data);				
			$this->load->view('include/footer',$this->data);
		}
	}
	...
	function update()
	{
		...
		if($this->input->post('company_name'))
		{
			...
			$this->form_validation->set_rules('company_name', 'Company Name', 'required');
				...
				if ($this->form_validation->run() == TRUE)
				{
					$id=$this->clientmanagermodel->update_record($this->input->post('company_id'));
					redirect('clientmanager/?upd=1');
				}else{
	...
	{
		redirect('clientmanager/?delete=stop');
			if(!empty($id))
			{
				
				if($this->is_related($id)){

					redirect('clientmanager/?del=2');
				}else{
	...
	function add_calls()
	{
		...
		{
			$job_id=$this->clientmanagermodel->add_calls();	
			
			if($this->input->post('create_task')!='')
			{
				$task_id=$this->clientmanagermodel->add_task();
			}
	...
	{
		$this->data["records"] = $this->clientmanagermodel->get_calls_history($this->input->post('company_id'));
		$output_str='
	...
	{
		$row = $this->clientmanagermodel->get_all_contacts($this->input->post('company_id'));
		$output_str='';
	...
	function add_jobs()
	{
		...
		if($this->input->post('company_id')!='')
		{
			$job_id=$this->clientmanagermodel->add_jobs();	
			$response = array(
			    'data' => '',
				'status'=>'success',
			);
	...
	function assign_requirement()
	{
		...
		$output_str='<table width="100%" border="1" cellpadding="3" cellspacing="3">
			  ...
			  $this->data["records"] = $this->clientmanagermodel->get_assignment_recruiter($this->input->post('company_id'));
	...
	function save_assignment()
	{
		...
		if($this->input->post('company_id')!='')
		{
				$company_name=$this->clientmanagermodel->get_company_name($this->input->post('company_id'));
				...
				{	
					...
					{
						 $return_val = $this->clientmanagermodel->add_recruiter_to_company($this->input->post('company_id'),$val);
						 if($comments[$val]!='')
						 {
							 $data = array(
							"task_title"          => $comments[$val].' | Company -'.$company_name.'| Assigned by - '.$_SESSION['company_firstname'],
							"company_id"          => $this->input->post('company_id'),
							"start_date"          => date('Y-m-d'),
							"due_date"            =>date('Y-m-d',strtotime("+7 day")),
							"admin_id"            => $val,
							"assigned_by"         => $_SESSION['company_session'],
							"task_priority_id"    => '1',
							"task_status_id"      => '1',
							"task_desc"           => $company_name.'<br>'.$comments[$val].'<br><br><br>'.$_SESSION['company_firstname'],
							"status"              => 1				
							);
							
							$return_val = $this->clientmanagermodel->add_task_assignment($data);
						 }
					}
				}
	...
	public function getstate()
	{
		...
		{
			$data=array();
			$data["state_list"] = $this->clientmanagermodel->state_list_by_city($_POST['country_id']);
	...
	{ 
		...
		{
			$data=array();
			$data["city_list"] = $this->clientmanagermodel->city_list_by_state($_POST['state_id']);
