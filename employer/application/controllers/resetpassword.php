...
class Resetpassword extends CI_Controller {
	...
	public function index()
	{
		...
		if($this->input->post('user_id'))
		{
			 ...
			 $this->db->update('pms_companyuser_password_change', array('status'=>0), array('md5(user_id)' =>$this->input->post('user_id')));
			 $this->db->update('pms_company_users', $array,array('md5(user_id)' => $this->input->post('user_id')));
			 redirect('login');
		}
		...
		$this->data['pass_status']='';
		$query = $this->db->query("select * from pms_companyuser_password_change where md5(user_id)='".$this->data['user_id']."'");
	...
	public function sendpasswordlink()
	{
		...
		{
			$query = $this->db->query("SELECT * from pms_company_users where email='".$this->input->post('email')."'");
			if($query->num_rows() > 0)
			{	
				$row = $query->row_array();	
				$unique_id = md5(uniqid($row["user_id"], true));
				$email = $this->input->post('email');			
				$id = md5($row["user_id"]);		
				$query1 = $this->db->query("SELECT * from pms_companyuser_password_change where md5(user_id)='".$id."'");
				
				if($query1->num_rows() > 0)	
				{
					$row1 = $query1->row_array();	
					$this->db->where('md5(user_id)', $id);
					$this->db->delete('pms_companyuser_password_change'); 
				}	
				...
				$this->db->insert('pms_companyuser_password_change', $array);	
	...
	{
        //echo $id." ".$email; exit();
		$query = $this->db->query("SELECT * FROM pms_company_users where md5(user_id) ='".$id."'");
		
		$row = $query->row_array();
		$subject = ' Recover Password ';
		
		$mail_body='<!DOCTYPE html>
			<head><title>'.$this->config->item('web_site').' - Reset Password</title>
					...
					<td align="left" valign="top" bgcolor="#67bd3c" style="background:#fff;  padding:12px 20px 12px 20px;" class="borter-inner-bottom"><table width="100%" border="0" cellspacing="0" cellpadding="0">
					  <tr>
						<td align="left" valign="top" style="font:Normal 12px Arial, Helvetica, sans-serif; color:#67bd3c; padding:8px 0px 16px 4px; line-height:22px;"><p><strong>Dear '.$row['firstname'].',</strong><br />
							...
							Please click here to change your password:<br>'.base_url().'resetpassword/?user_id='.$id.'
						  </p>
						  <p>Thank You ! <br>
			...
			<!--  Header Part End-->					  
				  ...
				  <tr>
					...
					<table width="600" border="0" cellspacing="0" cellpadding="0" class="main">
					 ...
					 <p style="text-align:center; font-weight:bold;">'.$this->config->item('website').'. Your Recruitment Solutions Provider.</p><br />
			
			<br />
			<center>
			Company Name & Address</center></p></td>
					  </tr>
					  <tr>
		...
		$this->email->from($this->config->item('email_from'),$name);
		$this->email->to($email);
