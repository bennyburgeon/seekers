...
class Projectsmodel extends CI_Model{
	...
	{
		$query=$this->db->query("select admin_id,firstname from ".$this->project_admin_users." where status=1 order by firstname" );
		$customers_list = $query->result();
	...
	{
		$query = $this->db->query("SELECT a.user_id,b.firstname from ".$this->project_team." a JOIN ".$this->project_admin_users." b on a.user_id=b.admin_id where a.project_id=".$id);
		$dropDownList = array();
		$customers_list = $query->result();
		foreach($customers_list as $dropdown)
		{
