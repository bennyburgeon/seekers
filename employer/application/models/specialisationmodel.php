...
class Specialisationmodel extends CI_Model {
    ...
    {
		$this->table_name='pms_specialisation';
	...
	{
		$query = $this->db->query('select spcl_id, spcl_name from pms_specialisation where course_id='.$course_id.' order by spcl_name');
		$dropdowns = $query->result();
			 ...
			 foreach ($delete_rec as $key => $val)
 				{
					...
					$this->db->delete('pms_specialisation'); 
				}
			return true;
		}else
		{
