...
class Candidateallmodel extends CI_Model {
	...
	function record_count($search_name,$search_email,$search_mobile,$cur_employer, $cur_job_status,$lead_source,$job_folder_id,$job_cat_id,$func_id,$desig_id, $skill_id, $level_id,$course_id,$spcl_id, $exp_years_from, $exp_years_to, $salary_from, $salary_to, $marital_status, $visa_type, $gender, $driving_license_country, $nationality, $country_id, $state_id, $city_id, $age_from, $age_to, $lang_id) 
	{
		...
		if($cond!='')
			$cond.=" and a.candidate_id in (select ja.candidate_id from pms_job_apps ja inner join pms_jobs pj on ja.job_id=pj.job_id inner join pms_job_apps_shortlisted jas on ja.job_app_id=jas.app_id where pj.company_id=" .$_SESSION['company_id'].")";
		else
				$cond.=" a.candidate_id in (select ja.candidate_id from pms_job_apps ja inner join pms_jobs pj on ja.job_id=pj.job_id inner join pms_job_apps_shortlisted jas on ja.job_app_id=jas.app_id where pj.company_id=" .$_SESSION['company_id'].")";
	...
	{
		$sql="SELECT a.candidate_id, a.cur_job_status , a.first_name, a.last_name, a.username,a.reg_date,a.lead_opportunity, a.mobile,a.photo,a.status_updated_on,";
...
//designation		
	...
	{
		$query = $this->db->query('SELECT a.*,b.firstname from pms_job_apps_calls a left join pms_admin_users b on a.admin_id=b.admin_id where a.candidate_id='.$candidate_id.' order by a.app_call_id desc');
		if($query->num_rows()>0)
		{
...
$this->db->query("update pms_candidate_files set file_name='no_photo.png',file_type='no_photo.png' where file_name='".$row['photo']."' and candidate_id=".$id);
	...
	function exp_years_to()
	{
	 ...
	 if($this->input->post('date_of_birth')!='')$age = $this->get_age($this->input->post('date_of_birth'));
		...
		$data =array(
			'candidate_id'=> $id,
			'admin_id'=> $_SESSION['company_session'],
