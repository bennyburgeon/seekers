<?php
class Walkinsmodel extends CI_Model{
	...
	function record_count($searchterm)
	{
		if($searchterm=='')
		{
			$sql="select count(*)as interview_id from pms_walkins";
		...
		{
			$sql="select count(*)as interview_id from pms_walkins where walkin_title like '%" . $searchterm . "%'";
			$query = $this->db->query($sql);
			$row=$query->row_array();
			return $row['interview_id'];
		}
	...
	{		
		...
		$sql="SELECT a.*,b.job_title,c.company_name FROM pms_walkins a left join pms_jobs b on a.job_id=b.job_id left join pms_company c on b.company_id=c.company_id where c.company_id=".$_SESSION['company_id'];
		//$sql.=" order by a.interview_id ".$sort_by." limit ".$start.",".$limit;
		$sql.=" order by a.interview_id desc limit ".$start.",".$limit;
		$query = $this->db->query($sql);
	...
	{
		$query = $this->db->query('select a.job_id, a.job_title,b.company_name,b.user_id from pms_jobs a inner join pms_company b on a.company_id=b.company_id where a.company_id='.$_SESSION['company_id'].' order by a.job_title asc');
