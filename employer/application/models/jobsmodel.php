...
class Jobsmodel extends CI_Model 
{
	...
	function record_count($searchterm,$job_status,$company_id,$job_priority) 
	{
		...
		$cond = ' company_id='.$_SESSION['company_id'];
	...
	function get_list($start,$limit,$searchterm,$sort_by,$job_status,$company_id,$job_priority)
	{
		...
		$cond=' a.company_id='.$_SESSION['company_id'];
...
//		exit();
	...
	{ 
		...
		$this->db->query("update pms_job_apps_interviews set rejected_by=".$_SESSION['company_session']." where interview_id=".$data['candidate_id']);
...
//Job INDUSTRIES DROPDOWN LIST
	function fill_jobindustry()
	{
		$query = $this->db->query('select distinct job_cat_id, job_cat_name from pms_job_category order by job_cat_name asc');
		$dropdowns = $query->result();
		$dropDownList['']='Select Job Industry';
		foreach($dropdowns as $dropdown)
		{
			 $dropDownList[$dropdown->job_cat_id] = $dropdown->job_cat_name;
		}
...
function all_func_list()
    ...
    {
		$query=$this->db->query("select distinct a.skill_id, a.skill_name from pms_candidate_skills a inner join pms_skills_to_designation b on a.skill_id=b.skill_id where b.desig_id=".$desig_id." order by a.skill_name");
	...
	function get_domain()
   {
	...
	function function_list_by_category($category_id='')
    {
		$dropDownList=array();
		$dropDownList['']='Select Function';	
	 	$query=$this->db->query("select a.func_id,a.func_area from pms_job_functional_area a inner join pms_job_func_to_category b on a.func_id=b.func_id where b.job_cat_id=".$category_id." order by a.func_area asc");
	 ...
	 function functional_without_category()
	{
		$query = $this->db->query('select distinct func_id, func_area from pms_job_functional_area where func_id not in (select func_id from pms_job_func_to_category) order by func_area asc');
		$dropdowns = $query->result();
		$dropDownList['']='Select Function';
		foreach($dropdowns as $dropdown)
		{
