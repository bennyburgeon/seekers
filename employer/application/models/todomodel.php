...
class Todomodel extends CI_Model
{
	...
	function record_count($searchterm) 
	{
		...
		if($searchterm!='')
		{
			...
			//$cond.=" and connum=".$connum;
				...
				$cond =" a.created_by=".$_SESSION['company_session']." and b.title like '%" . $searchterm . "%'";
			}	
		} 
		else{
			$cond = " a.created_by=".$_SESSION['company_session'];
		}
	...
	/*function get_list($no_rec, $offset)
    {
       	...
       	$query=$this->db->query("select a.*,b.* from pms_todo a inner join pms_todo_description b ON a.todo_id=b.todo_id where a.created_by=".$_SESSION['company_session']);
		return $query->result_array();
    }*/
    
	function get_all()
    {
       	$query=$this->db->query("select a.*,b.* from pms_todo a inner join pms_todo_description b ON a.todo_id=b.todo_id where b.language_id=1 and a.created_by=".$_SESSION['company_session']);
		return $query->result_array();
    }	
