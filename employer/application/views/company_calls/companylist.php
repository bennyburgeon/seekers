...
<a href="<?php echo base_url();?>index.php/company_calls/add" class="attach-subs tools"><img src="<?php echo base_url('assets/images/plus.png');?>">Add New</a>
...
<form name="form1" method="post" id="form1" action="<?php  echo $this->config->site_url();?>/company_calls/multidelete?rows=<?php echo $rows;?>" >
<table class="tool-table new">
<thead>
								<tr role="row" class="heading">
<th width="24"><div class="checker"><span><input type="checkbox" class="group-checkable" id="selectall"></span></div></th>
                                        <th width="534"> <a href="<?php echo $this->config->site_url()?>/company_calls?sort_by=<?php if($sort_by=='asc'){echo 'desc';}else{echo 'asc';};?>&searchterm=<?php echo $searchterm;?>&rows=<?php echo $rows;?>">Company Name</a></th>
                                    <th width="304">Contact Details</th>
                                    <th width="71">Actions</th>
                                    <th width="93">Next Call</th>
                                </tr>
</thead>
<tbody>
...
if($records!=NULL)
{
	foreach($records as $result){ ?>
		<tr class="odd gradeX">
			...
			<td>
				<a href="<?php echo base_url();?>index.php/company_calls/edit/<?php echo $result['company_id']?>" class="views" title="Edit this company"><?php echo $result['company_name']?></a><br> 
				
				<?php if($result['total_flp']>0){ ?>
                <a href="javascript:;" title="Get list of all calls made" onclick="get_calls_history(<?php echo $result['company_id'];?>);"  id="get_calls_company" class="btn btn-info btn-xs"> History[<?php echo $result['total_flp']?>]</a>
				<?php }else{ ?>                
		...
		<a href="<?php if(isset($host_ar['path']) && $host_ar['path']!='')echo 'http://'.$host_ar['path']; if(isset($host_ar['host']) && $host_ar['host']!='') echo ''.$host_ar['host'];?>" target="_blank"><?php echo $result['company_website'];?></a></td>
            ...
            <a href="javascript:;" title="Log Calls" onclick="add_calls(<?php echo $result['company_id'];?>);"  data-url="<?php echo base_url(); ?>index.php/company_calls/add_calls/?company_id=<?php echo $result['company_id'];?>"  id="add_calls" class="btn btn-info btn-xs"> Make Calls</a>
			<br><br>
            ...
            <td>
            <a href="javascript:void();" class="btn btn-warning btn-xs"><?php if($result['flp_next_date']!='')echo $result['flp_next_date'];else echo 'Not Set';?></a><br> <br> 
			
			<?php if($result["flp_date_diff"]<0){?>
            <a href="javascript:void();" class="btn btn-danger btn-xs"><?php echo $result["flp_date_diff"];?> &nbsp;Days Left</a>
			<?php }elseif($result["flp_date_diff"]>0){?>
            <a href="javascript:void();" class="btn btn-info btn-xs"><?php echo $result["flp_date_diff"];?>&nbsp; Days More</a> <?php } ?></td>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <div class="col-md-15">
        <form class="form-horizontal form-bordered"  method="post" action="<?php echo $this->config->site_url();?>/company_calls/add_calls" id="calls_form" name="calls_form"> 
                ...
                <tr>
                  ...
                  data-url="<?php echo $this->config->site_url();?>/company_calls/add_calls" />
...
<div class="modal fade" id="jobs_modal" role="dialog" aria-labelledby="enquiry-modal-label">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <div class="col-md-15">
        <form class="form-horizontal form-bordered"  method="post" action="<?php echo $this->config->site_url();?>/company_calls/add_jobs" id="jobs_form" name="calls_form"> 
                ...
                <tr>
				 ...
				 <tr>
                  ...
                  data-url="<?php echo $this->config->site_url();?>/company_calls/add_jobs" />
...
$(document).ready(function()
{
	...
	$("#sel_limit1").change(function(){
		...
		window.location.href = '<?php echo $this->config->site_url();?>/company_calls?limit='+limits;
	});
...
function get_calls_history(company_id){
	...
	$('#show_followup_history').html('');	
	 $.ajax({			
			type: 'POST',
			url:"<?php echo base_url(); ?>index.php/company_calls/get_calls_history/?company_id=<?php echo $result['company_id'];?>",
