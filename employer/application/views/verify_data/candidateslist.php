...
<br>
<form id="searchForm" method="get" action="<?php echo $this->config->site_url()?>/verify_data/">
    ...
    <table class="tool-table new">
        <thead>
            <tr role="row" class="heading">
                ...
                <th><a href="<?php echo $this->config->site_url()?>/verify_data?sort_by=<?php if($sort_by=='asc'){echo 'desc';}else{echo 'asc';};?>&limit=<?php echo $limit;?>&search_name=<?php echo $search_name;?>&search_email=<?php echo $search_email;?>&search_mobile=<?php echo $search_mobile;?>&rows=<?php echo $rows;?>">Candidate Name</a></th>
                
                <th><a href="<?php echo $this->config->site_url()?>/verify_data?sort_by=<?php if($sort_by=='asc'){echo 'desc';}else{echo 'asc';};?>&limit=<?php echo $limit;?>&search_name=<?php echo $search_name;?>&search_email=<?php echo $search_email;?>&search_mobile=<?php echo $search_mobile;?>&rows=<?php echo $rows;?>">Email</a></th>
                
                <th>Reg Date</th>
                <th><a href="<?php echo $this->config->site_url()?>/verify_data?sort_by=<?php if($sort_by=='asc'){echo 'desc';}else{echo 'asc';};?>&limit=<?php echo $limit;?>&search_name=<?php echo $search_name;?>&search_email=<?php echo $search_email;?>&search_mobile=<?php echo $search_mobile;?>&rows=<?php echo $rows;?>">Mobile</a></th>
        ...
        <tbody>
			...
			<?php if($records!=NULL){ $i=0; foreach($records as $result){ $i+=1; ?>            
            	<tr class="odd gradeX">
                    ...
                    <a href="<?php echo base_url();?>index.php/verify_data/summary/<?php echo $result['candidate_id']?>" class="views" title="View">Manage</a>&nbsp;|&nbsp;
                    <a href="<?php echo base_url();?>index.php/verify_data/profile_approval/<?php echo $result['candidate_id']?>" class="views" title="Verify">Verify</a>
...
$(document).ready(function()
{
	...
	$("#sel_limit1").change(function(){
		...
		window.location.href = '<?php echo $this->config->site_url();?>/verify_data?limit='+limits+'&search_name='+search_name+'&search_email='+search_email+'&search_mobile='+search_mobile+'&reg_status='+reg_status;
	});
	 ...
	 {  // triggred submit
		...
		if(count_checked >0) {
			if($('#admin_id').val() == 0){
				...
				$.ajax({
					type:"POST",
					url: "<?php echo $this->config->site_url();?>/verify_data/assignAdmin",
					data:{ 
					...
					success: function(msg) {
						...
						window.location='<?php echo $this->config->site_url();?>/verify_data';
						}
						else{
	...
	$("#btn_change_status").click(function()
	 {  // triggred submit
		...
		$.ajax({
			type:"POST",
			url: "<?php echo $this->config->site_url();?>/verify_data/change_status",
			data:{ 
					'selectedArr' : selectedArr,
