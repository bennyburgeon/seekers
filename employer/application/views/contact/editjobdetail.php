...
<td>From Date</td>
<td><input class="form-control hori " type="text" name="from_date" id="datepickfrom" value="<?php echo $formdata['from_date'];?>" placeholder="Enter From Date Date YYYY-MM-DD"></td>
</tr>
</tr>
<td>To Date</td>
...
<input type="button" class="attach-subs" value="Update & Continue" id="edit_candidate4" style="width:180px;">
...
$( document ).ready(function() {
   ...
   $('#edit_candidate4').click(function(){
		...
		if(isContactValid) {
			...
			$.ajax({
				type: "post",
				url: "<?php echo site_url('contact/editJobDetail'); ?>"+'/'+candidateId,
				cache: false,				
				data: dataStringprop,
				success: function(json){
					try{		
						...
						if(ret['STATUS']==1) {
                            ...
                            var site_url = "<?php echo site_url('contact/loadEditFilehtml'); ?>" +'/'+ candidateId;
...
var dataStringprop = $("#candidate_form4").serialize();
	$.ajax({
				type: "post",
				url: "<?php echo site_url('contact/skip_step4'); ?>"+'/'+candidateId,
				cache: false,				
				data: dataStringprop,
				success: function(json){
					try{		
						...
						if(ret['STATUS']==1) {
                            ...
                            var site_url = "<?php echo site_url('contact/loadEditFilehtml'); ?>" +'/'+ id;
                            $("#step1").load(site_url, function() {
                                //alert("success step2");
                            });
						}
				...
				success: function(json){
					try{		
						...
						if(ret['STATUS']==1) {
							...
							$("#step1").html('<img src="'+img_path+'" alt="Uploading...."/>');
							var id = ret['SUCCESS_ID'];
                            var site_url = "<?php echo site_url('contact/loadEditEducationhtml'); ?>" +'/'+ id;
                            $("#step1").load(site_url, function() {
                                //alert("success step2");
                            });
						}
