...
<input type="button" class="attach-subs" value="update & Continue" id="edit_candidate1" style="width:180px;">
...
<a href="<?php echo $this->config->site_url();?>/contact/summary/<?php echo $candidate_id ?>" class="attach-subs subs">Done</a>
</span>
</td>
</tr>
</tbody>
...
$( document ).ready(function() {
   ...
   $('#edit_candidate1').click(function(){
		...
		var candidateId = '<?php echo $candidate_id ?>';
			$.ajax({
				type: "post",
				url: "<?php echo site_url('contact/editCandidateDetail'); ?>"+'/'+candidateId,
				cache: false,				
				data: dataStringprop,
				success: function(json){
					try{		
						...
						if(ret['STATUS']==1) {
                            ...
                            var site_url = "<?php echo site_url('contact/loadEditPassporthtml'); ?>" +'/'+ candidateId;
...
$('#country_id').change(function() {
	...
	if($('#country_id').val()=='')return;
		...
		$.ajax({
		  type: 'POST',
		  url: '<?php echo $this->config->site_url();?>/contact/getstate/',
...
$('#state_id').change(function() {
	...
	if($('#state_id').val()=='')return;
		...
		$.ajax({
		  type: 'POST',
		  url: '<?php echo $this->config->site_url();?>/contact/getcity/',
...
$('#city_id').change(function() {
	...
	if($('#state_id').val()=='')return;
		...
		$.ajax({
		  type: 'POST',
		  url: '<?php echo $this->config->site_url();?>/contact/getlocation/',
				...
				success: function(json){
					try{		
						...
						if(ret['STATUS']==1) {
							...
							$("#step1").html('<img src="'+img_path+'" alt="Uploading...."/>');
							var id = ret['SUCCESS_ID'];
                            var site_url = "<?php echo site_url('contact/edit'); ?>" +'/'+ id;
