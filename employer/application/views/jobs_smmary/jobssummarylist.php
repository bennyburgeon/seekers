...
<a href="<?php echo base_url();?>index.php/jobs_summary/add" class="attach-subs tools"><img src="<?php echo base_url('assets/images/plus.png');?>">Add New</a>

|| 
<a href="<?php echo base_url();?>index.php/jobs_summary/import_csv" class="attach-subs tools"><img src="<?php echo base_url('assets/images/plus.png');?>">Import CSV</a>
...
<tr>
<td width="10%"><?php echo form_dropdown('admin_id', $bde_list , $admin_id,'style="width:100px;" class="form-control"  id="admin_id" ');?> </td>
<td width="10%"><?php echo form_dropdown('company_id', $company_list , $company_id,'style="width:200px;" class="form-control"  id="company_id" ');?></td>
<td width="80%">
...
<form name="form1" method="post" id="form1" action="<?php  echo $this->config->site_url();?>/jobs_summary/multidelete?rows=<?php echo $rows;?>" >
...
if($records!=NULL)
{
	foreach($records as $result){ ?>
		<tr class="odd gradeX">
            ...
            <td><strong><font color="#3366CC"><a href="<?php  echo $this->config->site_url();?>/jobs/manage/<?php echo $result['job_id']?>" target="_blank"><?php echo $result['job_title']?></a></font></strong><br><?php echo $result['company_name']?></td>
        ...
        <!--     <td <?php if($result['total_interview_rejected']>0)echo 'bgcolor="#DC3912"';?>><?php echo $result['total_interview_rejected']?></td> -->
            ...
            <td <?php if($result['total_offered']>0)echo 'bgcolor="#E3E3E3"';?>><?php echo $result['total_offered']?>              <?php //echo $result['total_offered_rejected']?></td>
            <td <?php if($result['total_offered']>0)echo 'bgcolor="#E3E3E3"';?>><?php echo $result['total_offered_accepted']?></td>
...
$(document).ready(function()
{
	...
	$("#sel_limit1").change(function(){
		...
		window.location.href = '<?php echo $this->config->site_url();?>/jobs_summary?limit='+limits;
	});
...
function get_calls_history(app_id){
	...
	$('#show_followup_history').html('');	
	 ...
	 $.ajax({			
			type: 'POST',
			url:"<?php echo base_url(); ?>index.php/jobs_summary/get_calls_history/",
...
function assign_requirement(app_id)
{
	...
	$('#data_holder').html('Loading..................');	
	 $.ajax({			
			type: 'POST',
			url:"<?php echo base_url(); ?>index.php/jobs_summary/assign_requirement/?app_id="+app_id,
