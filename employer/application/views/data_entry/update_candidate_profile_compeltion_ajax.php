...
//BEGIN FILE DETAILS
	$('#candidate_form5').submit(function(evt){
        ...
        $.ajax({
			...
			type: 'POST',
			url: "<?php echo site_url('data_entry/editfiles'); ?>"+'/'+candidateId,
			data:formData,
			cache:false,
			contentType: false,
			processData: false,
		...
		/*$(this).ajaxSubmit({success:function(data){ 
			...
			if(($.trim(data)=='Add title')||($.trim(data) == 'Invalid file format')||($.trim(data) == 'Choose file')||($.trim(data) == 'Image file size max 1 MB')||($.trim(data) == 'failed')){
				...
				window.location.href = '<?php echo $this->config->site_url();?>/data_entry/';
			}	
		}*/
		});
});
...
$('#country_id').change(function() {
	...
	if($('#country_id').val()=='')return;
		...
		$.ajax({
		  type: 'POST',
		  url: '<?php echo $this->config->site_url();?>/data_entry/getstate/',
...
$('#state_id').change(function() {
	...
	if($('#state_id').val()=='')return;
		...
		$.ajax({
		  type: 'POST',
		  url: '<?php echo $this->config->site_url();?>/data_entry/getcity/',
...
$('#city_id').change(function() {
	...
	if($('#state_id').val()=='')return;
		...
		$.ajax({
		  type: 'POST',
		  url: '<?php echo $this->config->site_url();?>/data_entry/getlocation/',
