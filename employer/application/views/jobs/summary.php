...
<div class="profile_top_left">Job Summary</div>  
...
<form class="form-horizontal form-bordered" action="<?php echo $this->config->site_url();?>jobs/send_shortlisted"  method="post" id="shortlisted_form" name="shortlisted_form"> 
    ...
    <table border="0" cellpadding="0" cellspacing="0" width="100%" class="table table-bordered table-condensed">
      ...
      <tbody>
        <?php $i=0; foreach($shortlisted as $candidate){ $i+=1;?>
        <tr>
           <td width="2%"><?php echo $i;?>
           </td>
           <td width="20%">   
                    
           <a href=#" title="View Profile" ><?php echo $candidate['first_name'].' '.$candidate['last_name'];?></a> 
             
             <a href="<?php echo $this->config->item('client_cv_preview').'profile_rms?candidate_id='.md5($candidate['candidate_id']).'&job_app_id='.$candidate['job_app_id'];?>" target="_blank" class="btn btn-warning btn-xs" title="View Profile">Client CV</a>
...
<tr>
    ...
    <?php if($offerletter['offer_status']==1){?>		
      ...
      <a href="javascript:;" onclick="accept_offer(<?php echo $formdata['job_id'].','.$offerletter['app_id'].','.$offerletter['candidate_id']?>);" class="btn btn-success btn-xs"> Accept </a> 
      
       <a href="javascript:;" onclick="reject_offer(<?php echo $formdata['job_id'].','.$offerletter['app_id'].','.$offerletter['candidate_id']?>);" class="btn btn-warning btn-xs"> Reject </a>&nbsp;
       
       <!-- <a href="javascript:;"  data-url="<?php echo base_url().'index.php/jobs/delete_offercandidate/?job_app_id='.$offerletter['app_id'].'&candidate_id='.$offerletter['candidate_id'].'&job_id='.$formdata['job_id'];?>"  id="delete_offer_candidate" class="btn btn-danger btn-xs">X </a> -->
