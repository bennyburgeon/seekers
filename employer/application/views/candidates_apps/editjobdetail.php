<form class="form-horizontal form-bordered" action="<?php echo site_url('candidates_apps/update_job_details'); ?>"  method="post" id="update_job_form" name="update_job_form" >
  ...
  <table class="hori-form">
    <tbody>
      ...
      <tr>
        <td>Industry</td>
        <td><?php echo form_dropdown('job_cat_id',  $industry_list, $formdata['job_cat_id'],'class="form-control" id="job_cat_id_prof_edit"');?></td>
      </tr>
      <tr>
        <td>Department</td>
        <td><?php echo form_dropdown('func_id',  $functional_list, $formdata['func_id'],'class="form-control" id="func_id_prof_edit"');?></td>
      </tr>
      <tr>
        <td>Designation</td>
    ...
    <tr>
      <td colspan="2"><span class="click-icons">
        ...
        ||
        <input type="button" class="attach-subs" onClick="window.location='<?php echo site_url('candidates_apps/summary'); ?>/'" value="Cancel" id="cancel_update_job_profile" style="width:180px;">
        </span></td>
    </tr>
    </tbody>
...
$('#job_cat_id_prof_edit').change(function() {
	...
	if($('#job_cat_id_prof_edit').val()=='')return;
		...
		$.ajax({
		  type: 'POST',
		  url: '<?php echo $this->config->site_url();?>candidates_apps/getdepartment/',
...
$('#func_id_prof_edit').change(function() {
	...
	if($('#func_id_prof_edit').val()=='')return;
		...
		$.ajax({
		  type: 'POST',
		  url: '<?php echo $this->config->site_url();?>candidates_apps/getdesignation/',
