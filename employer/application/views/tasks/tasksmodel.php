...
class Tasksmodel extends CI_Model{
	...
	{
		$query = $this->db->query("SELECT a.user_id,b.firstname from ".$this->project_team." a JOIN ".$this->admin_users." b on a.user_id=b.admin_id where a.project_id=".$id);
		$dropDownList = array();
	...
	{
		$data = array();
		$query=$this->db->query("select a.user_id,b.firstname from ".$this->project_team." a join ".$this->admin_users." b on a.user_id=b.admin_id where project_id=".$id);
		...
		foreach($team_list as $dropdown)
		{
			...
			$dropDownList["name"] = $dropdown->firstname;
